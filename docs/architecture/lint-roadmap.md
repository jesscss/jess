...
| No matching mixin/function overload | Compiler/evaluator | Initial shared same-file mixin diagnostics landed as `mixin/undefined` for Less/SCSS/Jess calls and `call/no-matching-overload` for Less fixed-arity mixin calls plus SCSS positional mixin/function calls whose same-file simple definitions have no arity match. `CallSignature` and `OverloadSet` facts can later handle namespaces, named function arguments, guards, imports, defaults/rest params beyond the current conservative SCSS arity range, and richer overload messages. |
