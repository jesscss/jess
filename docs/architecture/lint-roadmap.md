...
| Landed | Vendor-prefix style policy | `property-no-vendor-prefix`, `at-rule-no-vendor-prefix` | Opt-in Stylelint-name diagnostics for authored CSS vendor-prefixed property names and keyframe at-rules; distinct from recommended VSCode `vendor-prefix` missing-standard diagnostics. |
