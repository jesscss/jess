...
| P13 | `.jess` has **three `$` sigil forms, chosen by POSITION**: `${…}` = interpolation in name/selector/string positions, body is a **NAME** (Less-style, not an expression — "so you don't stuff too much in interpolation positions"); `$[…]` = **lookup** in value position, body is the key value; `$(…)` = expression/grouping in value position. `${…}` was ADDED because `$[…]` is a lookup against the ambient scope — `$[$foo]` already means "the variable NAMED BY `$foo`" — leaving no plain interpolation spelling, and bare interpolation is impossible because `-` is an identifier byte (`--$name-color` has no name boundary). `${m[key]}` / `${m.key}` are a backward-compatible LATER extension and are deliberately not pre-built. `${…}` was verified free in `.jess` before landing. | SETTLED (owner 2026-07-24; `${…}` landed) | jess-parser `DirectJessDollarBrace`; `docs/jess/02-Language/08-interpolation.mdx` |
