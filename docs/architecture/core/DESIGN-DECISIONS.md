...
| F6 | NUMBER OUTPUT POLICY: a COMPUTED number emits the shortest decimal within a RELATIVE tolerance of `1e-10` of the double — noise removal, not a precision limit. NO significant-figure cap (a cap saves 0.077% of corpus bytes and destroys earned digits at magnitude ≥ 10) and NO per-unit policy. Magnitude-invariant, so small magnitudes survive where the old 8-decimal floor flattened everything under ~5e-9 to `0`. An EXACT INTEGER is never trimmed — it carries no float noise, so there is nothing to remove. Scientific notation is never emitted (CSSOM §6.7.2). One policy for every position: declaration value, interpolation splice, property name, selector — the `emitValueInterp` full-precision bypass is DELETED. Un-operated literals are governed by V1, not by this row, and the 8-dp denoise that used to contradict V1 in `dimensionFromFields` is gone. | SETTLED (owner 2026-07-24; landed) | `../../design/numeric-precision-policy.md`; `packages/core/src/ast/format-number.ts`; resolves SEMANTIC-INVARIANTS **S1** |
