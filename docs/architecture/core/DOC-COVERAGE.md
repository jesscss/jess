...
| Number precision (shortest decimal within 1e-10; one policy every position) | ✓ `ast/format-number.ts` (`formatNumber`) ← `ast/serialize-value.ts`, `ast/color.ts`, `ast/literal-tag.ts` | ✓ DD `F6` · `V5-OUTPUT-SEMANTICS.md` A9 · `../../design/numeric-precision-policy.md` | ✓ `advanced/number-precision` (rewritten) | ✓ `06-Advanced/10-number-precision` (rewritten) |
