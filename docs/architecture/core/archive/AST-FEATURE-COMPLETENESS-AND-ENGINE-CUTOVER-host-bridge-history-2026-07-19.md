...
> **Execution correction (owner, 2026-07-18).** The historical C1/C2 sequencing
> below must not preserve or extend `ParseBuildHost`, `BuilderHost`, action maps,
> `parse*Fn`, source reparsing, or `renderAst*` names. Parser-local grammar
> reductions construct canonical AST nodes directly through `@jesscss/core/ast`;
> deleting duplicate producers leads, and old internal consumers are repaired or
> removed afterwards. Import/interpolation facts are first-parse structure, never
> a variable-sniff or reparse subsystem. `HANDOFF.md` and
> `AST-REORG-EXECUTION.md` are the live execution authority.
...
> **Superseded sequencing (owner, 2026-07-18):** this document's feature-first deletion
> gate is no longer active. AST-v2 canonicalization, grammar fusion, and deletion of
> `parse-host`/`BuilderHost` proceed first; internal legacy consumers may intentionally go
> red and are repaired or deleted afterwards. Keep this document's feature-gap inventory and
> final correctness gates, but use `AST-REORG-EXECUTION.md` for the live execution order.
>
> **Historical boundary.** The dual-host, `parse-host`, action-list, and
> `less-compat` bridge descriptions below explain the observed feature gaps at
> the time of measurement. They are not implementation options: the deleted
> bridge and host have no replacement. Retain the feature/eval inventory and
> independent output evidence; implement remaining work through parser-local
> direct AST construction and the core engine.

---

## 0. Historical architecture evidence (the former dual-target reality)
...
## 2. Historical cutover analysis — retained feature/eval evidence

The former staged cutover is complete/superseded. The feature gaps in §1 remain
valid closure work, but do not authorize reviving a host, bridge, compatibility
adapter, or dual-target parser.
...
## 3. Historical `less-compat` bridge analysis
