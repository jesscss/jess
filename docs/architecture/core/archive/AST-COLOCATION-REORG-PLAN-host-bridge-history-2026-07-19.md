...
> **Execution correction (owner, 2026-07-18).** This detailed plan remains useful
> for its inventory and family move map, but its old host-preserving clauses are
> superseded. Do not add v2 `buildNode` cases, a v2 `BuilderHost`, a parser action
> registry, `parse*ToAst` public APIs, `ImportState`, variable-sniff passes,
> `parseLessFn` reparses, or `%%` text splices. These are invalid structures to
> delete, not work to relocate. Dialect grammars reduce directly to parser-local
> functions backed by `@jesscss/core/ast`, export the uniform operation `parse`,
> and emit typed import/interpolation/trivia/extend facts on the first parse.
> `HANDOFF.md` and `AST-REORG-EXECUTION.md` own live ordering and gates.
>
> **Reading boundary.** The detailed `parse-host`, `BuilderHost`, action-list,
> and bridge descriptions below are retained only as historical evidence for
> the grammar and family inventory. They are not relocation work: those seams
> are deleted, have no successor, and must not be restored to execute any
> remaining plan.
...
2. **PEER OBJECTIVE — parser regex-recovery elimination (§0.11):** the maintained `less-parser/src/builders.ts` (3,281 lines, ~63 build-path regexes) throws away Parseman's structured output and re-parses strings with regex — directly undermining the parser-optimization work. The cure is the SAME as the keystone's relocated construction: grammar owns structure, builder is a thin reader, pattern-matching only inside `regex()`. First-class, same acceptance gate, tracked as a ratchet.
3. **Family co-location** of everything that STAYS in core (engine / value / expr-eval / selector-compose / rule-merge / mixin / extend / serialize), plus duplicate-name resolutions and the `engine/emit` spine, per `AST-V2-STRUCTURE-BLUEPRINT.md` (HYBRID, owner-ratified 2026-07-16).

The keystone (§0) and the peer regex-recovery objective (§0.11) share ONE principle — *grammar owns structure, builder is a thin reader* — applied on both sides of the package boundary. §1–§7 (co-location) apply to the residual, core-resident tree.
...
### 0.2 Historical `FunctionalParseHost` callback evidence
...
Split-module note: a handful of actions (`selector-interp`, `value-expr`) carry both a CSS-base arm and a Less arm. The css-parser builder owns the base; the less-parser builder (which `extends` it) overrides/augments the Less arm — exactly as legacy `buildNode` inheritance already does. Determine each split at the `buildNode`-case granularity when executing; the legacy split is the reference.
...
**Bucket (c) — DISSOLVES (plumbing, §0.5):** `dispatch-host` `ParseBuildHost`/`parseToAst`/`run`, `actions/index.ts` `ACTION_LIST` registry, `host-context` `BuildContext`/`BuildArgs`/`BuildFn`/`Placeholder`/`isPlaceholder`/`isStatement`. No structure re-derivation and no successor dispatch: grammar reductions call parser-local typed factories directly.

### 0.8 Historical invalid seams (use only for behavior inventory)
...
**(a) `import.ts` (631) — a whole resolution SUBSYSTEM, not a node builder.** It is post-parse `@import` resolution: `createImportState()`/`ImportState` (per-parse `seen: Set`, `stack`, `varScopeCache: Map`, `entry`), `collectFileVars`/`importScopeVars` (reads files via `fs`, RE-PARSES them through `parseLessFn`), `interpTemplateOf`/`fillInterpTemplate` (fills an `Interpolated{source,replacements}` path template by splitting `source` on `%%` at `:132` — this is the KEYSTONE-COMPLIANT consumer: it reads the parser's structure, not a re-tokenize), `isCssPassthrough`/`resolveLessPath`, `resolveImportStatements`/`resolveDirectImports`/`spliceImport` (the splice that replaces a `StyleImport` head with the imported file's statements).
  - **Disposition (superseded):** delete this subsystem. The Less grammar emits a
    complete typed import fact on its first parse; a dialect may parse newly loaded
    source once, but never sniffs variables, reparses text, splices text, or carries
    `ImportState`/`%%` template machinery.
  ...
  - Active gate: migrate useful import behavior coverage to parser/core seams;
    do not move or recreate a bridge fixture harness.
...
**(b) The `:extend` marker protocol — cross-family parse-time mutable state, must move ATOMICALLY.** `host-context.ts:242–285`: `ExtendMarker`/`extendMarker`/`isExtendMarker` + `ExtendTargetMarker`/`extendTargetMarker`/`isExtendTargetMarker` + `attachSelectorExtends`/`takeSelectorExtends` + the per-parse `ctx.selectorExtends` WeakMap. The protocol spans TWO builders: the **selector** builder (css-parser) recognizes `:extend()` and calls `attachSelectorExtends`; the **ruleset**/**extend** builder DRAINS the side table (`takeSelectorExtends`) onto the enclosing `Rule`. Today it lives in the shared `host-context` precisely so both families see it without a cross-family import — after relocation that shared home is gone.
  - **Disposition (superseded):** delete the markers and `selectorExtends` WeakMap.
    The Less grammar emits the typed extend fact and its reduction attaches it
    directly; no parser build context or cross-family side channel replaces it.
  - **Atomicity:** change producer and consumer together so no transient marker
    protocol survives.
...
**(c) trivia-decode + `declParts`/`sliceSpan` crossing the package boundary — the byte-identity risk.** `sliceSpan` (`ctx.src.slice`), `declParts` (decl byte-split), `selectorText` (raw selector recovery), `rulesetBodyWindow`, and the trivia log decode (`hasWhitespaceTriviaBefore`/`blockCommentTrivia`/`TRIVIA_STRIDE`/`TRIVIA_*`) encode EXACT span/trivia semantics that the byte-identity output depends on.
  - **Disposition:** grammar structure and Parseman trivia are the only source of
    spans and bounds. Delete byte slicing and `declParts` recovery rather than
    porting it; prove output with focused parser/core behavior coverage.

### 0.9 Tier-B (#6) grammar plan for interpolation-bearing families
...
### 0.11 FIRST-CLASS OBJECTIVE — `builders.ts` regex-recovery: Parseman's structure thrown away and re-parsed

This is a PEER objective to the parse-host relocation (§0), not a footnote. `packages/less-parser/src/builders.ts` (**3,281 lines, ~63 raw-regex sites**) is the maintained less-parser builder, and it actively **undermines the Parseman parser-optimization work**: the grammar produces structured output, and the build path THROWS IT AWAY and re-parses strings with regex. Every such site is parser effort spent, then discarded. Same principle, same gate as the relocated ast construction: **grammar owns structure, builder is a thin reader, pattern-matching only inside `regex()`.**

**The exemplar — `builders.ts:400 _buildNsAccessor` (a BIFURCATION, not a mere flatten).** The grammar is `NsAccessor = node(noTrivia(sequence(nsHead, refIndex, many(choice(refIndex, refCall)))))` (`grammar.ts:530`). The TAIL — `many(choice(refIndex, refCall))` — is PROPERLY recursive (chained `[index]` / `(call)`). But the HEAD is `nsHead = regex(/[.#]…(?:[.#]…)*/)` (`grammar.ts:529`), a REGEX that flattens `#ns.options` to a string, which the builder then re-splits with `headText.match(/[#.][^#.>+~\s]*/g)` to recover `['#ns', '.options']`. Two compounding harms:
- **(a) re-parse:** the builder regex-recovers the segments the grammar just discarded.
- **(b) structural incapacity → forced bifurcation:** a regex head CANNOT represent a call-headed namespace like `#ns(1, 3, { bar: foo }).options[@one](blah)[two]` (a call with args + a detached-ruleset arg). So call-headed forms cannot use `NsAccessor` at all — the grammar SHUNTS them to a SEPARATE path (`GluedParen` / `_tryParseNamespaceRef`, `grammar.ts:521–531,781`). One concept — the namespace accessor — is served by TWO grammar paths and TWO builder representations, purely because the flat regex head can't express the recursive form.

**The fix is ONE uniform recursive `node()` for the ENTIRE accessor** — head segments, call-with-args, `[index]`, `(lookup)` all as structured links of a single recursive accessor-chain node, so `#ns.options` and `#ns(args).x[k](y)[z]` are the SAME node shape. NO regex head, NO builder re-split, and the two paths UNIFY into one. The "atomic operand for arithmetic folding" requirement is satisfied by that node being atomic — **a structured node is still one operand; flattening to a string was never required for folding.** This is the strongest evidence for "grammar owns structure": the regex head is not merely slow/ugly, it is structurally INCAPABLE of the recursive form and therefore FORCES the duplicate path.

**This is a PORT REGRESSION in COVERAGE, and the fix restores that coverage in Parseman's own idiom.** The owner already built full support for these cases — namespace accessors with calls / args / detached-ruleset args, repeated calls, repeated lookups, recursion — in the RETIRED CHEVROTAIN parser (`packages/less-parser/src/productions/values.ts`, `@ts-nocheck`; siblings `productions/{selectors,guards,root}.ts`). The Chevrotain→Parseman port was SUPPOSED to audit that and preserve the coverage; for `NsAccessor` it did NOT — it regressed to the lossy regex `nsHead` + the bifurcated `GluedParen`/`_tryParseNamespaceRef` path, dropping the call-headed recursive form entirely. **Chevrotain is the reference for WHICH cases must work and what they MEAN — coverage + semantics — NOT a 1:1 structural template to copy.** Chevrotain carried a COARSENESS forced by its separate pre-tokenization (lexer) stage; Parseman does CONTEXTUAL (scannerless) tokenization and can represent the same cases MORE FINELY and efficiently. So the fix is a single uniform recursive `node()` in **Parseman's cleanest contextual form** — informed by Chevrotain's case list, not slaved to its node shape. Use `productions/values.ts` to confirm the accessor / call / lookup chain cases and their meaning; structure them the way Parseman does best; keep the builder thin. The byte-identity gate confirms coverage/semantics parity.

**Method — a Chevrotain COVERAGE/SEMANTICS audit, structured in Parseman's OWN idiom (NOT a 1:1 structural copy).** Every builder regex-recovery is a candidate PORT REGRESSION: a spot where the Chevrotain→Parseman port UNDER-structured. The retired `productions/` tree (`values.ts` + `selectors.ts`/`guards.ts`/`root.ts`, all `@ts-nocheck`) is the reference for **which cases must work and what they MEAN — coverage + semantics — NOT a node-shape template**. Chevrotain's shapes carry a coarseness FORCED by its separate pre-tokenization (lexer) stage; Parseman is scannerless/contextual and can represent the same cases MORE finely and efficiently. So the fix for each shape is:
- **(a) Don't REGRESS any case Chevrotain supported** — use `productions/` to confirm the full case list (recursive accessors, calls, args, detached-ruleset args, repeated lookups, …) and their meaning.
- **(b) Structure it in Parseman's BEST contextual form** — which may (and often should) be finer/cleaner than Chevrotain's lexer-forced coarseness. Do NOT copy Chevrotain's node shape verbatim; do NOT under-structure either. Parseman's contextual tokenization is a strength to exploit, not a constraint to mirror.
- **(c) Thinnest possible builder** — thin assembly over the grammar's structured children, no regex.

The byte-identity gate confirms COVERAGE/SEMANTICS parity (byte-identical output), not structural sameness with Chevrotain.

**Two failure modes — both cured by "grammar owns structure (Parseman's idiom), builder is a thin reader," both COVERAGE-audited against Chevrotain:**

1. **Flatten-then-recover / bifurcation** — the grammar flattened structure with a regex (for operand identity), the builder regex-recovers it, AND the flat form can't express the full shape so a parallel path is forced. **Fix: replace the regex head with a uniform recursive `node()` in Parseman's cleanest form** — atomic operand AND full structure; the flat string is never needed; the two paths unify. `_buildNsAccessor` / `NsAccessor` vs `GluedParen`/`_tryParseNamespaceRef` is the archetype (Chevrotain confirms the call-headed cases must work; Parseman structures them its own way).
2. **Under-structure** — the grammar NEVER emitted the fine structure; it matched coarsely and the builder tokenizes the string. **Fix: push the structure into the grammar** (a `regex()` combinator or structural rule emitting the finer node), covering the cases Chevrotain covered. This is Tier-B (§0.9) generalized past `@{}` interpolation.

**Audit step (plan action):** for each `builders.ts` regex-recovery site (the four named offenders + the long tail), check Parseman's COVERAGE against the Chevrotain `productions/` reference for that shape — which cases + what they mean. Where Parseman under-structures or drops a case, design the cleanest Parseman `node()` that covers those cases (its own idiom, not Chevrotain's shape) and drop the builder regex; the byte-identity gate ratifies parity. The named offenders (dimension→number+unit, `@import` prelude, value-token classify) are LIKELY the same under-structuring as `NsAccessor` — coverage-check each against `productions/values.ts` before designing.

**Plan RISK / ASSUMPTION (meta-lesson):** the Chevrotain→Parseman port CANNOT be assumed complete — `NsAccessor` proves at least one silent COVERAGE regression shipped (the call-headed recursive form). The grammar-structuring phase must check Parseman's case coverage against Chevrotain per shape and treat any DROPPED or under-structured case as a BUG to fix — the fix being Parseman's cleanest contextual node, informed-by but not slaved-to Chevrotain. A builder regex is the tell-tale; absence of a regex does not guarantee full coverage, so spot-check structurally-rich shapes (accessors, calls, guards, selectors) against `productions/` even where no builder regex flags them.

**Per-site bucketing of `builders.ts` (worst offenders named + assigned a mode + the grammar change that eliminates the regex — NOT all 63; the long tail rides the same two-mode cure). "Grammar change" for every row = cover the Chevrotain `productions/` cases in Parseman's cleanest contextual `node()` (its idiom, not Chevrotain's shape):**

| Offender | Site(s) | Mode | Grammar change that kills the regex |
|---|---|---|---|
| **Ns-accessor head re-split + path bifurcation** | `400` `_buildNsAccessor` — `headText.match(/[#.][^#.>+~\s]*/g)`; `nsHead` regex `grammar.ts:529` vs `GluedParen`/`_tryParseNamespaceRef` | **1 · bifurcation** | ONE recursive `node()` for the whole accessor (segments + call-args + `[index]` + `(lookup)`); drop the regex head; unify `NsAccessor` and the call-headed path. The node is the atomic folding-operand. |
| **Dimension re-split ×2** | `943`, `2653` — `/^(\d+)([a-zA-Z]+|%)?$/.exec(authoredValue)` | **2 · under-structure** | grammar emits `Dimension{ value, unit }` (number child + unit child) instead of one value string. |
| **@import prelude re-parse** | `2367` `/['"]([^'"]+)['"]/`, `2357/2427/2943` `/\bas\s+…/` | **2 · under-structure** | grammar emits typed prelude leaves (path node, `as`-alias, option list) — the import specifier structuring already scoped in §0.9 A0. |
| **Value-token re-classify** | `2525` `singleVarRe`, `2533` `escapedStrRe`, `2564` `varAccRe` | **2 · under-structure** | grammar emits typed value nodes (`VarRef` / escaped `Quoted` / `VarIndirect`·`MapAccessor`) rather than a word the builder re-classifies. |

**CORRECTNESS CHECK (not just cleanup) — the bifurcation may be hiding a latent bug.** Before unifying the accessor node, verify the CURRENT call-headed path (`GluedParen`/`_tryParseNamespaceRef`) actually renders the hard mixed form `#ns(1, 3, { bar: foo }).options[@one](blah)[two]` — repeated calls + repeated lookups interleaved, with a detached-ruleset arg — CORRECTLY today. The flat-head bifurcation exists precisely because `NsAccessor` can't express this, so the call-headed path is where such a form is (or isn't) handled. **Add this as a fixture and verify it during the grammar-structuring**, so unifying the two paths into one recursive node does not silently paper over a pre-existing correctness gap. If it already renders correctly, the unification must preserve that; if it does NOT, that is a bug to fix as part of the work, surfaced by the fixture — not to be masked by the refactor.

**Gate + sequencing:** the §0.10 acceptance gate ("no regex outside `regex()`") applies to `builders.ts` ON THE MAINTAINED PATH — this is not deferred to the legacy-tree deletion (§0.10 point 2 corrected: only the legacy-tree construction dies at A4; these re-parse regexes are maintained and in-scope). Each site is retired with its grammar change, byte-identity gated per shape, as a phase of the generalized grammar-owns-structure effort (A0-family). The §0.10 `builders.ts` exclusion lifts shape-by-shape, reaching empty when the last offender collapses. Track `builders.ts` regex count as a ratchet: monotonically down, never a new `.exec`/`.match`/`.test` added on the build path.
...
## 3. Historical `parse-host` disposition

Superseded by §0 — the directory and its bridge machinery are deleted. Keep the
family mapping only to identify the parser-local grammar reduction or core
behavior test that now owns each surviving semantic concern; do not relocate
its host, action list, import reparse, or test harness.
...
- A2. Stand up direct grammar reductions in **css-parser**, importing only typed
  factories from `@jesscss/core/ast`, with the public entry named `parse`. No
  host, dispatch map, action registry, or dialect-suffixed migration API is
  permitted. The interpolation-bearing families (`at-rules`, `custom-props`)
  move after their A0 structure is available. Gate: parser/canonical-AST proof
  plus the runtime-boundary verifier.
- A3. Extend those direct reductions in **less-parser** for
  mixins/variables/value-ops and then typed imports. `:extend` becomes an explicit
  grammar fact consumed directly by Less; its marker and `WeakMap` protocol are
  deleted atomically. Import facts contain literal/interpolation segments and
  never use stateful sniff/reparse/splice machinery. Gate: canonical-AST behavior
  coverage, then the full corpus and boundary verifier.
- A4. **Completed:** deleted `core/ast/parse-host/` (dispatch-host, host-context,
  actions, import) and the bridge seam. The remaining `builders.ts` coarse-grammar
  re-parse debt is an active A0-family scanner-cleanup target, retired shape by
  shape through grammar structuring; do not recreate a host to service it.
...
**Behavior gating:** preserve relevant parser/core corpus behavior and validate
contested output against the independent Less reference. The former bridge and
whole-document drivers are historical evidence, never an active conversion or
oracle path.
