...
| T7 | **fns Stage F (in-flight)** — convert the last ~11 Less fns to AST‑v2 value shape in `@jesscss/fns` (`FNS-PACKAGE-MIGRATION-SPEC §S6`; HEAD `53e9db8dd` = "get-unit … partial"). Only matters if benchmark.less calls an unconverted fn; core→fns wiring already landed (Stage D/E); as of 2026-07-24 registration derives from the dialect index (`makeLessRegistry` ← `less/index.ts`). | correct fn eval for any unconverted fn used | **PAR** (HARD only if benchmark.less hits one) | S each | per-fn differential + byte-identity |
