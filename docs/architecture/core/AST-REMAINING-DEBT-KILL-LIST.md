...
| `native/list-helper.ts:45/103` `topLevelSplit`/`coerceListItems` split branch | Legacy duplicate of typed list recovery | **C7 list-capability cutover** — delete the dialect-local helper; keep the exact recovery/index primitives in `@jesscss/core/value` until parser/evaluator value assembly retains the `List` fact directly |
...
| `native/list-helper.ts:96` "byte-faithful port" comments | Legacy annotations on the duplicate helper | Delete with the helper; retain parity documentation beside the core recovery API and Less/Sass policies instead. |
