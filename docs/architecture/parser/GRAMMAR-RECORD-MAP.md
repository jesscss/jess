...
- **P33** — `/` in a value is a LIST SEPARATOR with its own rung, not a value atom: comma is loosest, then slash, then whitespace. css spells the rung as `ValueTerm` (`css-parser/src/grammar.ts`), the name scss and jess already use for it. A leading `/` is rejected because a separator has no left operand — the rejection is EMERGENT and there is deliberately NO first-position guard; an implementation that adds one has built the wrong thing. Custom properties and `var()` fallbacks stay permissive (css-variables-1 §2 `<declaration-value>`). Spacing of the emitted slash is **G33**, not P33.
