...
| D6 | `@page` pseudo-page selector | corpus:313-327 | jess | `@page :first {…}`, `@page wide:left {…}` → parse error (consumes nothing) → accepted (css-page-3 §3); `@page wide {…}` parses | HIGH-R | FIXED (jess grammar `PagePseudo`/`pageHeader`; DESIGN-DECISIONS A11) | unowned | pseudo-page arm added to the jess at-rule prelude clause; reduces to one verbatim `Any`, byte-identical to css/scss |
