...
8. **Classify by a UNION plus a NAMED EXCLUSION list, never by an inclusion
   list alone.** Two implementations disagreed 42 against 38 on css. Neither was
   right; the reconciled answer is **40**, and each side contributed two errors
   in opposite directions on the same axis:

   - The permissive script admitted anything const-bound in the factory, so it
     over-counted `cssCase` and `identWord`. Those are **`makeWhen(...)` /
     `makeWord(...)` factory aliases**: they bind a matcher POLICY — a
     case/boundary rule that call sites apply — not a parse table. "Inlined
     transitively at each site" does not describe them, so they are not H1
     sites. GRAMMAR-REBUILD-SPEC §0.2 blesses the authoring form inside a
     `rules()` factory. **Exclude them by name.**
   - The enumerating script admitted only a listed set of combinators, so it
     silently dropped every const bound to one it had forgotten. `otherwise`
     was missing — `COMBINATOR.test('otherwise(choice(')` returned false — and
     the omission was never specific to it. Also required: **`when`, `attempt`,
     `gate`, `leaf`, `parser`, `skip`, `trivia`**, alongside `transform` (§1
     note) and the obvious `sequence`/`choice`/`many`/`node`/`dispatch` set.

   The asymmetry is the point: **a forgotten exclusion is visible in a
   two-name list; a forgotten inclusion is invisible in a forty-name one.**
   Prefer the shape whose failure mode you can see.

9. **Never capture a bounded-length context inside a `matchAll` pattern.** A
   capture group CONSUMES input and advances `lastIndex`, so a trailing context
   capture silently swallows however many following declarations fit inside it.
   They are never classified, never counted, and leave no gap in the output —
   the const does not exist as far as the script is concerned.

   Concretely, `/^ {2}const (\w+) = ([\s\S]{0,40})/gm` over a factory body
   skips any const declared within 40 characters of the previous one.
   `const importAtKeyword = identWord('@import');` is short enough that its
   40-character window swallows the whole of the following
   `const urlOpen = `, which is how a real H1 site went missing while every
   component check on it passed individually.

   **Fix:** read the initialiser without consuming it —
   `/^ {2}const (\w+) = (?=([\s\S]{0,40}))/gm` — or match the initialiser with
   an unbounded but non-greedy pattern anchored to a line start.

   **Detection, which is the durable part:** assert the number of classified
   declarations against an independent count of `const` bindings in the same
   slice. The two must agree. That check would have caught this immediately and
   costs one line. Blast radius when it was fixed, composites detected:
   ...
   **A newline-anchoring hypothesis for this defect was proposed and is
   WRONG** — the `m` flag and the `^ {2}` anchor were both correct, and
   comment-stripping preserved offsets. The cause is `lastIndex` advancement
   and nothing else. Recorded so nobody re-derives the wrong explanation.

**There are now NINE filters, and four of them were found by one count
disagreeing with another count rather than by anyone reading code**: the
`node<T>(` blind spot (#7, caught by "more map keys than composites"), the
type-position family (#5, caught by three implementations giving three
answers), the inclusion/exclusion asymmetry (#8, caught by 42 against 38), and
the `lastIndex` trap (#9, caught by a single unexplained missing row). Review
found none of those four. **That is the argument for keeping two independent
implementations alive rather than consolidating to one** — the disagreement is
the instrument, and consolidating removes it.
...
**But the consequence is NOT universal, and stating it as a universal is an
error two lanes made.** The penalty is `closure depth × reference-path count`,
so at shallow depth and low path count the naming overhead EXCEEDS the
inlining it avoids. Measured, tree-neutral (map entry removed, both `g.X` sites
switched to bare const, nothing else touched):

| shape | `ast.js` raw |
| --- | ---: |
| `CompoundSelector` named + `g.`-referenced | 1,241,692 |
| same, inlined at both sites | **1,237,408** |

**−4,284 B for inlining it.** Its closure is two leaves deep and it is
referenced twice, so `F^d` never gets off the ground and the ~950 B/site naming
cost dominates. Cutting the whole selector recursion in the same grammar was
worth only −7,179 B (0.6%) for the same reason.

So: `g.`-reference the deep spine — value, selector, block, anything recursive
— and do not bother, or actively prefer const, for shallow leaves referenced
once or twice. **Per-const measurement, not a blanket rule.** The 13.69× in
§2.2 and this −4,284 B are the same law at opposite ends of `F^d`.
