...
9. **`makeWhen(...)` / `makeWord(...)` aliases are NOT composites.** They return
   ...
   transitively at each reference" does not describe their cost. `cssCase`
   (`grammar.ts:1045`, 27 call sites) and `identWord` (`:1041`) were ranked the
   most-inlined rules in the css grammar; both are factory aliases and the
...
10. **A consuming initialiser capture advances `lastIndex` past the next
   ...
   `importAtKeyword` declaration's capture. **Every per-row check passes** — the
   classifier returns true, the declaration regex matches, the reference count
   is right — and the row simply never appears.
   **Fix:** read the initialiser with a lookahead or a separate `slice`; never
   capture it. **Detection:** assert declarations *visited* against an
   independent `const` count and refuse to report on mismatch.

**Prefer a permissive include plus a SHORT NAMED EXCLUDE over an enumerated
include-list.** The two shapes fail in opposite directions and both were
observed: an enumerated include under-counts on combinators nobody remembered
to list (three scripts omitted `otherwise`, `when`, `routed`, `noTrivia` and
more, so `const x = otherwise(choice(...))` was invisible to H1 in a
dispatch-heavy grammar); a fully permissive include over-counts on factory
aliases (filter 8). **A forgotten exclusion is visible in a five-line list; a
forgotten inclusion is invisible in a forty-name one.**
...
> **An identity check on the wrong key reports absence for something present.**

Every face below failed **silently and clean**, produced a result that read as
normal, and **not one was caught by its author.** That combination is the
signature: if a check can only fail by returning a plausible value, it needs an
assertion that is not the check itself.

| face | the wrong key | what it reported | detection it admits |
| --- | --- | --- | --- |
| **SHA-vs-content** | commit ancestry | "the facts file is not on `dev`" — it had been for hours; the commits were rebased to new SHAs | ask the tree, not the history: `git ls-tree <ref> -- <path>` |
| **path-vs-artifact** | a hard-coded artifact path | the candidate's byte column, which was the *incumbent's* `lib/grammar/ast.js` sitting in the snapshot | print the resolved path beside every number; refuse if a declared entry was not the one measured |
| **absence-vs-wrong-presence** | "file missing?" | a `catch` guard that defended against a *missing* artifact and was defeated by the *wrong one* being present | guard on identity, not existence |
| **success-vs-consumption** | `ok === true` | a parse that consumed 54 of 1945 bytes and dropped a whole at-rule | assert `span.end === source.length` |
| **freshness** | file contents | "399,168 B unchanged" for a build that had **failed** — it read the previous artifact | compare mtime before/after; refuse if not rewritten |
...
check that did not run.** `namecheck` was silent on success for three rounds;
`id.illusory` was `undefined` and swallowed by `?? 0`, printing a clean PASS
while doing nothing. Every check prints a count on success, never nothing.
...
### 2.4-sign Promotion has a SIGN, and the sign is set by sharing

**The less-parser result in §2.4 is not a general rule and must not be read as
one.** The same transformation — a composite promoted from bare const to a
named map rule referenced via `g.` — has been measured with **opposite signs**:

| grammar | promotion | result |
| --- | --- | ---: |
| less, 4 query productions | H1 sites collapsed | **−621,785 B** |
| Candidate A, `Url` | H1 already 0 | **+11,939 B** |

**The sign is determined by whether the reference was shared.** Promotion
removes duplicated inline closures; where there is no duplication to remove, a
named map entry is *pure addition* — a rule-table entry bought for nothing.
Candidate A's grammar is H1 0 / H2 0, so the promotion lever is **exhausted**
there and every further promotion costs.
...
sizes per variant so the slope cancels fixed overhead. Probe at-keywords appear
in no corpus input, so every variant is tree-identical by construction; the
grammar was restored and `diff`-verified.

| variant | per production | per site |
| --- | ---: | ---: |
| **heavy** — `sequence(routed(), g.AtRulePrelude, stylesheetBodyBlock)` | **16,408 B** | 5,469 B |
| **light** — `sequence(routed(), g.AtRulePrelude, literal(';'))` | **7,972 B** | 1,993 B |

**Both are new NAMED productions in the rules map, referenced once via `g.`** —
the shape that was supposed to cost ~950 B/site. It does not, at either weight.
The two differ in exactly one reference: `stylesheetBodyBlock` is a by-const H1
site, so every production referencing it re-inlines the whole stylesheet-body
closure. That is the entire 2.06× gap.

**Cost per SITE is the wrong unit. Cost is dominated by what a new production
REFERENCES**, not by how many combinators it contains. The ~950 B/site figure in
§2.1 describes a *named rule already referenced via `g.`*; it does not describe
*adding* coverage.

**CONFIRMED — the first result this session to earn the word, and it is
confirmed on the budget question, NOT on the 2.06× ratio, which remains open.**
Two methods, two grammars, neither author reading the other's method first:

| method | grammar | says |
| --- | --- | --- |
| patch/build/measure, per production | 3.34 MB incumbent | **~4.9 light productions** affordable in 39,442 B |
| in-situ marginal ratio (175 B source/case × 23.4×) | 430 KB candidate | **~9.6 coverage cases** affordable in 39,442 B |

Same direction, same order of magnitude. **99 outstanding gaps need ≈405,405 B
against 39,442 B — a shortfall of ≈365,963 B, about 10× over.**

> **Goal 2 is not reachable by writing grammar.** The gap must come from
> codegen.

**And the denominator escape route is closed:** the marginal ratio of writing
coverage is **23.4×** against a 10.85× average, so **writing more source does
not buy budget, it spends it faster.**

The in-situ number is the better of the two — it is measured in the candidate's
own grammar and does not inherit the incumbent's large-closure caveat. The
incumbent absolutes should be re-measured per grammar before being spent; what
is expected to transfer is the reference-weight finding, not the byte figures.
...
**Settled at H1 40 / H2 10.** Two independently written scripts read 42 and 38.
The four-row gap was **two errors pointing opposite ways**, so neither count was
right and averaging would have been wrong: one over-counted 2 factory aliases
(filter 8), the other under-counted `urlOpen` (filter 9, `lastIndex`) and
`opaqueAtRuleOtherwise` (a missing `otherwise` include). H2 = 10 was agreed by
both and hand-verified site by site.
...
**Retracted, so it is not re-derived:** two successive hypotheses that B's
missing `urlOpen` row came from (a) comment-stripping plus a newline-anchored
declaration regex, and (b) a missing `noTrivia` include. Both falsified — (a) by
reading `grammar.ts:1052`, (b) by B testing its own classifier, which returned
`true` for `noTrivia(sequence(` before any fix. **The second is the instructive
one and its error is an inference error, not a measurement error:** a true
observation about *A's* script was transferred to *B's* as confirmation. Both
dropped the same row for different reasons. **Observation transfers between
implementations; interpretation does not.**
