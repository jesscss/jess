# WRONG-TEST-DATA RE-AUDIT (alpha corpus goldens)

Scope: re-adjudicate every "the alpha `.css` golden is wrong" claim against the
...
Less 4.x ⇒ the golden is wrong."** That is exactly backwards for the alpha
corpus: the top-level `.css` is the **v5** golden and *intentionally diverges
...
wrong golden. The burden of proof for "wrong test-data" is a **positively-confirmed
...
- Claim (GOAL1-SCORECARD §2/§5, `mixin-var-leak.test.ts`): golden `blue` is
  wrong; ast/ + real 4.6.7 both emit `#989`, so "golden wrong".
...
- The "golden wrong" verdict used precisely the banned "ast/+4.x agree ⇒ golden
  wrong" inference. ast/'s `#989` matches **4.x**, not v5 → real gap: ast/ still
...
- Claim (`proposed-alpha-corrections/README.md`, GOAL1-SCORECARD §3): golden
...
- **Reality (verified):** ast/ render is **byte-identical** to the proposed
  `merge.css`; the only diff vs the committed golden is the `+`/`+_` combined-line
...
- v5 LAST-occurrence anchor is **owner-SETTLED**: DESIGN-DECISIONS **M1**
  ...
  ast/ to FIRST to chase the golden and was reverted on
  `fix/merge-anchor-revert-to-last`.
- The golden is the un-graduated outlier (no `legacy/` subfolder; last touched by
...
- This is **not** a "golden wrong" claim — it is a *new* proposed alpha fixture
...
- Claim (GOAL1-SCORECARD §3a): un-operated value → source-verbatim, so golden
  `rotate(0deg)` is the v5 divergence.
- **Reality:** both the v5 golden AND `legacy/` 4.x golden emit `rotate(0deg)`;
  **no divergence** exists — ast/ is the lone outlier at `rotate(-0.0000000001deg)`.
  ...
  rule does **not** extend to this `rotate()`-arg case per the owner golden.
- ast/ over-applies verbatim (or fails to canonicalize the `rotate()` arg) → real
  gap. "verbatim ⇒ golden wrong" fails the burden of proof; the golden stands.
...
- Claim: CSS-superset verbatim pass-through, so the compact golden is the v5 target
  ast/ hasn't reached — framed as an intended divergence.
- **Reality (direction confirmed):** v5 golden **normalizes** the prelude to compact
...
- So the golden is v5-correct (positively: v5 normalizes vs 4.x); ast/ trails. The
  "verbatim pass-through ⇒ golden is the v5 target ast/ hasn't reached" framing is
...
- Claim: golden wrongly merges the two adjacent `guard-default-definition-order-2`
  blocks.
- **Reality (direction confirmed):** v5 golden MERGES adjacent same-selector rules
...
- ast/ additionally drops whole `default()`-overload blocks (`guard-default-scopes-1/3`,
  ...
  registration feature, `CUTOVER-STATUS.md:49`). Golden stands.
...
| 4 | `css-3` `rotate` verbatim | **MISATTRIBUTED** jess bug (v5 golden == 4.x == `0deg`) |
...
the LEGACY 4.x reference while the owner-maintained v5 top-level golden intentionally
...
*rationale* (real ast/ gap, not wrong golden), not their baseline status.

The one previously-accepted "wrong golden" outside this list — `extend.css` /
...
(still DIFF), so it is moot for the oracle. Note X2's owner-confirmation cites the
