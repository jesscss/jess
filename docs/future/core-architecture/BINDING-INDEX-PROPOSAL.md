...
| `Rules.find('function', ...)` / `FunctionRegistry` | remaining Less plugin compatibility APIs on `FunctionRegistry` (`get`, `getLocalFunctions`, `addMultiple`, `inherit`) and unsupported complex option shapes. Simple exact-name `findFunction(...)` is direct through `Rules.functionsByName`. | delete the remaining registry surface once plugin-style callers can use direct Rules function bindings and tests prove complex function APIs no longer enter `FunctionRegistry.find(...)` |
