...
Production follow-up: `searchedRules` no longer makes direct declaration lookup
return `UNCOVERED`. That option is old registry recursion bookkeeping; direct
lookup has its own visited set. Covered exact variable/property shapes remain
direct even when the option object carries `searchedRules`.

Production follow-up: empty `candidates` / `optionalCandidates` sets no longer
make direct declaration lookup return `UNCOVERED`. Non-empty candidate sets
remain registry-owned comparison state; this only keeps covered exact
variable/property lookups direct when the carried registry bookkeeping is empty.
...
function, declaration, and property references now use the same handle family.
It carries the target `Rules`, target `lookupVersion`, original key identity,
lookup type, call-state bits, contextual declaration/property lookup shape
(`start`, `local`, `ignoreParentScopeStart`), and the resolved lookup result
identity. It intentionally does not cache evaluated values, rendered text,
callable/function/declaration output, merge-chain output, or public
