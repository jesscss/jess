...
- Architecture surface: this pass deletes `composeStats`/`ComposeStats`, the bridge-only shadow eval/emit walker and public export, and adds `@jesscss/css-parser/ast` as a closed direct AST-v2 pilot. Its Parseman reductions create plain `Root`, `Rule`, selector, and `Comment` literals directly; it imports neither a parse host nor any legacy builder or bridge.
...
- Helper/API surface: one public subpath operation, `parse`; no migration-named alias, `parseCssFn` alias, `FunctionalParseHost`, `BuilderHost`, action map, or compatibility facade is retained. `composeStats`/`ComposeStats` and their bridge-only harness hooks are deleted.
