...
- Q-40 source-order preparation audit (2026-07-15, rejected as a new canonical cut): legacy-only profiling measured `_prepareForEval` `10,420` calls / `865.8 ms`, `_prepareRegistrationOnce` `3,060` / `85.7 ms`, and `_evaluateSourceOrder` `10,420` / `807.4 ms`; normalization found `8` candidates but performed `0` reorders, while live-binding placements repointed `6,987` times. The canonical spine bypasses this route, so a cleanup here would not move `benchmark.less` and could break legacy imports/calls/live bindings. Existing source-order normalization is owned by `7bb9b483e`; no duplicate worktree implementation was made.
