...
| P9 | `.jess` `&` needs NO new semantics: a parser-local `&…` terminal reducing to `SimpleSelector.text` IS the "dedicated semantic reduction" the exclusion test asks for — the same reduction Less landed in `d18c4bfe3`. Core recognizes a parent reference purely from resolved token text (`compoundHasAmpersand`, `resolveTokenAmp`), so `.jess` `&` is spec-faithful to CSS Nesting by construction and cannot diverge from Less. | PROPOSED (design written, not implemented) | [`JESS-PARENT-SELECTOR-DESIGN.md`](./JESS-PARENT-SELECTOR-DESIGN.md) §1–§3 |
| P10 | The parenthesized parent templates — `&()` (render parent, hoist this selector to root), `&(nil)` (suppress the parent; Less spelling `&('')`), `&(X)` ≡ `&X` — are an OUTPUT-PLACEMENT feature, not selector text. They are unimplemented in EVERY dialect (`less-parser` rejects them too) and need an owner ruling on their carrier (typed `Rule` field vs a `ParentSelector` value node) before any grammar work. Do not bundle them with plain `&`. | OPEN (needs owner ruling) | [`JESS-PARENT-SELECTOR-DESIGN.md`](./JESS-PARENT-SELECTOR-DESIGN.md) §4; `docs/jess/03-Features/02-nesting.md` |
