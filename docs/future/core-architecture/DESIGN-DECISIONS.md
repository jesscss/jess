...
| C7 | Typed list recovery and zero-based indexed access are core Jess capabilities, not Less-owned helpers. Core owns only value-domain structure (`List` recovery and zero-based access) plus the universal `defineFunction`/`Fn` contract; it does not normalize indices or impose one-based language semantics. Less and Sass register the same callable shape and implement their own one-based conversion, rounding, non-finite, bounds, unit, bracketedness, separator, and map policies. They do not get separate helper/function APIs. Every dialect function must use the core capability and be ported/tested against AST v2; Sass list APIs are not exempt from the cutover. | SETTLED | Owner correction, 2026-07-21; `HANDOFF.md` |
| C8 | Delimiter-bearing values use one AST-v2 `Block { inner, delimiter: 'paren' | 'square', escaped? }` wrapper. Public syntax and materialized value `List` both carry only their typed `value` payload and separator fact (`',' | ' ' | '/' | 'undecided'`); neither grows a dialect-specific `bracketed` field. The root package does not re-export the AST-v2 `Block` name because legacy tree `Block` remains a separate, path-scoped symbol. | SETTLED | Owner correction, 2026-07-21; `HANDOFF.md` |
| C9 | Semicolons are statement/declaration delimiters, not AST-v2 value-list separators. A semicolon placed between values outside the rules level reduces to the canonical comma `List` separator; `List` carries comma/space/slash (and only an explicitly justified undecided policy), never `';'`. | SETTLED | Owner correction, 2026-07-21; `HANDOFF.md` |
