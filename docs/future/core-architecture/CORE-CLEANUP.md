...
| Q-28D | **QUEUED — static CSS value components** — audit the remaining CSS producer sites that eagerly wrap inert identifier fragments in `Keyword`/`Any`, but only where the field already accepts strings and the token has no role-bearing, trivia, query-operator, or source-map obligation. Preserve calculated values, function/operator nodes, and comment-bearing wrappers. Claim only after rechecking CSS/parser worktrees for overlap with Q-28A and the parser/trivia owners. | `packages/css-parser/src/builders.ts`, narrowly scoped CSS AST tests, any proven string-ready core consumer | ownership audit, component/value shape matrix, trivia and role guards, CSS parser suite, byte-render parity, clean-build benchmark A/B, aggressive review |
