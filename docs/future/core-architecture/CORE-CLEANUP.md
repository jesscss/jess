...
| Q-15 | **EXISTING-LANE — WT-5 owner; standalone serializer guard triaged as superseded** — the exact writer-state guard from `b3f8dc8f5` was compared against current `dev`; the paired `import-reference-issues: repeated reference/multiple imports` test passes without it, and the current reference gate still keeps the broader shared-body route dormant. No redundant serializer code was landed. The remaining reference-routing, extender-inertness, import-wiring, and cloning tails still overlap `jess-ref-sharedbody`, `jess-refext-mech2`, `jess-refflake`, and `jess-refimport-wire`; do not reassign or broaden this row. The matched `benchmark.less` control (`263.94 ms`) and temporary guarded run (`253.51 ms`) were noisy, so no speed claim. | `reference.ts`, cloning helpers, existing reference/serialize worktrees | counterfactual focused test, current gate/ownership audit, later owner integration; benchmark control recorded but inconclusive |
...
| Q-28D | **FENCED — static CSS value components** — audit the remaining CSS producer sites that eagerly wrap inert identifier fragments in `Keyword`/`Any`, but only where the field already accepts strings and the token has no role-bearing, trivia, query-operator, or source-map obligation. Preserve calculated values, function/operator nodes, and comment-bearing wrappers. The existing `jess-q28a-css-at-rule-strings` worktree still has dirty CSS builder/AST-test files, byte-for-byte identical to current `dev`; leave it untouched and do not assign this row until that owner releases the lane. | `packages/css-parser/src/builders.ts`, narrowly scoped CSS AST tests, any proven string-ready core consumer | ownership audit including the dirty Q-28A worktree, component/value shape matrix, trivia and role guards, CSS parser suite, byte-render parity, clean-build benchmark A/B, aggressive review |
| Q-28E | **QUEUED — static SCSS value/name leaves** — after Q-28D, audit SCSS-only producers for inert identifiers in declaration/value/name positions. Keep interpolation, variable references, maps/collections, calls, operators, and role-bearing at-rule tokens as nodes; retain the Less grammar composition seam from Q-28C. Do not assign until `jess-scss-*` worktrees are checked for overlapping grammar/import/calc work. | `packages/scss-parser/src/builders.ts`, SCSS AST fixtures, only proven consumer adapters | SCSS ownership audit, AST contract and interpolation guards, parser/render parity, full SCSS suite, clean-build benchmark A/B, aggressive review |
| Q-28F | **QUEUED — static Less selector/combinator payloads** — audit remaining Less selector producers for throwaway node wrappers around inert selector atoms/combinators, using the existing string-backed selector/extend support. Preserve ampersand, interpolation, pseudo arguments, extend metadata, source trivia, and calculated selector surfaces. This is a separate selector-family lane and must not overlap the existing ampersand/extend worktrees. | `packages/less-parser/src/builders.ts`, selector AST fixtures, selector/extend consumer tests | selector ownership audit, string/node parity matrix, extend and trivia guards, spine/all-less gates, clean-build benchmark A/B, aggressive review |
...
| Q-30 | **QUEUED AFTER D-EVAL — typed value literals as `(string, tag)`** — extend the inert-string boundary from keywords/idents to static dimensions, numbers, colors, and Less booleans. Parser output should retain the verbatim literal plus a compact parse-time tag; `Declaration` carries a scalar tag or packed per-slot tag array, and only an operated/matched slot materializes the corresponding `Dimension`/`Num`/`Color`/`Bool`/`Keyword` node. This is separate from Q-28: Q-28 removes already-node-free static parser wrappers, while Q-30 removes wrappers for typed literals without losing calculation semantics. Do not start until the D-EVAL flip lands and a post-flip value-heavy profile proves the construction/materialization seam is worth opening. Dimension/Num comes first, then Color, then keyword/bool unification. | `packages/css-parser/src/cssRecursiveParser.ts`, `packages/less-parser/src/lessRecursiveParser.ts`, `packages/core/src/tree/declaration.ts`, `packages/core/src/tree/util/evaluate-node-array.ts`, `packages/core/src/tree/any.ts`, `packages/core/src/tree/dimension.ts`, `packages/core/src/tree/color.ts` | value-heavy allocation/materialization census, no-write-back/tag-shape tests, operated-path parity, all-less byte triage, same-directory `benchmark.less` A/B plus value-heavy fixture, aggressive review |
