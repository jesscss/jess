...
| Q-33 | **NEXT ELIGIBLE — lazy `AtRule` identity cache** — make `_valueOf` declaration-only so uncached `AtRule` instances do not pay an eager own slot; retain the existing memoized identity for `Node.compare()` and structured preludes. Guard every invalidation site so clearing a cache never creates the slot on an instance that has not used `valueOf()`. This is a bounded refinement of Q-06's field audit, not a reopening of the broader at-rule shape decision. | `packages/core/src/tree/at-rule.ts`, `packages/core/src/tree/__tests__/at-rule.test.ts` | own-key shape assertion before/after `valueOf()`, structured-prelude identity, mutation/eval invalidation, core/ratchet/all-less gates, same-directory clean-build `benchmark.less` A/B, aggressive review |
