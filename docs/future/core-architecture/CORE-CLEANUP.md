...
| Q-28B | **QUEUED — scalar Less value leaves** — in the active Less builder, preserve a single static identifier through `_assembleLessValue` as a string when it is not a color, dimension, reference, call, operation, interpolation, or list/group. Keep mixin-parameter keyword nodes and all calculated/reference results as nodes. Do not touch the custom-declaration fallback until `work/permissive-props-interp` is triaged. | `packages/less-parser/src/builders.ts`, Less parser AST/value fixtures | positive string shape plus negative node-shape guards, Less render/eval parity, parser suite, clean-build `benchmark.less` A/B, focused gates, aggressive review |
