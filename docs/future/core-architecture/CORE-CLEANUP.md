...
| Q-14 | **EXISTING-LANE — parser/trivia owner** — provenance/span and clean-build reconciliation is already being carried by `dev-verify-85` (`61fd158ef`), `lucid-nightingale-f7e027` (`c36b526d`), and the parser test-audit worktree. Do not assign parser code or duplicate the docs pass here. | `CORE-CLEANUP.md`, `SLIM_NODES_AUDIT.md`, parser docs/code only if needed | clean parser build, source-span fixtures, docs/code agreement |
| Q-15 | **EXISTING-LANE — WT-5 owner** — the reference/copy tail overlaps the active ref/shared-body/import worktrees (`jess-ref-sharedbody`, `jess-refext-mech2`, `jess-refflake`, `jess-refimport-wire`). Audit/finish one exact tail there; no broad rewrite or fresh duplicate. | `reference.ts`, cloning helpers | reference suite, heap/allocation evidence |
...
| Q-22 | **EXISTING-LANE — parser/trivia owner** — parse-once/render-many span/trivia work is already present in the parser/trivia worktrees named for Q-14; do not create a second parser lane. | css/less parser packages and parser fixtures | clean rebuild, parser A/B, byte-identical AST/render output |
...
making the closures above. The result is deliberately asymmetric: Q-07 and Q-23
closed on current-code evidence; Q-16 transferred to owner judgment; Q-10/11/12/14/15/22
remain open but are fenced to existing lanes. No new worker was assigned to any of
those rows, and no partial worktree was reset, cleaned, or replayed.
