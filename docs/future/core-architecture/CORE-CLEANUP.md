...
| Q-28D | **ACTIVE — ownership released; static CSS value components** — audit the remaining CSS producer sites that eagerly wrap inert identifier fragments in `Keyword`/`Any`, but only where the field already accepts strings and the token has no role-bearing, trivia, query-operator, or source-map obligation. Preserve calculated values, function/operator nodes, and comment-bearing wrappers. The `jess-q28a-css-at-rule-strings` worktree is clean and has no diff against current `dev`; its prior implementation is already integrated, so this row may take a fresh bounded slice without duplicating it. | `packages/css-parser/src/builders.ts`, narrowly scoped CSS AST tests, any proven string-ready core consumer | ownership release audit, component/value shape matrix, trivia and role guards, CSS parser suite, byte-render parity, clean-build benchmark A/B, aggressive review |
...
Follow-up ownership audit — 2026-07-14: `jess-q28a-css-at-rule-strings` was checked
read-only after the earlier fence. Its worktree is clean and its tree has no diff
against current `dev`; the prior Q-28A implementation is already integrated. Q-28D
is therefore released for one fresh, disjoint CSS-builder slice. The SCSS and selector
producer worktrees remain historical or owner-bound and continue to fence Q-28E/F.
