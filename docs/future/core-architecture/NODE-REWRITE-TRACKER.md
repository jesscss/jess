...
- [ ] `Interpolated`: direct source writer exists, public `replace(...)` no
  ...
  Remaining non-scalar selector/generic materialization and replacement arrays
  remain.
...
| Interpolated | `packages/core/src/tree/interpolated.ts` | `Node` | partial | Direct source writer exists, public `replace(...)` uses a plain placeholder loop instead of regex callback scaffolding, live writer replacement emission uses `writeSyntax(...)` instead of public `toTrimmedString(...)` transport, replacement plus selector/generic eval/resolve paths use thenable narrowing instead of local node/promise casts, whole-selector plus embedded selector interpolation with owned scalar token replacements now builds selector text directly instead of calling public `toTrimmedString(...)`, public `replace(...)` also reads owned scalar token text directly for those replacements, generic `Any` materialization writes evaluated replacements directly instead of calling public `Interpolated.toTrimmedString(...)` on itself, generated selector-list wrappers now write `PseudoSelector` syntax directly instead of calling public `PseudoSelector.toTrimmedString(...)`, and public `replace(...)` writes non-scalar replacements through direct `writeSyntax(...)` instead of public replacement `toTrimmedString(...)`. High-priority remaining selector/generic materialization boundaries and replacement arrays remain. |
