...
- [ ] `Interpolated`: direct source writer exists, public `replace(...)` no
  ...
  `toTrimmedString(...)` transport. Whole-selector interpolation with owned
  scalar token replacements (`Any`/`Anonymous`/`Keyword`) now builds the
  selector from direct scalar text instead of public `toTrimmedString(...)`.
  Cold replacement string assembly, embedded selector/generic materialization,
  and replacement arrays remain.
