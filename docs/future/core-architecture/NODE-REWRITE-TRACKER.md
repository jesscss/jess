...
- [ ] `Call`: direct source syntax writer exists, empty string-name source calls
  ...
  call-level mark. Remaining work is split callable output value selection,
  `evalArgNodes(...)` copy pressure, per-argument trim marks, async/helper
