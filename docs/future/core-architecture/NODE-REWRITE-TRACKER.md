...
- [ ] `Ampersand`: direct source writer now exists in live code and public
  ...
  around selector syntax. Structural selector replacement and broader raw
  fallback string assembly remain.
...
- [ ] `AtRule`: direct source writer exists, and `getHeaderString(...)` header
  ...
  trim/replace/probe paths. Dynamic leaf render reads exact
  `Any`/`Anonymous`/`Keyword`
  name/prelude text directly when no trivia is active instead of opening a
  leaf syntax mark/get/restore window.
