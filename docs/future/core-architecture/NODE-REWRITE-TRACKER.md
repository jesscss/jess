...
- [ ] `AtRule`: direct source writer exists, and `getHeaderString(...)` header
  ...
  `getHeaderString(..., true)` output, and comment-bearing name-to-prelude
  ...
  rediscover them. Nested `@layer` registration now derives layer identity and
  parent-layer matching through owned syntax text instead of public
  `toTrimmedString(...)` / `toString(...)` transport while composing
