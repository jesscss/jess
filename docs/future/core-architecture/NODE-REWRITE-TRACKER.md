...
| Rules | `packages/core/src/tree/rules.ts` | `Node` | partial | Direct braced source writer exists, public `toBraced(...)` is cold, registration/source-order eval async branches use `MaybePromise` narrowing, and root-owned `@charset` output writes the context-owned scalar charset syntax directly instead of public `toTrimmedString(...)` transport. High priority remains for body eval/render, imports, placement state, merge output, duplicate declaration materialization, and broader root serializer capture. |
