...
| Block | `packages/core/src/tree/block.ts` | `Node` | writeSyntax hook complete | Bracket emission writes directly; nil block render writes/buffers known delimiter text without writer readback; no-trivia child syntax avoids public `toString(...)`, while trivia mode keeps source serialization for authored inner comments/spacing. Render/eval use `evalImmediateSync(...)` for non-async child values and thenable narrowing for async values. Non-nil render still captures for string/buffer return. |
...
| Rest | `packages/core/src/tree/rest.ts` | `Node` | partial scalar wrapper complete | String/empty rest syntax writes the known source token directly with no writer readback; node-valued rest stays on the existing child writer boundary, and the cold `name` getter reads `valueOf()` instead of public `toString(...)` transport. Wrapper necessity remains. |
