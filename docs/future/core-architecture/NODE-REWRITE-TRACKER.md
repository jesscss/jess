...
| Rules | `packages/core/src/tree/rules.ts` | `Node` | partial | Direct braced source writer exists, public `toBraced(...)` is cold, registration/source-order eval async branches use `MaybePromise` narrowing, root-owned `@charset` output writes the context-owned scalar charset syntax directly instead of public `toTrimmedString(...)` transport, plain no-trivia root imports write direct `AtRule` syntax instead of public `toString(...)` on a detached writer, and plain no-trivia leading comments before root imports write direct `Comment` syntax instead of public `Comment.toTrimmedString(...)` on a detached writer. High priority remains for body eval/render, complex imports, trivia-backed leading comments, placement state, merge output, duplicate declaration materialization, and broader root serializer capture. |
