...
Per-function **JSDoc coverage is broadly a gap**: only **15 of 83** fn source files
carry a JSDoc block. Location 3 exists on the **Less** site for every category; the
**Jess** site is missing three categories.

| Category | L1 JSDoc (per-fn) | L2 internal | L3 Less | L3 Jess |
|---|---|---|---|---|
| Math (`abs ceil floor round mod pow sqrt sin cos tan …`) | ~ mixed | ✓ `.cursor/rules/packages/fns.mdc` | ✓ `functions/math-functions` | ✓ `04-Functions/08-math` |
| Logical (`if iif boolean not and or`) | ~ | ✓ | ✓ `functions/logical-functions` | ✓ `04-Functions/07-logical` |
| String (`e escape replace format`) | ✗ (`replace`,`format` none; `e`,`escape` have) | ✓ | ✓ `functions/string-functions` | ✗ **missing** |
| List (`length extract range each`) | ~ | ✓ | ✓ `functions/list-functions` | ✓ `04-Functions/06-list` |
| Type (`iscolor isnumber isstring ispixel isunit …`) | ✗ (`types.ts` none) | ✓ | ✓ `functions/type-functions` | ✗ **missing** |
| Misc (`unit get-unit isdefined isruleset convert data-uri image-* svg-gradient`) | ✗ mostly | ✓ | ✓ `functions/misc-functions` | ✗ **missing** |
...
| Color operations (`saturate lighten darken fade spin mix tint shade …`) | ✗ mostly | ✓ | ✓ `functions/color-operations` | ✓ `04-Functions/04-color-operations` |
| Color blending (`multiply screen overlay softlight hardlight difference …`) | ✗ (none) | ✓ | ✓ `functions/color-blending` | ✓ `04-Functions/05-color-blending` |
...
|---|---|---|---|---|
| G1 | **68 of 83 built-in fns have no JSDoc block** — no `@param`/`@returns`/example on the source. | L1 | fns feature agent | Biggest single coverage gap. Worst: type (`types.ts`), color-blending (all), several color-ops, `replace`/`format`. |
| G2 | **Jess site missing 3 function-category pages**: string, type, misc. Present on Less site, absent under `jess/04-Functions`. | L3 Jess | docs | Mirror the Less pages (or `shared/`) with Jess framing. Not done here (out of the decided-feature priority); next-highest L3 gap. |
