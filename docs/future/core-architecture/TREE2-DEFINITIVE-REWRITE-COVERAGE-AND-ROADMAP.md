...
- **Legacy `_firstTopLevelBareAtVar` cleanup (deferred to BuilderHost retirement)**: `builders.ts` carries `_firstTopLevelBareAtVar` (`03f91d487`, the 4.x deprecation-WARNING sync) — a hand-rolled `()`-only paren-depth scan mirroring the now-fixed less.js `hasTopLevelBareVariable`. It is SUPERSEDED by the v5 grammar hard-error (`63663e900`): production (`plugin-less` → Parseman `Parser`) hard-errors bare `@var` structurally via `preludeTokenTop`/`preludeParen`/`preludeSquare` (correct, no manual counter). `_firstTopLevelBareAtVar` runs only on the dying legacy `LessGrammar` BuilderHost + `deprecation-warnings.test.ts` (which asserts obsolete 4.x warn-not-error behavior). **Action: delete `_firstTopLevelBareAtVar` + its `_warnAtRulePreludeVars` caller + the stale `variable-in-at-rule-prelude` cases in `deprecation-warnings.test.ts` when the BuilderHost is retired (reorg A4)** — unless a 4.x-compat warn mode is deliberately kept. The upstream less.js `hasTopLevelBareVariable` was already removed (PR #4469, `f999e460`) by leaning on `permissiveValue`'s entity loop; the jess maintained path never had the smell.
