...
Whitespace and comments must **not** become children in the AST. They stay outside the node tree. The AST remains clean typed structures — no interleaved trivia nodes polluting child arrays.
...
### Trivia ownership: who emits what?
...
5. **Add `consumedTrivia?: Set<IToken[]>`** to `PrintOptions`
...
7. **Simplify `getRulesWithComments`** — comments from trivia are handled during serialization, not extracted from pre/post arrays
...
**Cons (DX):**
- **Index-based access breaks.** `rules[0]` might be a `Comment`, not a `Declaration`. Every indexed access needs filtering or a wrapper like `.items()` / `.nonTrivia()`. This is a pervasive tax on all code that touches child arrays.
...
Nodes created during evaluation (`:is()` wrappers, operation results, etc.) have no source location → no trivia lookup → default formatting. This is the correct behavior — these nodes should use clean default formatting, not inherit source whitespace.
...
```ts
if (options.compress) return; // Skip all trivia
```
...
**Current**: 2 property slots per node (`pre`, `post`). Arrays allocated for multi-token trivia.

**Proposed**: 0 property slots per node. The trivia maps exist once per parse (they're already allocated today in `set input`). The only new cost is `consumedTrivia` Set during serialization.
...
- Add `trivia` / `consumedTrivia` to `PrintOptions`
...
1. **Trivia ownership / double-emit**: The consumed-set prevents double-emission, but edge cases around adjacent nodes sharing an offset boundary need testing.
...
4. **Source maps**: Trivia tokens carry their own offsets — source map accuracy should be maintained, but needs verification.
...
| New infra | — | `TriviaMap` type, `emitTrivia()` (~15 lines), `consumedTrivia` Set |
