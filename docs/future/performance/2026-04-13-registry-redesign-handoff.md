...
- [ ] Slice 13c — Wire `@import` scope isolation through `ScopeFrame` / canonical-only evaluation. Imported files still share their parsed AST across call sites, but the broad session-wide `preserveOriginalNodes` import-eval toggle is now gone from `import-style.ts`: `@compose` now evaluates its canonical tree directly on first use, the `with/set` path keeps using the fresh flattened wrapper tree it already builds, and only the ordinary `@import` path still begins from an explicit import-local deep clone. Remaining work is to keep narrowing clone pressure at those exact import-local call sites before Track 5 takes over the full buffered-render model.
