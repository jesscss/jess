...
- [ ] Slice 13 — Delete fork/renderKey system from **all nodes**; the renderKey system serves two distinct purposes that must both be addressed before it can be removed: (1) **mixin param isolation** — a shared mixin body node needs different param values in different call contexts; Slices 10–11 addressed this via `liveSlotsByName` / `ScopeFrame`; (2) **eval-time node mutation** — nodes like `Interpolated`, `Operation`, `SelectorPseudo`, `SelectorAttr`, `Ampersand` store their evaluated results keyed by renderKey so a shared source node can hold different evaluated values per render context; these have nothing to do with params and are not addressed by `liveSlotsByName`; eliminating them requires either (a) stopping all storage of eval results on nodes (always re-evaluate, relying on context), or (b) the buffered render from Track 5 which gives each render call its own output surface; known call sites using renderKey for (2): `interpolated.ts`, `selector-pseudo.ts`, `ampersand.ts`, `selector-attr.ts`, `operation.ts`; also `mixin.ts`, `serialize-helper.ts`, `rules.ts`; the `renderKey` prop on `FindOptions` also goes away; the proposal's "No forks" promise is not met until ALL of these are gone
  ...
  - Completed cleanup cuts: dead `ruleset-trace` / discarded `processPrePost('pre', ...)` header-path work is gone; render-key re-eval gating is centralized in `Node.reconcileRenderKeyState(...)`; `Node.preEval()` / `evalNode()` use context-owned traversal helpers; `Reference` no longer injects redundant lookup render keys; `Rules.flatRulesWithKeys()` is gone, with serializer traversal now carrying inherited live render keys directly instead of materializing parallel node/key arrays on `Rules`; mixin / `$for` output no longer reassigns `_renderKey` after eval when the same wrapper instance already carried that live key into evaluation; `Node._visitValues(...)` no longer carries an unused renderKey fork path; `Node.forEachNode(...)` now reads the active render key from `Context` directly instead of taking a naked renderKey parameter and is no longer exposed as a general node API surface; `Node.toTrimmedString()` no longer threads a dead renderKey argument into `_visitValues(...)`; `Node.setForContext(...)` is gone, with the last callsite writing through the live session render key directly; `Context.pushRenderKey(...)` no longer accepts `undefined`, so “no render key” is represented by not pushing at all instead of by a fake optional stack payload; the internal `Context.renderKeyStack` accessor is gone, leaving only the current session render key visible; `getActiveRenderKey(...)` is gone, with the remaining serializer/ruleset paths reading the live session render key directly from `Context`; and the print-layer render-key push/pop helpers are gone, leaving direct session stack management in the remaining renderer paths.
