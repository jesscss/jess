...
- [ ] Slice 13c — Wire `@import` scope isolation through `ScopeFrame` / canonical-only evaluation. Imported files still share their parsed AST across call sites, and `import-style.ts` still relies on clone/preserve-original-nodes paths in a few places. Re-validate those paths now that renderKey/fork storage is gone, and narrow the remaining import-localization work to exact call sites before Track 5 takes over the full buffered-render model.
