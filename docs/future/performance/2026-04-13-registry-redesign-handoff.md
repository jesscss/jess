...
- [ ] Slice 13 — Delete fork/renderKey system from **all nodes**; the renderKey system serves two distinct purposes that must both be addressed before it can be removed: (1) **mixin param isolation** — a shared mixin body node needs different param values in different call contexts; Slices 10–11 addressed this via `liveSlotsByName` / `ScopeFrame`; (2) **eval-time node mutation** — nodes like `Interpolated`, `Operation`, `SelectorPseudo`, `SelectorAttr`, `Ampersand` store their evaluated results keyed by renderKey so a shared source node can hold different evaluated values per render context; these have nothing to do with params and are not addressed by `liveSlotsByName`; eliminating them requires either (a) stopping all storage of eval results on nodes (always re-evaluate, relying on context), or (b) the buffered render from Track 5 which gives each render call its own output surface; known call sites using renderKey for (2): `interpolated.ts`, `selector-pseudo.ts`, `ampersand.ts`, `selector-attr.ts`, `operation.ts`; also `mixin.ts`, `serialize-helper.ts`, `rules.ts`; the `renderKey` prop on `FindOptions` also goes away; the proposal's "No forks" promise is not met until ALL of these are gone
  ...
  - Completed live-state moves: active composed-selector cache lives on session render state instead of `Ruleset`; `Node.render()` reuses `context.printState`; serializer stack/frame/flag changes are managed on live render state with save/restore instead of cloned session objects; lookup render keys now resolve from `options.context` first and no longer cross through serializer helpers; serializer flattening now takes the inherited live render key from the active session/render call instead of treating `Rules._renderKey` as an implicit default, no longer falls back to `Rules._renderKey` at the top-level flatten entry point, and no longer propagates the fake `CANONICAL` sentinel as if it were a live render key; flattened serializer entries now carry their effective live render key directly instead of falling back to `options.context` again during leaf emission; serializer child/container emission no longer pushes the same render key back onto the session stack when it is already active; `serializeRulesContainerInternal()` now captures the active session/render key once per container render instead of repeatedly rereading it through `options.context` across child emission; and `Rules` wrapper rendering now captures that same live key once per wrapper render instead of rereading `options.context` through the hot path, while also refusing to push the fake `CANONICAL` sentinel onto the live render-key stack.
  - Completed lookup deletion: `FindOptions.renderKey` is gone. Lookup render-key state now comes only from `options.context`, which keeps render-key ownership on the live session instead of on ad hoc lookup bags.
  - Completed serializer deletion: `PrintOptions.renderKey` fallback is gone. Active render-key state during serialization now comes only from `Context`, and the render-specific tests seed `context.renderKey` directly instead of passing render keys through options bags.
  - Completed cleanup cuts: dead `ruleset-trace` / discarded `processPrePost('pre', ...)` header-path work is gone; render-key re-eval gating and canonical fork reset handling are centralized in `Node.reconcileRenderKeyState(...)`, and that path no longer does a no-op canonical restore when the node is already on the canonical key; `Node.preEval()` / `evalNode()` use context-owned traversal helpers; `Reference` no longer injects redundant lookup render keys; `Rules.flatRulesWithKeys()` is gone, with serializer traversal now carrying inherited live render keys directly instead of materializing parallel node/key arrays on `Rules`; mixin / ruleset-as-mixin / `$for` output no longer reassigns `_renderKey` after eval when the same wrapper instance already carried that live key into evaluation, `$for` no longer rewrites `_renderKey` onto a post-eval replacement wrapper, and the `ruleset-as-mixin` deep-clone path, the ordinary mixin-call wrapper path, and the per-iteration `$for` wrapper path no longer assign or push render keys at all; `Rules._emitRulesBody()` no longer pushes `this._renderKey` onto the live session stack during serialization, and `flattenVisibleRulesForRender()` no longer propagates nested `Rules._renderKey` through flattened serializer entries, so the remaining serializer `Rules` flow now follows only the inherited live render context; `Node._visitValues(...)` no longer carries an unused renderKey fork path; `Node.forEachNode(...)` now reads the active render key from `Context` directly instead of taking a naked renderKey parameter and is no longer exposed as a general node API surface, and once a node is already sitting on the active fork it now writes child replacements straight onto that live fork object instead of bouncing back through `set(..., renderKey)` again; `Node.toTrimmedString()` no longer threads a dead renderKey argument into `_visitValues(...)`; `Node.getValue()` now exits immediately on the canonical/no-render-key path instead of dragging that case through the larger fork guard; `Node.getParent(renderKey)` now treats only `undefined` as “canonical parent” instead of collapsing render key `0` into the same path, and `Node.adopt(...)` now treats render key `0` as a real live fork key too instead of dropping that parent fork on the floor; `Ruleset.getHeaderString()` and hoisted-ruleset carrier composition now also bypass `getValue(renderKey)` when the active render path is still canonical for that ruleset, including the `CANONICAL` sentinel case, instead of entering fork lookup just because a session key exists; `Node.set()` no longer re-reads `this.value` when entering the forked branch, and an existing forked-object update no longer bounces back through `getValue(renderKey)` just to fetch the same fork again; `Node.setForContext(...)` is gone, and `Ruleset.evalNode()` no longer routes its field writes through a blanket `set(..., context.renderKey)` helper, while also capturing the active render key once per eval instead of re-reading session state on every field write, and once a ruleset is already sitting on that active fork its field updates now write straight onto the live fork instead of re-entering keyed `set(...)`; `Context.pushRenderKey(...)` no longer accepts `undefined`, so “no render key” is represented by not pushing at all instead of by a fake optional stack payload; `Context.popRenderKey()` now drops the stack object entirely once the live render-key stack empties, and it skips the extra array mutation entirely when popping the last live render key; the internal `Context.renderKeyStack` accessor is gone, leaving only the current session render key visible; `getActiveRenderKey(...)` is gone, with the remaining serializer/ruleset paths reading the live session render key directly from `Context`; and the print-layer render-key push/pop helpers are gone, leaving direct session stack management in the remaining renderer paths.
  ...
  - Remaining Track 1 seam: `Node.set(key, value, renderKey)`, `Node.getValue(renderKey)`, `_renderKey`, `_childForks`, `_parentForks`, and the remaining import/canonical-fork renderKey flow are still alive. A broader attempt to remove composed-selector cache behavior from `Ruleset` / `serialize-helper` was reverted after real double-composition regressions (`.child .child`, `.hover:hover`, `.sub .sub`), so that seam still needs a real deletion strategy rather than another blind cut.
