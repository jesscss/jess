...
- [ ] Slice 13c — Wire `@import` scope isolation through `ScopeFrame` / canonical-only evaluation. Imported files still share their parsed AST across call sites, but the broad session-wide `preserveOriginalNodes` import-eval toggle is now gone from `import-style.ts`: ordinary import/module eval now starts from an explicit import-local deep clone, while the `with/set` path keeps using the fresh flattened wrapper tree it already builds. Remaining work is to keep narrowing clone pressure at exact import-local call sites before Track 5 takes over the full buffered-render model.
