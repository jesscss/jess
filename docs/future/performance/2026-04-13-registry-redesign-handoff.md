...
- [ ] Slice 13c — Wire `@import` scope isolation through `ScopeFrame` / canonical-only evaluation. Imported files still share their parsed AST across call sites, and the broad session-wide `preserveOriginalNodes` import-eval toggle is gone from `import-style.ts`: `@compose`, plain mutable `@import`, protected `mutable:false` imports, implicit `_dedupe` imports, and explicit `reference: true` imports now all evaluate canonical trees directly on first use; queued plain CSS imports now return an empty `Rules` result instead of a synthetic `Nil` placeholder shell because output is carried entirely by `topImports`; imported `Rules.sourceNode` provenance is now bound once instead of being rewritten per import site; plain mutable imports now reuse the evaluated `Rules` directly when no wrapper-local visibility/reference/readonly state would change, no-op `with/set` configs now keep that same tree instead of allocating a fresh configured wrapper, additive `with/set` configs now keep their outer wrapper rooted in the imported rules whenever they only add non-variable nodes and only fall back to the config-rooted wrapper when new configured vars still need same-scope declaration lookup, mixed replacement+additive `with/set` configs now keep their outer wrapper rooted in the imported replacement surface whenever the additive side is non-variable-only and only fall back to the config-rooted wrapper when new configured vars still need same-scope declaration lookup, variable-replacement `with/set` configs now keep replacement semantics on a shallow cloned imported `Rules` container instead of rebuilding a fully synthetic top-level `Rules.create([])` tree, inline import postlude wrapping now uses one canonical inline-source carrier plus derived wrappers instead of rebuilding fresh `Rules.create([wrapped])` shells per layer, and evaluated import postlude wrappers now derive from the imported `Rules` surface instead of wrapping that result in a fresh synthetic `Rules.create([@media/...])` shell at each step; `Ruleset.preEval()` now returns a derived wrapper instead of mutating the canonical `Ruleset` in place, while `AtRule.preEval()` only derives when pre-eval actually changes the at-rule shape so static-name at-rules stay canonical when their child rules do; `Mixin.preEval()` no longer shallow-clones its body `Rules` just to stamp visibility and now stays canonical unless its name is actually interpolated; static regular mixin bodies now only take shallow wrapper clones at call time, while dynamic bodies still keep the deeper isolation boundary; params-only mixin calls now bind live slots directly onto the cloned body `Rules` and only allocate an `outerRules` wrapper when a guard still needs a separate scope surface; those remaining guard-scoped `outerRules` wrappers are now also derived from the cloned canonical body `Rules` instead of starting from synthetic `Rules.create([])` shells, multi-candidate mixin call output now also derives its carrier `Rules` wrapper from the first resolved mixin body output instead of synthesizing a fresh `Rules.create([])` shell, and mixin calls that resolve against real candidate bodies but emit no output now return a derived empty wrapper from that candidate surface instead of always falling back to a synthetic `Rules.create([])` shell; static guards now evaluate without taking a defensive deep-copy tier for default-routing probes, `default()`-guard candidates no longer take a dead outer guard copy before their probe passes, and static guard probes no longer allocate temporary `outerRules` wrappers just for evaluation; named `VarDeclaration` arguments now stay canonical carriers until binding; the call-frame `@arguments` binding now reuses the already-evaluated bound argument nodes instead of deep-copying them again into a second clone tier; simple evaluated parameter bindings, expanded `Rest` arguments, and rest-parameter items now also reuse those already-bound nodes directly, while list/sequence-shaped bound values only take the local normalization copy path when they actually need leading-item whitespace cleanup; zero-iteration `$for` output now derives from the canonical loop body wrapper instead of returning a synthetic empty `Rules`, single-iteration output now reuses the produced iteration `Rules` directly instead of wrapping it in an extra outer `Rules` container, multi-iteration output now also derives its aggregate `Rules` wrapper from the canonical loop body instead of synthesizing a fresh `Rules.create([])` shell, per-iteration `$for` evaluation now derives its wrapper from the canonical loop body `Rules` instead of synthesizing an empty `Rules` shell and re-pushing every child on each iteration, and the emitted loop-output `Rules` no longer retain iteration-local `scopeFrame` bindings after evaluation; the `with/set` path no longer does a shallow imported-`Rules` clone or mutates a declaration registry just to plan top-level replacements; and the only remaining import-local isolation surface is the configured wrapper path when `with/set` still needs same-scope replacement semantics plus new configured vars together. Remaining work is to converge that last import-local wrapper path, and the sibling mixin / `$for` output paths, onto the same canonical-tree-plus-binding-frame model before Track 5 takes over the full buffered-render model.
