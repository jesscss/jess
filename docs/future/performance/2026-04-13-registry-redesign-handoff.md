...
- [ ] Slice 13c — Wire `@import` scope isolation through `ScopeFrame` / canonical-only evaluation. Imported files still share their parsed AST across call sites, and the broad session-wide `preserveOriginalNodes` import-eval toggle is gone from `import-style.ts`: `@compose` now evaluates its canonical tree directly on first use, plain mutable `@import` now also evaluates canonically on first use, protected `mutable:false` imports now also evaluate canonically, imported `Rules.sourceNode` provenance is now bound once instead of being rewritten per import site, the `with/set` path keeps using the fresh flattened wrapper tree it already builds, and the remaining explicit import-local deep clone boundary is the reference-like import surface (`reference` / `_dedupe`) that still preserves referenced-selector identity. Remaining work is to decide whether those exact import-local boundaries can also converge on a binding-frame model before Track 5 takes over the full buffered-render model.
