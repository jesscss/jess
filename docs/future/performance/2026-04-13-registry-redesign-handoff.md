...
- [ ] Slice 13c — Wire `@import` scope isolation through `ScopeFrame` / canonical-only evaluation. Imported files still share their parsed AST across call sites, and the broad session-wide `preserveOriginalNodes` import-eval toggle is gone from `import-style.ts`: `@compose`, plain mutable `@import`, protected `mutable:false` imports, implicit `_dedupe` imports, and explicit `reference: true` imports now all evaluate canonical trees directly on first use; imported `Rules.sourceNode` provenance is now bound once instead of being rewritten per import site; plain mutable imports now reuse the evaluated `Rules` directly when no wrapper-local visibility/reference/readonly state would change, and no-op `with/set` configs now keep that same tree instead of allocating a fresh configured wrapper; `Ruleset.preEval()` now returns a derived wrapper instead of mutating the canonical `Ruleset` in place, while `AtRule.preEval()` only derives when pre-eval actually changes the at-rule shape so static leaf at-rules stay canonical; `Mixin.preEval()` no longer shallow-clones its body `Rules` just to stamp visibility and now stays canonical unless its name is actually interpolated; static regular mixin bodies now only take shallow wrapper clones at call time, while dynamic bodies still keep the deeper isolation boundary; static guards now evaluate without taking a defensive deep-copy tier for default-routing probes, `default()`-guard candidates no longer take a dead outer guard copy before their probe passes, and static guard probes no longer allocate temporary `outerRules` wrappers just for evaluation; named `VarDeclaration` arguments now stay canonical carriers until binding; the call-frame `@arguments` binding now reuses the already-evaluated bound argument nodes instead of deep-copying them again into a second clone tier; simple evaluated parameter bindings, expanded `Rest` arguments, and rest-parameter items now also reuse those already-bound nodes directly, while list/sequence-shaped bound values only take the local normalization copy path when they actually need leading-item whitespace cleanup; the `with/set` path no longer does a shallow imported-`Rules` clone or mutates a declaration registry just to plan top-level replacements; and the only remaining import-local isolation surface is the fresh configured wrapper tree that `with/set` still builds when the config actually changes the imported top-level variables. Remaining work is to converge that last import-local wrapper path, and the sibling mixin / `$for` output paths, onto the same canonical-tree-plus-binding-frame model before Track 5 takes over the full buffered-render model.
  ...
  the at-rule shape so static leaf at-rules stay canonical; mixin arguments now
