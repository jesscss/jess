...
1. `PrintOptions.renderKey` fallback paths that still exist only to bridge old
   serializer behavior.
2. `Rules.flatRulesWithKeys()` and the wrapper-node renderKey transport used by
   mixin calls / `$for` output.
3. `Node.set(key, value, renderKey)`, `Node.getValue(renderKey)`, `_renderKey`,
