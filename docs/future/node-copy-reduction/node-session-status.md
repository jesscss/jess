...
- detached shallow-wrapper / materialized-wrapper primitive
  - goal: provide a lower-level wrapper/materialization path that can carry wrapper metadata without canonically reparenting shared top-level children the way `clone(false)` does today
  - primary proof:
    - focused `rules.test.ts` / `import-style.test.ts` proof that current shallow clone reparents shared top-level children immediately and still shares nested ruleset bodies
    - one production consumer proving the new primitive preserves parent integrity without breaking existing wrapper semantics
  - note:
    - the returned mixin-wrapper `Ruleset` selector/body parent issue is now fixed locally in `ruleset.ts`
    - `ImportStyle` and the remaining control-path parent-integrity issue now both bottom out in the shallow-wrapper primitive itself
    - the next owner is lower-level clone/materialization infrastructure, not another `Rules`, `Ruleset`, or `control.ts` local tweak
