...
- `Ruleset.copy()`
  - goal: eliminate the remaining canonical selector `sourceNode` dependency in the context-free copy/materialization path
  - primary proof:
    - node/public-path behavior in the node's own test file
    - broader confirmation in dependent selector/ruleset tests as needed
  - note:
    - the latest landed slice moved selector `sourceNode` handling in `preEval()` / `evalNode()` onto session runtime; the remaining gap is now the copy/materialization path, not active eval
...
| `Func`                       | `partial`   | Render reads for `name` / `params` / `body` are session-aware, `evalCall()` reads `params`, `body`, and parent context through the session-aware view, the temporary mixin-wrapper setup now detaches `params` / `body` inputs so canonical children are not re-parented just to build the wrapper, and the wrapper no longer calls `parent.adopt(...)` just to participate in lookups. The remaining narrow gap is that the temporary wrapper still needs a real `parent` field because broader mixin machinery in `rules.ts` still dereferences `candidate.parent!` directly. |
