...
- `ImportStyle` returned-tree / finalization clone-pressure follow-up
  - goal: reduce the remaining import finalization/materialization clone pressure without regressing the now-green focused visibility/`with`/`set` cases
  - primary proof:
    - focused `import-style.test.ts` proof around returned imported-tree materialization
    - the smallest node/session proof that returned import trees remain usable without mutating canonical cached source trees
  - note:
