...
- `Rules` output-shaping / wrapper-materialization follow-up in `rules.ts` on top of the landed shallow-wrapper helpers in `node-base.ts`
  - goal: close the last `Rules`-local partial by keeping returned wrapper/output shaping session-backed all the way through materialized output, now that the mixin guard wrapper / lookup path is fixed again
  - primary proof:
    - focused `mixin.test.ts` / `rules.test.ts` proof for the exact returned-output shaping branch
    - one production slice proving the remaining wrapper/output parentage stays session-backed without regressing guarded mixin invocation
  - note:
    - the guard wrapper / lookup regression is green again
    - the control prior-scope consumer is now fixed on top of `cloneLookupSafeShallowWrapper(ctx)`
...
- The live remaining owner is no longer guard lookup; the guarded mixin path is green again. The current remaining `Rules` work is lower wrapper/output shaping on top of the shallow-wrapper/materialization helpers.
