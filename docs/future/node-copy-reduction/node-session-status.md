...
- lower `Rules` mixin/output-shaping materialization
  ...
  - note:
    ...
    - the next live owner is back in `Rules` / `getFunctionFromMixins()`, not another local `ImportStyle` tweak
...
- `selectorMatch()` cannot safely consume `Selector.getKeySet(context?)` yet because it has no `Context` parameter and its pair cache is not context-aware.
