...
- mixin guard wrapper / lookup path in `rules.ts` / `reference.ts`
  - goal: restore `should call a mixin with a guard condition` without reintroducing canonical mutation, now that selector-pattern param prep is fixed
  - primary proof:
    - focused `mixin.test.ts` proof for guarded mixin invocation
    - one production slice proving guard lookup sees the right wrapper/source scope without canonical parent writes
  - note:
    - selector-pattern param preparation is no longer the blocker
    - the control prior-scope consumer is still next after this, but it remains blocked below `control.ts`
...
- There is a live remaining `Rules`/mixin blocker on the current head: `mixin.test.ts > should call a mixin with a guard condition` fails with `'color' is not defined`, so the guard wrapper / lookup path is the current highest-priority owner.
- Remaining control-family parent-integrity work is still back at the consumer boundary: `control.ts` can try the new lookup-safe shallow-wrapper primitive on prior-scope reuse, but the direct swap is not sound yet.
