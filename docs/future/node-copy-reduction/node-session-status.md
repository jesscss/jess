...
- `control.ts` prior-scope reuse on top of `cloneLookupSafeShallowWrapper(ctx)`
  - goal: replace the remaining lookup-sensitive `clone(false)` wrapper use in `$for` prior-scope accumulation with the new lookup-safe wrapper primitive
  - primary proof:
    - focused `control.test.ts` proof that prior-iteration output still resolves nested lookup correctly
    - one production consumer proving canonical parentage stays unchanged while active-session ancestry sees the wrapper
  - note:
    - `Node.cloneLookupSafeShallowWrapper(ctx)` now exists and is proven
    - `ImportStyle` is no longer the active owner
    - `Rules.evaluateCandidateOutput(...)` top-level output now looks less suspicious than before; focused characterization already shows returned wrapper children keep returned parents and source-root provenance
...
- `ImportStyle` now has both `Node.materializeEvaluatedCopy()` and `Node.cloneDetachedShallowWrapper(ctx?)`, and the `_dedupe` path now keeps cached evaluated top-level child slots and parent pointers stable. If more import work remains, it is no longer the old wrapper-finalization bug.
...
- Remaining control-family parent-integrity work is now back at the consumer boundary: `control.ts` can try the new lookup-safe shallow-wrapper primitive on prior-scope reuse.
- The next compare-adoption blocker after `Condition` is not matcher core anymore; selector-pattern mixin params in `rules.ts` still need fully prepared selector operands before `compare(context)` can help.
