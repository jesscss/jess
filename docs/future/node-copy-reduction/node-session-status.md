...
| `ImportJs`                   | `partial`   | Render and eval now read `path` / `imports` through the session-aware view, and the active eval-time `path` replacement is session-backed. It stays partial because the non-reset session path still deep-clones the `Quoted` child subtree before evaluating the path. |
