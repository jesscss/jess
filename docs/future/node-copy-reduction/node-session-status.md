...
- `Call.evalNode()` downstream result branch
  - goal: close the smallest remaining downstream materialization branch in `Call.evalNode()` now that the plain non-function and fallback branches are session-safe
  - primary proof:
    - node/public-path behavior in the narrowest existing `call.test.ts` case for that branch
    - broader confirmation only if the chosen branch needs it
  - note:
    - the semicolon and parent/provenance blockers below are now reduced; the outstanding `Call` branch is the smallest active gap currently still in flight
...
| `Url`          | `partial` | Render and eval now read `value` through the session-aware view, active eval-time child replacement is session-backed, `pathValue(context?)` now provides a context-aware path extractor, and `ImportStyle` uses a context-aware extractor for both `Url` and session-patched `Quoted` import paths at the active callsite. `valueOf()` remains intentionally canonical for contextless callers. |
