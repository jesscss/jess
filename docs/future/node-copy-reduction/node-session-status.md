...
- `Ruleset` selector/body materialization for returned mixin wrapper output
  - goal: make returned mixin-wrapper rulesets keep their own `selector` and `rules` containers parented to the returned ruleset, not to a non-returned clone/view owner
  - primary proof:
    - focused `mixin.test.ts` proof for returned nested ruleset selector/body parentage on the active mixin path
    - explicit proof that returned ruleset `selector.parent` and `rules.parent` both point at the returned ruleset while canonical mixin-owned nodes stay unchanged
  - note:
    ...
    - lower `Rules` provenance writes (`outerRules.parent`, bound-value `sourceParent`) are already sessionized
    - the remaining bad parent chain on the mixin path now sits on the returned `Ruleset`’s nested `selector` / `rules` containers
    - this is no longer another local `ImportStyle` or `Rules` provenance slice
...
- Remaining control-family parent-integrity work is no longer `control.ts`-local; the current proof points at shallow clone semantics in `node-base.ts`.
