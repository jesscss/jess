...
- `rules.ts` guard-ancestor parent walk
  - goal: replace the remaining direct canonical parent walk in `hasFailedGuardAncestor()` with a session-aware parent lookup on the active mixin-eval path
  - primary proof:
    - node/public-path behavior in the narrowest existing func/mixin test that exercises guard ancestry
    - broader confirmation in mixin/rules tests as needed
  - note:
    - `Func.evalCall()` no longer needs a real canonical parent or `parent.adopt(...)` for its temporary wrapper; the next exact blocker is the remaining canonical parent walk in `rules.ts`, not `Func` itself
...
| Node                         | Status      | Notes                                                                                                                                                                                  |
| ---------------------------- | ----------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Declaration`                | `partial`   | Active eval/serialization reads and field writes for `name` / `value` / `important` now use session helpers, and node-local assignment-option normalization in `preEval()` is now session-backed with node-local behavior plus eval-session proof. The remaining node-local observer gap is now explicitly characterized: `requiredSemi` is contextless, so it still reflects canonical `value` and needs a broader caller/API change rather than another local field migration. |
...
| `Ruleset`                    | `partial`   | `selector` / `rules` / `guard` / extend-managed selector fields are session-aware on active eval/render paths, `hoistToRoot` reads plus active `hoistToRoot` / `F_VISIBLE` writes are session-backed, `valueOf(context)` now reads the session-aware selector/effective-selector path without mutating the canonical cached value, `getHeaderString()` respects session-patched selector state in its hoist fallback branch, and active `preEval()` / `evalNode()` selector `sourceNode` reads-writes now use session runtime. The remaining explicit gap is the context-free `copy()` / materialization path, which still depends on the broader canonical nested-node provenance model rather than a node-local fix in `Ruleset` alone.                |
...
| `Mixin`                      | `partial`   | Render reads for `name` / `params` / `guard` / `rules` are session-aware, interpolated-name `preEval()` writes through the session field layer, and `rules.options.rulesVisibility` writes are now isolated from the canonical child `Rules` container under an active session. Remaining work is in caller-side binding/eval paths and broader mixin output shaping, not this node-local write. |
| `Call`                       | `partial`   | Render reads for `name` / `args` / `contentNode` are session-aware, the non-function eval materialization path writes `name` / `args` through the session field layer, `clone(...)` is now session-aware for `name` / `args` / `contentNode`, and both the fallback and plain non-function branches now materialize without mutating canonical `silentFail`, `contentNode`, or nested arg spacing. The remaining gaps are now downstream of `Call` itself: callee/result-node branches (`Func`, `Collection`, nested `Call`) and the larger direct-mixin-invocation cleanup tracked separately. |
| `Func`                       | `partial`   | Render reads for `name` / `params` / `body` are session-aware, `evalCall()` reads `params`, `body`, and parent context through the session-aware view, the temporary mixin-wrapper setup now detaches `params` / `body` inputs so canonical children are not re-parented just to build the wrapper, and the wrapper now uses `sessionSetParent(...)` rather than a real canonical parent or `parent.adopt(...)`. The remaining exact blocker is broader mixin machinery in `rules.ts`: guard-ancestor walking still starts from canonical `node.parent`, and two remaining `candidate.parent!` dereferences are Ruleset-only branches. |
...
| `Sequence`                   | `partial`   | Render, eval, clone, and `operate()` now use the session-aware `value[]` path on the active context-bearing surface, and `compare(other, context?)` now also honors session-patched arrays when a `Context` is supplied. The remaining observer gap is now explicitly characterized: context-free surfaces like `length`, raw iteration, and inherited `valueOf()` intentionally stay canonical because a plain getter cannot safely represent competing session overlays. |
| `QueryCondition`             | `inherited` | Inherits `Sequence` session-aware value reads via `_getValue(context)`.                                                                                                                |
| `List`                       | `partial`   | Render/read path for `value[]` is session-aware, and `operate()` now consumes session-patched left/right items on the active path without mutating the canonical list. The remaining observer surfaces are now explicitly characterized: cached `valueOf()`, `compare()`, `length`, and iteration still read canonical `value[]` because they have no `Context` channel and would otherwise be ambiguous across concurrent session overlays. |
...
| `ComplexSelector`      | `partial` | Render and eval now read `value[]` through the session-aware view, eval-time component-array updates are session-backed, and single-item collapse now preserves a session-only `hoistToRoot` patch without mutating canonical state. `valueOf()` is now explicitly non-mutating and intentionally canonical: it no longer normalizes `value` back onto the node, but it still has no `Context` channel and therefore remains a canonical selector-identity boundary. |
...
| `Quoted`       | `partial` | Render and eval now read `value` through the session-aware view, and active eval-time value replacement no longer overwrites the canonical node under an active session. `valueOf()` is now explicitly characterized as intentionally canonical across competing session overlays; the remaining observer gap is `compare()`, which also lacks a `Context` channel. |
| `Url`          | `partial` | Render and eval now read `value` through the session-aware view, and active eval-time child replacement is session-backed. `valueOf()` is now explicitly characterized as intentionally canonical because import-path consumers call it without a `Context`; closing that gap needs broader import-path API design rather than a Url-local patch. |
