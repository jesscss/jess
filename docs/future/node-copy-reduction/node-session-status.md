...
- `Call.evalNode()` collection result branch
  - goal: close the remaining downstream `Collection` materialization branch in `Call.evalNode()` now that the nested-call branch is session-safe
  - primary proof:
  ...
  - note:
    - the nested `Call` downstream branch is now closed; `Collection` is the smallest remaining active `Call`-local branch
...
| `Call`                       | `partial`   | Render reads for `name` / `args` / `contentNode` are session-aware, the non-function eval materialization path writes `name` / `args` through the session field layer, `clone(...)` is now session-aware for `name` / `args` / `contentNode`, both the fallback and plain non-function branches now materialize without mutating canonical `silentFail`, `contentNode`, or nested arg spacing, and downstream `Func` / nested-`Call` results are now materialized before outer call provenance is applied. The remaining `Call`-local downstream branch is `Collection`; broader direct-mixin-invocation cleanup is tracked separately. |
