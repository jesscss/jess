...
- shared node materialization contract for returned trees
  - goal: provide a node-level evaluated-view materialization path that preserves current evaluated subtree state, stable returned-tree parent chains, and canonical cached/source tree immutability
  - primary proof:
    - focused `import-style.test.ts` proof that `_dedupe` cannot use `sourceNode` copies or shallow top-level clones
    - the smallest node/session proof that returned import trees materialize from the evaluated view without re-parenting canonical descendants
  - note:
    ...
    - declaration, mixin, and function lookup now climb the session parent chain on the active paths
    - the remaining import work is clone-pressure / returned-tree cleanup, not the old lookup failures
    - the next owner is now the shared node materialization layer centered on `node-base.ts`, not another safe local `import-style.ts` tweak
...
| `Extend`                     | `partial`   | `clone(false, ..., context)` now keeps `selector` / `target` parent reassignment in the session layer instead of re-parenting canonical children, clone now also sources `selector` / `target` / `namespace` / `flag` through session-aware getters so patched extend fields survive the clone boundary without mutating the canonical node, `evalNode()` now honors session-patched `target`, `selector`, and `namespace` when recording extend instructions, extend registration now treats a session-patched `selector` as explicit instead of falling back to implicit `&` composition, downstream extend application now keeps `hoistToRoot` session-local on the active implicit-ampersand hoist path, and `Ruleset` now repairs session parent provenance for a session-patched `rules` container before extend processing consumes it. The next owner is now downstream instruction consumption in `extend-roots.ts`; the broader extend rewrite pipeline is still a major remaining clone/copy cluster. |
...
| `Ampersand`            | `pending` | The simple-parent collapse/hoist aliasing case is now covered with a node-local proof: a simple canonical parent selector no longer needs to be mutated just to normalize the collapsed result. `valueOf(context?)` and `getResolvedSelector(context?)` now also read a session-patched parent selector without mutating the canonical parent, and focused characterization now proves both that `keySet` intentionally stays canonical under one session patch and that it cannot derive a sound session-specific value when two sessions patch the same parent selector differently. The next owner is broader selector/key-set semantics: `computeKeySets()` and the inherited contextless `Selector.keySet` API have no `Context`, so `Ampersand` alone cannot safely choose between competing session overlays for parent-derived key material. |
