...
- internal materialization debt on active eval/runtime paths
  - immediate owner: derived-`Ruleset` `clone(false, ..., context)` rules-body handling
  - second owner: `Call.materializeDownstreamResult(...)` for composite downstream results
  - primary proof:
    - `ruleset.test.ts`: `shallow clone of a derived ruleset gives the clone its own selector while keeping the rules body lookup-safe in a session`
  ...
  - note:
    ...
    - `Extend` and `control.ts` are no longer the active owners
    ...
    - source-ruleset shallow clones are now locally consistent and keep a live shared-session rules body; the remaining `Ruleset` debt is the derived-ruleset `rules` body still using an internal materialization step
    - `Call` has already reduced leaf downstream results to shallow clone plus provenance carry-over; the remaining debt is composite downstream results only
