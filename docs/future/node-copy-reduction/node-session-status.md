...
- shallow `Rules.clone(false, ..., context)` semantics in `rules.ts` / `node-base.ts`
  ...
  - note:
    ...
    - the remaining `ImportStyle` pressure is now only the plain cached compose wrapper branch that must preserve shared top-level evaluated child identity across per-import wrappers
