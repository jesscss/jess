...
- `Rules` root serialization semicolon path
  - goal: thread the new `Declaration.requiresSemi(context)` decision into the remaining `Rules` root serialization path that still reads `n.requiredSemi` canonically
  - primary proof:
    - node/public-path behavior in the narrowest existing declaration/rules serialization test
    - broader confirmation in rules serialization tests as needed
  - note:
    - `serialize-helper` is already context-aware now; the next exact semicolon gap is the direct `Rules` root serialization path, not `Declaration` itself
...
| Node                         | Status      | Notes                                                                                                                                                                                  |
| ---------------------------- | ----------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Declaration`                | `partial`   | Active eval/serialization reads and field writes for `name` / `value` / `important` now use session helpers, node-local assignment-option normalization in `preEval()` is session-backed, and serialization now has a context-aware semicolon decision via `requiresSemi(context?)` that `serialize-helper` uses on active ruleset/at-rule paths. The remaining exact gap is broader caller adoption: `Rules` root serialization still uses the legacy contextless `requiredSemi` property. |
...
| `SelectorCapture`      | `complete` | Render and eval now read the active `value` through the session-aware path, including `preEval()` on the patched-selector path, node-local behavior coverage now also pins `valueOf()` as an intentionally canonical contextless observer, and eval-session immutability proof remains in place for the active surface. |
...
| `Quoted`       | `partial` | Render and eval now read `value` through the session-aware view, active eval-time value replacement no longer overwrites the canonical node under an active session, and both `valueOf()` and `compare()` are now explicitly characterized as intentionally canonical contextless observers. The remaining boundary is therefore API-level rather than another safe Quoted-local migration. |
| `Url`          | `partial` | Render and eval now read `value` through the session-aware view, active eval-time child replacement is session-backed, `pathValue(context?)` now provides a context-aware path extractor, and `ImportStyle` uses that extractor at the active import-path callsite. `valueOf()` remains intentionally canonical for contextless callers. |
