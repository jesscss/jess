...
- lower returned-output shaping in `Rules.evaluateCandidateOutput(...)`
  - goal: stop the next downstream reparenting/materialization write after wrapper construction itself is already session-safe
  - primary proof:
    - focused `mixin.test.ts` proof that wrapper construction is session-safe but nested content is still rewritten later
    - one production slice proving returned wrapper contents keep the right effective parent/provenance without canonical mutation
  - note:
    - parameter-wrapper population now routes through `outerRules.push(thisContext, ...)`
    - the next owner is no longer wrapper construction itself
    - adjacent control-family prior-scope work still points lower to lookup-safe shallow-clone semantics in `node-base.ts`
...
- Remaining control-family parent-integrity work is no longer `control.ts`-local; current proof says control already consumes materialized `Rules` correctly and the next owner is lower lookup-safe shallow-clone/output semantics.
