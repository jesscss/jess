...
| `Ruleset`                    | `partial`   | `selector` / `rules` / `guard` / extend-managed selector fields are session-aware on active eval/render paths, `hoistToRoot` reads plus active `hoistToRoot` / `F_VISIBLE` writes are session-backed, and `valueOf(context)` now reads the session-aware selector/effective-selector path without mutating the canonical cached value. Broader composition paths still need pressure reduction.                |
...
| `Call`                       | `partial`   | Render reads for `name` / `args` / `contentNode` are session-aware, the non-function eval materialization path writes `name` / `args` through the session field layer, and the `silentFail` fallback branch now materializes `name` / `args` through the same setters without mutating canonical nested arg spacing. Remaining work is a session-aware `clone()` / `contentNode` materialization path and the larger direct-mixin-invocation cleanup tracked separately. |
...
| `Sequence`                   | `partial`   | Render, eval, clone, and `operate()` now use the session-aware `value[]` path on the active context-bearing surface, and the node has both node-local behavior coverage and eval-session immutability proof for eval-time value replacement. It remains partial because context-free observers like `length` and `compare()` still read canonical `value[]`. |
| `QueryCondition`             | `inherited` | Inherits `Sequence` session-aware value reads via `_getValue(context)`.                                                                                                                |
| `List`                       | `partial`   | Render/read path for `value[]` is session-aware, and `operate()` now consumes session-patched left/right items on the active path without mutating the canonical list. It remains partial because `valueOf()`, `compare()`, `length`, and iteration still read canonical `value[]`. |
