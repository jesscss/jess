...
| `Ruleset` | `in_progress` | Direct field getters are field-aligned (`getSelector(renderKey?)`, `getRules(renderKey?)`, `getGuard(renderKey?)`, etc.), and `.maybeClone(...)` is gone in favor of explicit `.clone()`. Recent progress: import-style is green again after placement-owned top-level import wrappers and render-key-aware `enterRules()` body ownership fixes, and the `media.less` duplicate mixin-output path is gone after preserving resolved definition identity across `Reference` eval. Remaining blocker is the exact-extend `&&` materialization seam. |
...
4. Keep baseline attention on extend integration / Less fixture output, not mixin/import regressions that are already green again.
