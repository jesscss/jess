...
2. Keep the extend/import frontier grounded in the actual failing fixture output. Current live extend/mixin work is split between:
   - selector-shape-only output in `extend-nest.less`
   - a parser-backed exact/local extend seam in `extend.less`
   - same-selector guarded/default output merging in `mixins-guards-default-func.less`
