...
- Static leaf-like buffer renderers such as `Any`, `Bool`, `Rest`,
  `Combinator`, and `DefaultGuard` write their own text directly instead of
  resolving first and re-entering the bridge. Prefer that shape for similarly
  trivial leaves when tests can prove no eval or ownership boundary is being
  skipped.
