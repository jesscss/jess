...
- Plain CSS `Call` argument/content rendering uses the writer helper directly
  ...
  API name.
- Static leaf-like buffer renderers such as `Any`, `Bool`, and `Rest` write
  their own text directly instead of resolving first and re-entering the bridge.
  Prefer that shape for similarly trivial leaves when tests can prove no eval
  or ownership boundary is being skipped.
