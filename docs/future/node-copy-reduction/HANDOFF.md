...
- Dynamic optional fallback call args now reuse source-free static `List` and
  ...
  eval target when preserving source parents.
...
- Generated `:is(...)` pseudo rendering now has a
  ...
  normalized argument text for placement rendering. Direct generated selector
  list args can omit the wrapper, and evaluated dynamic args that become
  selector-list output now use the same omission proof. The owned generated
  pseudo wrapper still exists because selector parentage/bit metadata have not
  been split into placement state. It must not grow into a selector AST
  replacement; add another fact only with selector-shape evidence.
...
- Mixin-output wrapper replacement requirements were re-inventoried. The
  ...
  responsibility moves. The next slot record must carry those facts before
...
- Ruleset-as-mixin child ownership was re-audited against complex parent
  ...
  The simple declaration placement proof now locks that in: the output
  declaration is owned by the output `Rules`, source declaration parentage stays
  canonical, and reusable scalar leaves remain source-owned.
...
| # | Focus | Main result |
| --- | --- | --- |
| 1 | Record output / generated pseudo / direct containers | At-rule evaluated body output moved onto `AtRuleBodyEvalRecord`; generated dynamic `:is($selector-list)` output omits the wrapper when placement-proven; source-free direct index containers render text-only; mixin slot no longer stores unused `rulesVisibility`; source-backed fallback content remains owned with tests. |
| 2 | Eval-record facts / fallback content / slot identity | At-rule body registration pairing moved onto `AtRuleBodyEvalRecord`; source-free static fallback call content now reuses frozen inert output; direct index scalar hits render text-only; `MixinOutputSlot` is the sole mixin-output identity; generated pseudo and ruleset-as-mixin proofs document current ownership blockers. |
| 3 | Source-layer state / direct targets / fallback content | Nested at-rule layer lookup now reads canonical source state instead of derived eval-frame identity; direct `index` targets cover `List`, `Sequence`, `JsArray`, `JsObject`, and `Rules`; fallback call content now owns output content instead of stealing the source parent; dead mixin reference-mode cleanup was removed. |
| 4 | Direct-target guard / slot fact / state naming | Direct index targets that resolve to `List`/`Sequence` no longer redirect through mixin namespace lookup; mixin-output `referenceMode` clearing moved into `attachMixinOutputSlot(...)`; at-rule eval-state naming now reflects runtime side state instead of node-field writes; selector/ruleset ownership blockers remain documented. |
| 5 | At-rule output facts / sequence text / fallback args | At-rule body hoist frames no longer mutate the canonical at-rule during eval; source-free reference `Sequence` paths render text-only for runtime/declaration lookups; static fallback call arg containers avoid copies; selector, mixin-output, and ruleset-as-mixin audits documented current ownership blockers. |
| 6 | Body resolve / nil guard / assignment sequence / lookup proof | Public at-rule resolve skips render-frame prelude writes; guarded nil selectors render through owned guard/body output; merged sequence assignment output streams from state; ampersand state owns input item facts; mixin-output lookup tests prove type/target-specific traversal; source-free fallback `Sequence` is inert reference output. |
| 7 | Layer state / lookup policy / text containers / nil body | At-rule layer-name registration moved out of extend-root scratch into invocation state; lookup entry gates are lookup-owned; source-free direct-reference and declaration assignment containers avoid owned result copies; ampersand placement carries item text/count; simple nil-selector render skips wrapper prep and evaluates an owned body directly. |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | Record output / generated pseudo / direct containers | 22.06ms | 28.10ms | 33.21ms | 15.83ms | 8.76ms | read-only sample was 21.47/31.60/32.74/15.66/7.62ms; saved run is mixed and media RSD is 26.1%, so treat as descriptive only |
| 2 | Eval-record facts / fallback content / slot identity | 21.36ms | 27.92ms | 32.47ms | 15.64ms | 7.56ms | read-only sample was similar; saved media compares faster than #3, but RSD is 16.4%, so treat as descriptive only |
| 3 | Source-layer state / direct targets / fallback content | 21.70ms | 28.79ms | 32.54ms | 15.45ms | 9.18ms | media compares slower to #4, but read-only media was 8.41ms and saved RSD is 20.9%; treat as descriptive/noisy, not a regression claim |
| 4 | Direct-target guard / slot fact / state naming | 21.70ms | 29.14ms | 33.26ms | 16.12ms | 7.87ms | mostly noise versus #5; media compares faster but RSD is 24.2%, so treat as descriptive only |
| 5 | At-rule output facts / sequence text / fallback args | 22.95ms | 29.04ms | 33.12ms | 16.07ms | 8.86ms | media compares slower to #6, but the preceding read-only run was 8.13ms and RSD is high; treat this as descriptive/noisy, not a regression claim |
| 6 | Body resolve / nil guard / assignment sequence / lookup proof | 21.53ms | 28.68ms | 32.12ms | 15.43ms | 7.53ms | slower than #7, but the preceding read-only run was similar; RSD remains high and static counts are near-flat, so treat as descriptive only |
| 7 | Layer state / lookup policy / text containers / nil body | 18.27ms | 22.52ms | 27.40ms | 13.99ms | 6.09ms | recorded after read-only run; all medians compare faster than prior saved row, but RSD remains high, so treat as descriptive only |
...
1. **Delete one remaining at-rule body derived-frame dependency.**

   - Goal: inventory the remaining derived-body-frame readers/writers and move
     a whole responsibility family into `AtRuleBodyEvalRecord`, not another
     runtime map.
...
2. **Design fallback-call content placement state for source-backed content.**

   - Goal: inventory source-backed, dynamic, and async content paths and reduce
     one safe shape without stealing source parents, or record the exact
     placement-state field that is missing.
...
3. **Narrow public direct-target resolve ownership for source-free containers.**

   - Goal: decide whether any public `resolve(...)` direct-target container
     result can avoid public metadata while preserving API identity and parent
     safety, or document why text-only must remain render-only.
...
4. **Audit generated pseudo omission against extend/keyset behavior.**

   - Goal: prove the dynamic generated `:is(...)` omission rule across extend
     matching and selector-bit metadata before moving more wrapper facts into
     placement state.
...
5. **Move mixin-output rule-index assignment behind slot helpers.**

   - Goal: make source/output index mapping and rule-index assignment a single
     slot-owned helper family while keeping lookup on owned output children.
...
6. **Expand ruleset-as-mixin placement proof beyond simple declarations.**

   - Goal: cover nested rules and comment/declaration order so the future
     placement record is based on representative output, not one easy case.
