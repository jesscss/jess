...
- `pnpm run verify:baseline`, `pnpm run verify:node-copy-frontier`,
  ...
  frontier, and package-export coverage; changed-only mode intentionally runs
  the full baseline when verifier scripts or root dependency metadata changed.
  Changed-only mode includes local unstaged and staged files as well as
