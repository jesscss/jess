...
- Immediate next owner is now `Call.materializeDownstreamResult(...)` for composite downstream results.
...
- `ImportStyle` and `Ruleset` now adopt context-aware materialization only on explicit returned-tree consumer paths, and `$for` prior-scope reuse now stays on lookup-safe wrappers/shallow clones instead of materializing prior iteration output. The returned-tree materialization points are still transitional debt to reduce, not the desired steady-state eval model.
...
- `Ruleset` characterization now shows both shallow-clone branches are locally consistent live-session contracts: source rulesets clone the selector and share the `rules` body with the source ruleset, while derived rulesets clone the selector and use a lookup-safe shallow wrapper for the `rules` body. That is no longer an internal-materialization blocker on this head.
...
- `Call` is now the only live semantic owner on the internal-materialization axis: `materializeDownstreamResult(...)` still uses internal `materializeEvaluatedCopy(context)` for composite downstream results on active eval paths, while trivial leaf results have already been reduced to shallow clone plus runtime provenance carry-over.
- `Ruleset` is no longer in the internal-materialization queue on this head; its focused shallow-clone proofs now characterize shared live-session behavior rather than active materialization debt.
