...
- The next `rules.ts` characterization now proves the remaining pressure is the non-`Rules` child branch in `evaluateCandidateOutput(...)`, and it narrows further to canonical source-ruleset shallow-clone semantics rather than another `rules.ts`-local seam.
