...
- Immediate next owners are now the remaining internal-materialization debt sites, led by `Ruleset.clone(false, ..., context)` on the derived-ruleset `rules` body and `Call.materializeDownstreamResult(...)` for composite downstream results.
...
- Detached-ruleset unlock now has an explicit `Rules.cloneDetachedUnlockWrapper(context)` seam instead of spelling raw `clone(false, ..., context)` at that callsite.
...
- The next `rules.ts` characterization now proves the remaining pressure is the non-`Rules` child branch in `evaluateCandidateOutput(...)`, but that branch is now only the exposing caller: it narrows to canonical source-ruleset shallow-clone semantics plus wrapper parent assignment rather than another `rules.ts`-local seam.
- A direct local `materializeEvaluatedCopy(context)` fix at that internal eval point was explicitly rejected on this head because it violates the “no internal eval materialization” rule.
...
- `ImportStyle` characterization now pins the remaining local tension to only the plain cached compose wrapper branch: per-import wrappers need different wrapper metadata while still sharing the exact cached top-level evaluated child identity. That is now a characterization boundary, not evidence that `import-style.ts` itself is still the next production owner.
...
- `Ruleset` characterization now pins the next non-`Rules` child owner more precisely: for `clone(false, ..., context)`, derived rulesets now clone the selector and use a lookup-safe shallow wrapper for the `rules` body, while source rulesets clone the selector and still share the `rules` body with the source ruleset. The current focused proof also shows that the source-ruleset branch is locally consistent today because active ownership is shifted onto the clone through the session layer and the clone remains a live session view over shared nested children; changing it further would be a deliberate clone-contract change, not another missing `ruleset.ts` cleanup.
- `control.ts` prior-scope reuse is now fixed on top of lookup-safe wrappers/shallow clones, and the focused control suite is green. Control-family nodes are no longer in the internal-materialization queue.
- `control.ts` is no longer the live semantic owner: characterization now proves the remaining parent-integrity leak happens downstream during `wrapper.eval(context)`.
- `Call` is also no longer a live semantic owner, but it must stay `partial` because `materializeDownstreamResult(...)` still uses internal `materializeEvaluatedCopy(context)` for composite downstream results on active eval paths; only trivial leaf results have been reduced to shallow clone plus runtime provenance carry-over.
- `Ruleset` must also stay `partial` while `clone(false, ..., context)` still materializes the derived-ruleset `rules` body internally on the active shallow-clone path, even though the selector side has now been reduced to a shallow clone.
...
- Recent completed node slices: `RawRules`, `Block`, `Negative`, `Rest`, `AttributeSelector`, `InterpolatedSelector`, `AtRule`, `Operation`.
...
- Latest partial node slice: `Func.evalCall()` now reads `params`, `body`, and parent context through the session-aware view and has both a direct `evalCall()` proof in `func.test.ts` and an eval-session immutability proof. The remaining higher-order temporary mixin-wrapper path is still open.
- Latest completed node slice: `Operation` now routes preserved-expression `left` / `right` updates through the session field layer and has both a direct node-local proof in `operation.test.ts` and an eval-session immutability proof.
...
- New recorded architecture cleanup: after the fundamentals gate is much further along, the API should be semantically cleaned up so canonical identity, active session structure, and source/call-site provenance are explicit separate concepts. This is now tracked as deferred `Stage 20.6` in `dependency-graph.md`, not as part of the current lower-order node queue.
...
- `PseudoSelector` is now complete for this fundamentals pass: render and eval now read `name` / `arg` through the session-aware view, eval-time `arg` updates are session-backed, `selector-pseudo.test.ts` covers behavior parity, and `eval-session.test.ts` proves canonical `arg` stays unchanged under an active session.
- `CompoundSelector` is now complete for this fundamentals pass: render and eval now read `value[]` through the session-aware view, eval-time component-array updates are session-backed, compound serialization no longer mutates child spacing state, `selector-compound.test.ts` covers behavior parity, and `eval-session.test.ts` proves canonical components stay unchanged under an active session.
- `ComplexSelector` has a new partial fundamentals slice: render and eval now read `value[]` through the session-aware view, eval-time component-array updates are session-backed, `selector-complex.test.ts` covers nested-child behavior parity, and `eval-session.test.ts` proves canonical components stay unchanged under an active session.
- `SelectorList` is now complete for this fundamentals pass: render and eval now read `value[]` through the session-aware view, eval-time selector-array updates and top-level `:is()` flattening are session-backed, `selector-list.test.ts` covers behavior parity, and `eval-session.test.ts` proves canonical items stay unchanged under an active session.
- `Interpolated` is now complete for this fundamentals pass: render and eval now read `source` / `replacements` through the session-aware view, eval-time replacement updates plus evaluated-state marking are session-backed, `interpolated.test.ts` covers behavior parity, and `eval-session.test.ts` proves canonical replacements stay unchanged under an active session.
- `Range` is now complete for this fundamentals pass: render and eval read `start` / `end` / `step` through the session-aware view, the node has no remaining node-local eval-time field writes, `range.test.ts` covers behavior parity, and existing session-overlay coverage proves canonical bounds stay unchanged under an active session.
- `Condition` is now complete for this fundamentals pass: render, eval, and clone now read `left` / `operator` / `right` / `negate` through the session-aware view, the node has no remaining node-local eval-time field writes, `condition.test.ts` covers behavior parity, and `eval-session.test.ts` plus `condition.test.ts` prove canonical fields stay unchanged under an active session.
- `List` has a new partial fundamentals slice: `operate()` now consumes session-patched left/right items on the active path without mutating the canonical list, `list.test.ts` covers behavior parity for session-backed append/concatenate behavior, and `eval-session.test.ts` proves canonical items stay unchanged under an active session. It remains partial because `valueOf()`, `compare()`, `length`, and iteration still read canonical `value[]`.
- `Reference` has a new partial fundamentals slice: eval now reads patched `target` / `key` through the session-aware view, resolved `sourceParent` links are session-backed, `reference.test.ts` covers session-patched lookup behavior and source-parent isolation, and `eval-session.test.ts` proves canonical `key` stays unchanged under an active session. It remains partial because the higher-order mixin adapter path is still indirect and tracked separately as Stage 20.5.
- `Sequence` has a new partial fundamentals slice: clone, eval, and `operate()` now route active `value[]` reads/writes through the session-aware path, `sequence.test.ts` covers behavior parity for pre-eval clone preservation and eval-time value replacement, and `eval-session.test.ts` proves canonical items stay unchanged under an active session. It remains partial because context-free observers like `length` and `compare()` still read canonical `value[]`.
- `Declaration` has a new partial fundamentals slice: node-local assignment-option normalization in `preEval()` is now session-backed, `declaration.test.ts` covers behavior parity for the normalized assignment surface, and `eval-session.test.ts` proves canonical `options` stay unchanged under an active session. It remains partial because the remaining mutation pressure is caller-side, outside the node class itself.
- `Mixin` has a new partial fundamentals slice: `preEval()` now isolates `rules.options.rulesVisibility` writes from the canonical child `Rules` container, `mixin.test.ts` covers behavior parity for that path, and `eval-session.test.ts` proves canonical rules visibility stays unchanged under an active session. It remains partial because the remaining work is caller-side binding/eval behavior and broader mixin output shaping.
- `Call` has a new partial fundamentals slice: the `silentFail` fallback branch now materializes fallback `name` / `args` through node-local session-aware setters, `call.test.ts` proves canonical nested arg spacing and patched source args stay unchanged, and `eval-session.test.ts` proves canonical arg spacing stays unchanged under an active session. It remains partial because `Call` still lacks a session-aware `clone()` / `contentNode` materialization path, and the larger direct-mixin-invocation cleanup is still separate.
- `Func` has a new partial fundamentals slice: temporary mixin-wrapper setup in `evalCall()` now detaches `params` / `body` inputs so canonical children are not re-parented just to build the wrapper, `func.test.ts` covers the canonical-parent invariants, and existing `eval-session.test.ts` coverage continues to prove session-backed param/body reads. It remains partial because the wrapper path itself and downstream return extraction are still part of the broader direct-mixin-invocation cleanup.
- `Ruleset` has a new partial fundamentals slice: `valueOf(context)` now reads the session-aware selector/effective-selector path instead of the canonical cached selector, `ruleset.test.ts` covers session-patched selector visibility on that path, and existing `eval-session.test.ts` coverage continues to prove session-backed render/eval reads plus hoist-state immutability. It remains partial because broader composition-path writes and a few remaining canonical-only reads still need reduction.
- Follow-up partial batch now landed in the working tree:
...
- Latest exact-gap batch now landed in the working tree:
  ...
  - `Paren`: `options.escaped` is now session-aware on both render and eval paths; this closes the remaining node-local `Paren` gap.
  - `ComplexSelector`: `valueOf()` no longer mutates the node when `value` is non-array, and the canonical `valueOf()` boundary is now explicitly characterized with node-local tests.
...
- New completion-audit wave now landed in the working tree:
...
- Latest completion-audit wave now landed in the working tree:
...
- Next narrowed batch now landed cleanly:
  - `extend-roots`: `applyInstructionToRuleset()` now reads/writes `hoistToRoot` through the session layer, so hoist-producing extend application keeps `hoistToRoot` session-local on nested `Ruleset`s.
  - `control.ts`: `resolveEntries()` now reads iterable declaration `name` / `value` through the session layer for `Rules` / `Ruleset` / `Mixin` iterables, so loop key/value binding sees session-patched declarations instead of stale canonical data.
  - verification:
...
- Latest mixed follow-up now landed in the working tree:
  ...
  - `rules.test.ts` now proves the replacement is parented only in-session while canonical children remain unchanged.
  ...
  - `extend-rules.test.ts` now proves the instruction tuple records a session-patched namespace without mutating the canonical `Extend` node.
...
- Latest mixed follow-up now landed in the working tree:
  ...
  - `control.test.ts` now proves a loop body child replaced during evaluation is visible in `$for` output while the canonical loop template child remains unchanged.
...
- Latest mixed follow-up now landed in the working tree:
  ...
  - `ExtendRootRegistry` / `extend-roots.ts`: downstream namespace-aware matching is now wired through, and namespace-excluded misses are classified as `extend/not-found` instead of `extend/not-accessible`.
  ...
  - The live next owner is still the shared evaluated-view materialization contract in `node-base.ts`; the remaining extend work now looks more like selector rewrite/match semantics than namespace visibility/orchestration.
- Latest returned-tree / lower-mixin follow-up now landed in the working tree:
  ...
  - `import-style.ts`: configured-compose and `_dedupe` top-level returned-tree materialization now use `materializeEvaluatedCopy()`, so the active path materializes from current evaluated children instead of `sourceNode` copies.
  - `rules.ts`: the lower `getFunctionFromMixins()` parameter wrapper now keeps `outerRules.parent` in the session layer instead of canonically adopting it.
  - `selector-match-unit.test.ts`: focused characterization now proves `selectorMatch()` cannot safely consume `getKeySet(context)` yet because it has no context parameter and its pair cache is context-agnostic.
  ...
  - The immediate next live owner is now back in lower `Rules` mixin/output shaping, not `ImportStyle` local finalization.
- Latest lower-rules / matcher follow-up now landed in the working tree:
  ...
  - `selector-match-core.ts`: `selectorMatch()` now accepts an optional eval `Context`; when provided, matching uses session-aware `valueOf(context)`, `getResolvedSelector(context)`, and `getKeySet(context)` while leaving no-context canonical behavior unchanged.
...
- Latest wrapper/materialization follow-up now landed in the working tree:
  - `ruleset.ts`: shallow clones of derived rulesets now materialize their immediate `selector` and `rules` containers onto the clone under an active session, which fixes the returned mixin-wrapper nested selector/body parent chain.
  - `mixin.test.ts`: now proves returned nested rulesets keep both `selector.parent` and `rules.parent` on the returned ruleset.
  - `index.ts` and `selector-list.ts`: compare paths now accept the existing optional eval context and forward it through to `selectorMatch(..., context)`, so compare-side consumers can opt into session-aware selector matching without changing canonical no-context behavior.
  ...
  - Immediate next owners are now:
    - detached shallow-wrapper / materialized-wrapper primitive in `node-base.ts`
    - `ImportStyle` wrapper cleanup on top of that primitive
    - caller-side adoption of context-aware compare where session-sensitive selector semantics matter
