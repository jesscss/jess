...
- `JsImport` has a new partial fundamentals slice: render and eval now read `path` / `imports` through the session-aware view, the active eval-time `path` replacement is session-backed, `import-js.test.ts` covers behavior parity, and `eval-session.test.ts` proves canonical `path` stays unchanged under an active session. It remains partial because non-reset sessions still deep-clone the `Quoted` child subtree before path evaluation.
