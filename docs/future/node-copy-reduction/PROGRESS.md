...
- `Ruleset` characterization now pins the next non-`Rules` child owner more precisely: for `clone(false, ..., context)`, derived rulesets already materialize `selector` and `rules` onto the clone, while source rulesets still share those nested containers with the source ruleset. The current focused proof also shows that this source-ruleset branch is locally consistent today because active ownership is shifted onto the clone through the session layer and the clone remains a live session view over shared nested children; changing it would be a deliberate clone-contract change, not another missing `ruleset.ts` cleanup.
