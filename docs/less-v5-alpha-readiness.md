...
| Less math/unit evaluation | `tests-config/units/no-strict/no-strict.less`, `tests-config/units/strict/strict-units.less` | Explicit compile-level `mathMode`/`unitMode` wiring is present, but the fixtures set legacy `math: 0` and `strictUnits` under `language.less`; that plugin-normalization route is not yet proven in Context. The remaining evaluator gap is structural bare-slash precedence under eager `math: 0`/`mathMode: 'always'`; `parens-division` must preserve bare slash values. Strict-unit singularity must be checked after cancellation. | Promote the existing typed `ValueSlot[]` structure without reparsing source bytes; defer strict singularity validation to final typed materialization/emission while retaining strict incompatible `+`/`-` errors. The no-strict fixture's `test-division`, `t3`, and `t6` must evaluate under its `language.less.math: 0` config; equivalent bare slash values under `parens-division` remain authored. Add a public-route assertion for the nested `styles.config.cjs` options before calling the fixture route green. |
