...
| 9 | Inherit `RelativeComplex` | ⚠ KEPT-OVERRIDE | — | Blocked on TWO counts. (1) GENUINE reducer divergence: less's reducer wraps the result in `withSourceSpan(…, span)` (attaches a source span used by diagnostics/source-maps); css's `RelativeComplexSelector` reducer omits it — the span-attachment is NOT foldable/droppable without losing that metadata. (2) COMPOSE-MACRO FUSION break: converging (less referencing css's `g.RelativeComplexSelector`, which open-recurses into less's own `ComplexSelector` override) makes the parseman macro fail to fuse — `check:compose-fused` + `check:macro` go red (132 artifact fallbacks; `IR direct node builder for VarCall references module import(s) … a runtime compose() cannot supply`). Reverting restores 0 fallbacks. The oracle could not even be measured (harness fails to load the non-fused artifact). Flag for parseman-capability review — the span difference is a real override regardless. |
