...
| 3 | Inherit `Dimension` | ⚠ KEPT-OVERRIDE | — | GENUINE OVERRIDE. Deletion moved the AST oracle: `threw` 120→208 (88 more corpus files fail), 98/666 AST entries moved. less's shared `g.DimensionUnit` (recognition.ts:342) ends `|%`, so less's `Dimension` matches `50%` wholesale as `dimension(50,'%','50%')`; css's `Dimension` uses `numberNoPercentage` `(?!%)` + a `%`-excluding `dimensionUnit`, routing `%` through a separate `Percentage` NODE (css:1279). less overrides `Percentage` with a **token** used only at keyframes (grammar.ts:1229), absent from the value-atom choice, so inheriting css's Dimension leaves `50%` unmatched in value position. Proof obligation FALSE. |
