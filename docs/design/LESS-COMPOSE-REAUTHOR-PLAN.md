...
| 3 | Inherit `Dimension` | ☑ CONVERGED | this branch | CONVERGED by making css the outlier fix (ADDITIVE toward the three, DESIGN-DECISIONS V20). css's `Dimension` now uses the shared `numberValue` (no `(?!%)`) + a `dimensionUnit` that admits `%` — exactly the shared recognizer — and css's separate `Percentage` NODE is DELETED (css keeps `Percentage` as the same **token** the other three carry, used only by `keyframeSelector`). less/scss/jess now inherit css's `Dimension`+`Percentage` by name (both deleted here). Session-isolated oracle (same tree, before/after): less **ast 0 moved**, cst 6 moved — all pre-existing parse-error/url files whose `expected` label dropped the `Percentage` alternative (`threw` 118→118, no file flipped parse↔error). Public CST numeric type unaffected: `cst-host.ts:numericGrammarType` classifies from leaves (`%`→Percentage). less-parser 741 green. |
| 4 | Inherit `CustomPropertyValue` | ⚠ KEPT-OVERRIDE | — | GENUINE OVERRIDE. css's `CustomPropertyValue` body references `g.CustomPropertyName` (less's own interpolation-**widened** rule, §4.3-W accepting `--@{x}`), whereas less's uses the flat escape-free `g.CustomPropertyToken`. AST byte-identical, but the CST moved on 24 real (non-error) corpus files: inheriting css's rule nests a `CustomPropertyName` child node under `CustomPropertyValue` (LS-consumed tree shape) and widens the accepted language to interpolation in that position. The proof obligation (divergence = only the escape-free token override, AST identical) is insufficient — the sub-rule reference itself differs. |
| 5 | Inherit `OpaqueBody` | ⚠ KEPT-OVERRIDE (ESCALATE) | — | Owner corpus byte-identical on BOTH surfaces, but a synthetic nested opaque body proves a real CST-structure divergence. css's `many(OpaqueBodyPart)` STRUCTURES the body into `OpaqueGroup`/`OpaqueString`/`OpaqueComment` child nodes; less's `scanTo('}')` keeps ONE flat opaque leaf. Captured TEXT is byte-identical (the brace-scan proof obligation on bytes HOLDS) and no corpus opaque body contains nested braces/strings/comments, so oracle stays IDENTICAL — but `@unknown { a {nested} "str}" b }` shows the CST parts diverge (LS-consumer tree). Flat-vs-structured opaque body is an owner semantics call → kept less's flat override; flag for owner review (corpus byte-identity suggests structured-opaque MAY be adoptable). |
| 6 | Inherit `KeyframeSelector` | ☑ DONE | this branch | CONVERGED. Both produce node type `SimpleSelector`; the map-key name (`KeyframeSelector` vs css `keyframeSelector`) is an internal binding invisible to CST/AST (grammarType = node TYPE). `keyframeEndpoint` byte-identical (css:484); `g.Percentage` resolves to less's override in the composed grammar. AST **and** CST byte-identical across the corpus; emitted-CSS oracle (82+31) + less-parser 740 unchanged. less ref updated `g.KeyframeSelector`→`g.keyframeSelector`; dead less-local `keyframeEndpoint` copy removed. |
| 7 | Inherit `NamespaceTypeSelector` | ⚠ KEPT-OVERRIDE | — | GENUINE OVERRIDE (structural CST). AST byte-identical, but CST moved on 4 real corpus files (css-3 namespace selectors). less's body uses the named `g.AttributeNamespace` decomposition NODE (intentional, reused for attribute interpolation per §4.6) → an `AttributeNamespace` child node in the CST; css inlines a flat `attributeNamespace` regex (css:542) with no such node. Converging flattens the CST (confirmed: `svg\|circle` loses its `AttributeNamespace` node). Also differs: css `noTrivia` wrapper + `g.Identifier` vs less `staticIdentifier` — but the AST proved byte-identical, so the divergence is purely the deliberate CST decomposition. No LS/diagnostics consumer references the grammarType today, so the owner COULD later flatten it. |
| 8 | Inherit `AttributeSelector` (static) | ⚠ KEPT-OVERRIDE | — | GENUINE OVERRIDE. Deletion moved the **AST** oracle: `threw` 120→122 (2 more corpus files fail), 4 AST entries + 25 CST entries moved. less's reducer emits `simpleSelector('[…]')` with canonical modifier spacing built from structured `AttributeName`/`AttributeMatch` fact nodes (decomposition reused for interpolation, §4.6) inside a `parser({trivia: staticSelectorTrivia})` scope; css emits `attributeSelector(children.map(sourceText))` — a **different AST constructor**, flat tokens, source-preserving spacing. Not byte-identical in accepted language OR AST. |
| 9 | Inherit `RelativeSelector` | ⚠ KEPT-OVERRIDE | — | Re-attempted on parseman 0.50.6 (the #138 "bare imported-factory reducer drops its import across compose" fix). Still blocked on TWO counts. (1) GENUINE reducer divergence: less's reducer wraps the result in `withSourceSpan(…, span)` (attaches a source span used by diagnostics/source-maps); css's `RelativeSelector` reducer omits it. This is SYSTEMATIC across the less selector tower — less's `ComplexSelector` and `SelectorList` reducers also attach `withSourceSpan` where css's omit it — so the span is a less-wide selector-span policy, NOT foldable/droppable without losing that metadata. (2) COMPOSE-MACRO FUSION still breaks: converging (delete less's `RelativeSelector` const → `g.RelativeSelector` resolves to css's, which open-recurses into less's own `ComplexSelector` override) STILL fails to fuse under 0.50.6 — `check:compose-fused` + `check:macro` go red (132 artifact fallbacks, identical count). 0.50.6 DID clear the old `IR direct node builder for VarCall references module import(s)` diagnostic; the fusion now fails with a DIFFERENT one — `compose(): could not be lowered to a table … ref() used before .define()` (css's `RelativeSelector` slot is referenced across the compose before its define). Baseline (0.50.6, no convergence) = 0 fallbacks; reverting restores 0. So the macro-fusion is no longer blocked by the VarCall-import bug but by a slot-ordering limitation — a separate parseman-capability question — AND the span divergence is a real reducer override regardless. **OWNER CALL (open):** should css's `RelativeSelector` (and the selector tower generally) attach a source span for ALL dialects, so less can converge? Until decided, KEPT-OVERRIDE. |
| 10 | Inherit `SupportsInParens`+`SupportsCondition` | ⚠ KEPT-OVERRIDE | — | GENUINE OVERRIDE. Deletion moved the **AST** oracle on 33/666 files (emitted CSS changes; no new parse failures). less's `SupportsInParens` body references `g.SupportsFeature` (a genuine less override emitting `operation(':')` with a less-math flag, §4.4-O) + `g.Enclosed`; css's references the media-shared `g.QueryFeature`. Inheriting css's rule swaps the @supports feature leaf to QueryFeature, changing @supports parsing. The plan's hypothesized parseman per-subtree leaf-rebind (keep less's SupportsFeature while inheriting css's SupportsInParens shape) is NOT achievable by deletion — a plain inherit takes css's whole body. **CORRECTED 2026-09-26: no parseman capability is missing.** An audit of the parseman API found the pattern documented: open-recursive override plus "shared shapes: one shape, many bindings" (`parseman/docs/guide/extending.md`) and "separate rules" (`docs/guide/context.md`, "Which tool"). The blocker is the css base: css's `SupportsInParens` references the media-shared `g.QueryFeature` directly, so the `@supports` leaf has no name to override. Fix: css names the `@supports` subtree's leaf (`SupportsFeature`, bound to css's current shape, so css output is unchanged); Less overrides only that leaf; `@media` keeps `QueryFeature`. Same for the widened `Enclosed`. (Compose-macro fused/macro gates stayed green here; the blocker is purely semantic.) |
| W-widen | WIDEN-LEAF → leaf-overrides | ⚠ KEPT-OVERRIDE | — | NOT a clean leaf-override. The plan's premise ("widening point is EnclosedContent; the structural parents inherit css") is contradicted by the code: less widens the Enclosed family at MULTIPLE levels and every parent's REDUCER differs from css's. css `EnclosedQuoted` = flat `customDoubleQuotedText`+`tokenText` join; less `EnclosedQuoted` adds `VariableInterpolation`/`BareVariableInterpolation` arms + `enclosedInterpolationFromChildren` (structured `Interpolation`). css `EnclosedGroup`/`EnclosedContent`/`Enclosed` reducers emit flat strings / `interpolation([{lit}])`; less's emit structured `Interpolation` nodes and `Enclosed` builds `block`/`funcCall` from Interpolation-typed content. css uses inline `QueryFunctionOpen`; less a factored `EnclosedFunctionName`. Deleting ANY parent to inherit css swaps a structured-Interpolation reducer for a flat-string one → AST change. The other W-widen leaves (`CustomPropertyName` — confirmed by increment 4; `Quoted`/`EscapedQuoted` interp arms; `PlainUrl`/`VariableUrl`; `OpaqueAtPrelude`) share the same interp-widening + structured-reducer pattern. |
...
| `Dimension` | ☑ CONVERGED | this branch | CONVERGED via the css outlier fix (Less #3, DESIGN-DECISIONS V20): css now accepts `%` in `dimensionUnit` exactly as the shared recognizer does, so jess inherits css's `Dimension` by name (deleted here). Session-isolated oracle: jess **ast 1 moved** (`calc-empty.css`, an empty-`calc()` parse error whose `expected` label dropped the `Percentage` alternative; `threw` 63→63, no parse↔error flip), cst 1 same file. jess-parser 510 green. |
| `Percentage` | ☑ CONVERGED | this branch | CONVERGED. css now keeps `Percentage` as the same **token** jess carries (`token(noTrivia(sequence(<number>, '%')))`), so jess inherits it by name (deleted here). Same kind, same product. |
...
| `Percentage` | ☑ CONVERGED (this branch) | CONVERGED via the css outlier fix (DESIGN-DECISIONS V20). css now keeps `Percentage` as the same **token** scss carries and admits `%` in `Dimension`'s unit, so scss inherits BOTH `Percentage` and `Dimension` by name (both deleted here, plus the now-dead scss-local `numberValue` regex). Session-isolated oracle: scss **ast 3 moved / cst 3 moved** — the identical pre-existing parse-error trio the NamedColor sweep recorded (`@if /**/ true`, `@for … from/through /**/ …`), whose `expected` label dropped the `Percentage` alternative; `threw` 726→726, no parse↔error flip. scss-parser 622 green. |
...
| Value/selector/interp towers (`Value`, `Quoted`, `CustomPropertyValue`, `CustomValue`, `PseudoSelector`, `PseudoArgument`, `AttributeSelector`, `NamespaceTypeSelector`, `ComplexSelector`, `CompoundSelector`, `SelectorList`, `RelativeSelector`, `SupportsInParens`, `SupportsCondition`, `Enclosed`, `OpaqueBody`, `Stylesheet`, `Declaration`, `AtRuleStatement`, `ImportStatement`, `ImportUrl`, `ImportTail`, `Ruleset`, `AtRulePrelude*`, `Query*`) | ⚠ KEPT-OVERRIDE | GENUINE OVERRIDES (Sass math/list, `#{…}` interpolation, structured selector decomposition, Sass-specific directives). Recognizer and/or reducer differ structurally; not byte-identical candidates. Same override families as the less sweep. |
...
| **9** | Inherit `RelativeSelector` | `RelativeSelector` (less:4400) | `RelativeSelector` (css:1018) | KEPT-OVERRIDE (re-attempted on 0.50.6). Span attachment is NOT foldable — less attaches `withSourceSpan` here and across the selector tower (`ComplexSelector`/`SelectorList`) where css omits it; a plain inherit drops the leading-combinator span. Fusion also STILL fails under 0.50.6 (132 fallbacks; new diagnostic `ref() used before .define()`, not the old VarCall-import one). Owner call: css attach spans for all dialects? | diagnostics-core `EXTEND_TARGET_TYPES` (`ComplexSelector`) |
| **10** | Inherit `SupportsInParens` + `SupportsCondition` | less:4973, 4984 | css:3185, 3201 | needs a css-base named leaf (NOT a parseman capability — see row 10 correction): converge only if the `@supports` subtree rebinds its feature-leaf (`SupportsFeature`) + widened `Enclosed` WITHOUT disturbing media's shared `QueryFeature`. Probe parseman first | none |
...
| RelativeSelector | 4400 | **O** | RelativeSelector:1018 | KEPT-OVERRIDE: `withSourceSpan` delta is a real span-policy override (tower-wide); fusion still red under 0.50.6 (`ref() used before .define()`, 132 fallbacks) |
...
| RelativeSelector (4400) | DISPROVEN (0.50.6 re-attempt): `withSourceSpan` (less:4409) is NOT foldable — tower-wide less span policy; a plain inherit drops the leading-combinator span. Also still fails fusion (`ref() used before .define()`). Owner call: css attach spans for all dialects? | span + parseman slot-ordering |
| SupportsInParens (4973) + SupportsCondition (4984) | parseman can rebind the `@supports` subtree's feature-leaf (`SupportsFeature`) + widened `Enclosed` WITHOUT disturbing media's shared `QueryFeature` | css-base slot (row 10 correction) |
