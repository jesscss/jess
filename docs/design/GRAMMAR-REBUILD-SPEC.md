...
| ----------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| parseman version jess resolves                  | **0.50.5** from the registry; parseman is a regular `dependency` of parser-shared + all four parser packages (moved from peer→dependency 2026-09-02 so a consumer installs it transitively, never itself), plus the root devDependency — all ranges `^0.50.5`, **6 declarations across 6 manifests** | `node -p "require('./node_modules/parseman/package.json').version + ' ' + require('fs').realpathSync('./node_modules/parseman')"` and `grep -rn '"parseman"' --include=package.json . \| grep -v node_modules` |
...
| parseman floor                                                     | `^0.50.5` on `dev` — a regular `dependency` in parser-shared and all four parser manifests (no longer a peer), plus the root devDependency — **6 declarations across 6 manifests** — regenerate with `grep -rn '"parseman"' --include=package.json . \| grep -v node_modules` |
