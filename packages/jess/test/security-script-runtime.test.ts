...
describe("Jess restricted script runtime integration", () => {
  ...
  it("reports Less inline backtick JavaScript before script runtime startup", async () => {
    ...
    const result = await compiler.renderToResult(styleFile, {
      breakOnError: false,
    });
  ...
  it("forwards removed bare Less prelude variable diagnostics", async () => {
    const root = makeTmpDir();
    const styleFile = path.join(root, "main.less");
    fs.writeFileSync(styleFile, "@media @q { .card { color: red; } }", "utf8");
    const compiler = new Compiler({
      compile: {
        plugins: [lessPlugin()],
      },
  ...
  it("loads file-based Less @plugin scripts through Deno when plugin-js is lazy-loaded", async () => {
    ...
    const compiler = new Compiler({
      output: { collapseNesting: true },
      compile: {
        plugins: ["@jesscss/plugin-js", lessPlugin(), lessCompatPlugin()],
      },
    });
