/**
 * ScssParserParseman — drop-in adapter wrapping ScssGrammar.
 *
 * Mirrors the ScssParser (Chevrotain) public API so callers can swap parsers
 * without changing call sites.
 */
import type { Node } from '@jesscss/core';
import { ScssGrammar } from './grammar.js';

export type ScssParserConfig = Record<string, never>;
...
export class ScssParserParseman {
  ...
  readonly grammar: ScssGrammar;

  constructor(_config?: ScssParserConfig) {
    this.grammar = new ScssGrammar();
  ...
  parse(text: string, rule = 'stylesheet', _options?: { context?: unknown }): ParseResult {
    ...
    // RuleKeys<ScssGrammar> only accepts capital-letter keys; cast needed for lowercase
