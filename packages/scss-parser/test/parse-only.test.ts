...
const positiveCases: ParseCase[] = [
  ...
  { name: 'Sass bracketed list literal', src: '.a { x: [foo]; y: [1, 2]; z: [[1, 2], [3, 4]]; }' },
  { name: 'SCSS arithmetic expression', src: '.a { width: $v + 2; }' },
  { name: 'isolated parenthesized slash division', src: '.a { width: (15px/30px); }' },
  { name: 'paren list with slash separator remains a list', src: '.a { font: (bold 15px/30px sans-serif); }' },
  ...
  { name: '@mixin suffix rest param', src: '@mixin foo($a, $rest...,) { @content; }' },
  ...
  { name: '@include bare mixin call', src: '@include wrap;' },
  { name: '@include spread args', src: '@include wrap($args...,);' },
  { name: '@include literal spread args', src: '@include wrap(1..., (c: 2)...,);' },
  ...
  { name: '@include bare module-qualified mixin call', src: '@include ns.foo;' },
  { name: '@include keyword args', src: '@include wrap($x: 1, $y: 2);' },
  ...
  { name: 'plain CSS @import url preserved', src: '@import url("foo.css");' },
