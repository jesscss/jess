...
| Less `parseLessAstStylesheet` proof | `../less-parser/src/ast.ts` | R2, R3, R5, R6, R10 | Builds on the same scanner boundaries to produce existing core `Stylesheet`, `Rules`, `Ruleset`, `Declaration`, `VarDeclaration`, `Mixin`, `Call`, `AtRule`, and `AtRuleStatement` nodes for a small Less subset. Simple `@name:` variables become `VarDeclaration` nodes with string values; ordinary declaration values such as `@tone` or `rgb(...)` stay strings until evaluation/hydration proves a typed value is necessary. Detached ruleset variable values of the form `@name: { ... }` become `VarDeclaration` values backed by anonymous `Mixin` nodes with recursively parsed `Rules`. Plain `& { ... }` scope blocks become direct `Rules` nodes with recursively parsed children, not fake ampersand selector `Ruleset` nodes. `.name() { ... }` / `#name() { ... }` mixin definitions become real `Mixin` nodes with string-backed bodies; cheap `@name` parameters become `List` entries of parameter `VarDeclaration(paramVar)` nodes with `Nil` values because current core callable matching consumes that shape; cheap literal pattern parameters such as `1`, `'left'`, and `left` become existing `Num`, `Quoted`, or atom `Any` nodes; cheap default parameters such as `@tone: red` become the same parameter declarations with string values; rest parameters `...` and `@name...` become existing core `Rest` nodes in the params list; comma-separated definitions keep top-level comma runs inside a default value when the next comma arm is not another parameter. This is accepted as a current-core bridge, not proof that parameter storage has reached the lowest-object final design. Parenthesized `.name(...)` / `#name(...) !important` mixin calls become real `Call` nodes with `Reference(type=mixin-ruleset, role=name)` names. Namespaced calls such as `#ns.mixin(1)`, `#theme.dark.navbar()`, `#theme > .mixin()`, and `#namespace .borders()` reuse the same `Call` node and existing `Reference.key` array shape instead of introducing a namespace-call wrapper or selector/combinator nodes. Empty calls avoid an argument list; cheap comma- or semicolon-separated argument calls split only at top-level delimiters. Semicolon-root calls keep commas inside an argument value instead of rejecting the call as mixed syntax, and trailing semicolons are accepted only for semicolon-root calls. Named call arguments such as `@tone: red` become `VarDeclaration(paramVar)` entries with string values. Positional call arguments still use `Any` nodes because current core `Call.args` is `List<Node>`; that `Any` bridge is current-core debt, not the desired final string-backed argument storage. Spread call arguments `@name...` and bare `...` become existing `Rest` nodes; invalid trailing ellipsis text remains unsupported. No-paren deprecated calls still wait for their own production slice. Root function-call statements such as `test-collapse()`, `store(@var)`, `test-atrule("@charset"; '"utf-8"')`, and `e('...')` become existing `Call` nodes with `Reference(type=function, fallbackValue=true)` names and `silentFail` options, matching the current Less function-call AST contract. Function-call arguments parse only a cheap defensible subset into existing nodes: variables become `Reference(type=variable)`, quoted strings become `Quoted`, bare numbers become `Num`, simple atoms become `Any`, mixed comma/semicolon groups become nested `List` nodes, and escaped parens become `Paren(escaped)`. Arguments containing real brace blocks, sequences, nested function expressions, or other structured values are rejected so `each(..., { ... })`, `if(..., { ... })`, `func(a b)`, and `func(rgb(...))` wait for their own productions instead of broad string smuggling. Cheap guarded ruleset and mixin headers split a top-level `when ...` suffix into string-backed `guard` fields when the guard starts with a parenthesized condition, with optional `not`; `& when (...) { ... }` becomes a nil-selector `Ruleset` so the guard has a real owning node without creating a fake ampersand selector. String guards are parse-only deferred fields and must be hydrated before evaluation. Cheap block at-rules such as `@media screen { ... }` become real `AtRule` nodes with string `name`/`prelude` fields and recursively parsed rules. Balanced simple preludes such as `screen and (min-width: 1px)` use the shared `@jesscss/parser` `scanCheapAtRulePrelude(...)` tokenizer, then Less locally materializes core `QueryCondition` / `Paren` nodes. Cheap comma-list preludes such as `screen and (min-width: 1px), print` use `scanCheapAtRulePreludeList(...)` and become core `List` preludes only when every comma item is already accepted by the cheap prelude tokenizer. When those cheap tokenizers cannot represent valid Less-only at-rule syntax, a Less-local balanced prelude validator accepts the field as a deferred string instead of importing CSS parser helpers or creating another structural facade; it handles strings, comments, line comments, escapes, `()`, `[]`, and `@{...}` interpolation, and rejects brace blocks inside the prelude. Nested guard internals, interpolation hydration, and general-enclosed syntax still wait for real deferred-field hydration. Cheap selector structure reuses the shared parser selector scanner, and block headers must be positively tokenized as cheap selector structure before they become `Ruleset` nodes, except interpolation-bearing Less selector headers and ampersand suffix selector headers that pass Less-local deferred-text validation and stay as string selectors. Keyframe selector headers are accepted only while parsing immediate `@keyframes` bodies: `from`, `to`, numeric percentages, and comma-lists of numeric percentages become string-backed selector fields or existing `SelectorList` containers with string entries. Top-level comma selector headers become real core `SelectorList` nodes; cheap list atoms stay string items and complex/compound list members use the existing selector containers with string components, rather than materializing `BasicSelector`, `PseudoSelector`, attribute selector, or `Combinator` leaves. Pseudo-no-parens, attribute selectors, and pragmatic non-ASCII names are cheap string atoms; pseudo functions and other structured selector syntax still wait for deferred selector parsing. The Less AST corpus gate now parses 1352 top-level rules with 121 warnings and zero errors/thrown failures. | This is an early Less AST-production proof, not Less output compatibility. Keep expanding productions from current Less tokens/tests one narrow family at a time. |
