...
| Less `parseFlatLessDeclarationStylesheet` proof | `../less-parser/src/ast.ts` | R2, R3, R5, R6, R10 | Builds on the same scanner boundaries to produce existing core `Stylesheet`, `Ruleset`, `Declaration`, `VarDeclaration`, and `AtRuleStatement` nodes for a tiny flat Less subset. Simple `@name:` variables become `VarDeclaration` nodes with string values; ordinary declaration values such as `@tone` or `rgb(...)` stay strings until evaluation/hydration proves a typed value is necessary. Cheap selector structure reuses the CSS helper and unsupported nested blocks/at-rule blocks produce diagnostics rather than fallback parsing through Chevrotain. | This is the first Less AST-production proof, not Less output compatibility. Keep expanding productions from current Less tokens/tests one narrow family at a time. |
