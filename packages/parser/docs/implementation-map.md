...
| Less `parseLessAstStylesheet` proof | `../less-parser/src/ast.ts` | R2, R3, R5, R6, R10 | Builds on the same scanner boundaries to produce existing core `Stylesheet`, `Ruleset`, `Declaration`, `VarDeclaration`, and `AtRuleStatement` nodes for a small Less subset. Simple `@name:` variables become `VarDeclaration` nodes with string values; ordinary declaration values such as `@tone` or `rgb(...)` stay strings until evaluation/hydration proves a typed value is necessary. Cheap selector structure reuses the CSS helper, and block headers must be positively tokenized as cheap selector structure before they become `Ruleset` nodes. Nested qualified rules recurse into real `Ruleset` children while unsupported at-rule blocks, mixin-like headers, and guarded headers produce diagnostics rather than fallback parsing through Chevrotain. | This is an early Less AST-production proof, not Less output compatibility. Keep expanding productions from current Less tokens/tests one narrow family at a time. |
