...
Today, `@jesscss/parser` owns:

- the recursive-descent parser base class
...
- optional services for deferred field hydration, visitor materialization plans, semantic indexes, and language activation, if they survive review
...
This is a design flow, not a current public import from this branch. The
scanner-first prototype branch has a `parseStructure(...)` experiment that
allocates a structural tree, field range side table, trivia ranges, diagnostics,
...
If a language activation layer survives review, it should bind caller-owned
profiles and optional provider setup to extensions:
...
That layer is the intended extension point for first-party and third-party
stylesheet languages. It records profile/extension bindings without hard-coding
language names into the parser package.
...
- [docs/requirements-and-scope.md](docs/requirements-and-scope.md) defines the
  ...
  answer.
- [docs/implementation-map.md](docs/implementation-map.md) audits the
  scanner-first prototype objects and maps them back to those requirements so
  this branch can import only the pieces that prove their value.

## Current Status

The scanner-first layer is still being selected from prototype work. It is
expected to change as the CSS/Less path proves a complete parse path and as
