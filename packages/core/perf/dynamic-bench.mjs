...
for (let i=1;i<=1200;i++) src+=`.block-${i}{ .mx(${i%20+1},@c${(i%2)+1}); width:(@base*${i%10+1}); color:mix(@c1,@c2,${i%100}%);}\n`;
const p = join(here,'dyn.less'); writeFileSync(p, src);
const t=[]; for(let i=0;i<3;i++) await new Compiler().render(p,{output:{collapseNesting:false}});
for(let i=0;i<25;i++){const a=performance.now(); await new Compiler().render(p,{output:{collapseNesting:false}}); t.push(performance.now()-a);}
t.sort((x,y)=>x-y);
