...
describe('Conversion Plugins', () => {
  describe('percentOf', () => {
    it('should convert percentage to fraction of base value', () => {
      ...
      expect((result as Num).value.number).toBe(127.5); // 50% of 255
    });
    ...
    it('should work with different base values', () => {
      ...
      expect((result as Num).value.number).toBe(75); // 75% of 100
    });
  });
  ...
  describe('angleToDegrees', () => {
    it('should convert turns to degrees', () => {
      ...
      expect((result as Num).value.number).toBe(180); // 0.5 turn = 180 degrees
    });
    ...
    it('should convert radians to degrees', () => {
      ...
      expect((result as Num).value.number).toBe(180); // π radians = 180 degrees
    });
    ...
    it('should convert gradians to degrees', () => {
      ...
      expect((result as Num).value.number).toBe(90); // 100 grad = 90 degrees
    });
    ...
    it('should pass through degrees and unitless values', () => {
      ...
      expect((converter(degInput) as Num).value.number).toBe(45);
      expect(converter(unitlessInput)).toBeInstanceOf(Num);
      expect((converter(unitlessInput) as Num).value.number).toBe(90);
    });
  });
  ...
  describe('normalizeHue', () => {
    it('should normalize hue values to 0-360 range', () => {
      ...
      // Test various inputs
      expect((converter(new Dimension({ number: 400, unit: 'deg' })) as Num).value.number).toBe(40);
      expect((converter(new Dimension({ number: -30, unit: 'deg' })) as Num).value.number).toBe(330);
      expect((converter(new Dimension({ number: 1.5, unit: 'turn' })) as Num).value.number).toBe(180);
    });
    ...
    it('should handle percentage inputs', () => {
      ...
      expect((result as Num).value.number).toBe(180); // 50% of 360 = 180
    });
  });
  ...
  describe('alphaToNumber', () => {
    it('should convert percentage alpha to 0-1 range', () => {
      ...
      expect((result as Num).value.number).toBe(0.6); // 60% = 0.6
    });

    it('should clamp values to 0-1 range', () => {
      const converter = alphaToNumber();

      expect((converter(new Dimension({ number: 150, unit: '%' })) as Num).value.number).toBe(1);
      expect((converter(new Dimension({ number: -10, unit: '%' })) as Num).value.number).toBe(0);
    });
    ...
    it('should handle unitless values', () => {
      ...
      expect((result as Num).value.number).toBe(0.8);
    });
  });
  ...
  describe('toNumber', () => {
    it('should convert any dimension to number', () => {
      ...
      expect((result as Num).value.number).toBe(42);
    });
  });
  ...
  describe('lengthToPx', () => {
    it('should convert various length units to pixels', () => {
      ...
      expect((converter(new Dimension({ number: 1, unit: 'em' })) as Num).value.number).toBe(16);
      expect((converter(new Dimension({ number: 1, unit: 'rem' })) as Num).value.number).toBe(16);
      expect((converter(new Dimension({ number: 1, unit: 'in' })) as Num).value.number).toBe(96);
      expect((converter(new Dimension({ number: 1, unit: 'cm' })) as Num).value.number).toBeCloseTo(37.8, 1);
    });
  });
  ...
  describe('timeToMs', () => {
    it('should convert time units to milliseconds', () => {
      ...
      expect((converter(new Dimension({ number: 1, unit: 's' })) as Num).value.number).toBe(1000);
      expect((converter(new Dimension({ number: 500, unit: 'ms' })) as Num).value.number).toBe(500);
    });
  });
  ...
  describe('frequencyToHz', () => {
    it('should convert frequency units to hertz', () => {
      ...
      expect((converter(new Dimension({ number: 1, unit: 'khz' })) as Num).value.number).toBe(1000);
      expect((converter(new Dimension({ number: 440, unit: 'hz' })) as Num).value.number).toBe(440);
    });
  });
  ...
  describe('angleToRadians', () => {
    it('should convert angle units to radians', () => {
      ...
      expect((converter(new Dimension({ number: 180, unit: 'deg' })) as Num).value.number).toBe(Math.PI);
      expect((converter(new Dimension({ number: 0.5, unit: 'turn' })) as Num).value.number).toBe(Math.PI);
    });
  });
});
