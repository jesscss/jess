...
import { decl, dimension, funcCall, operation, paren, quoted, rule, stylesheet } from '../nodes.js';
    ...
    const document = stylesheet([
      rule('.math', [
        decl('rounded', funcCall('round', [
          paren(operation('/', dimension(32), dimension(3))),
        ])),
        decl('unitless', funcCall('unit', [
          paren(operation('/', operation('*', dimension(4, 'px'), dimension(4, 'em')), dimension(2, 'cm'))),
        ])),
      ]),
    ]);
