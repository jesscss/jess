...
import {
  collection, collectionEntry, decl, dimension, keyword, nestedPropertyBlock, rule, stylesheet, type Stylesheet
} from '../nodes.js';
...
const nestedBlock = (
  entries: Parameters<typeof nestedPropertyBlock>[0],
  base?: Parameters<typeof nestedPropertyBlock>[1]
): ReturnType<typeof nestedPropertyBlock> => nestedPropertyBlock(entries, base);
...
/**
 * A `NestedPropertyBlock` at a PROPERTY ROOT is STRUCTURE: it expands to hyphenated
...
describe('SCSS nested-property flatten', () => {
  ...
  it('emits the carrier value first, then the leaves, in BOTH emitters', () => {
    const document = stylesheet([
      rule('.x', [decl('font', nestedBlock([entry('family', keyword('serif'))], dimension(20, 'px')))])
    ]);
  ...
  it('recurses through a deeper carrier in BOTH emitters', () => {
    const document = stylesheet([
      rule('.x', [decl('border', nestedBlock([
        entry('left', nestedBlock([
  ...
  it('keeps a nested property flattened alongside ordinary siblings in BOTH emitters', () => {
    const document = stylesheet([
      rule('.x', [
        ...
        decl('font', nestedBlock([entry('family', keyword('serif'))])),
  ...
  /**
   * A custom property takes a data `Collection`: `--foo: { a: 1 }` is valid CSS
