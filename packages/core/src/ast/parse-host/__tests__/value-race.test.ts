...
async function race(name: string, src: string): Promise<void> {
  ...
  const t2 = (await serialize(bridgeToAst(tree, src), { evaluator: buildEvaluator() })).css;
  ...
  for (let i = 0; i < WARM; i++) await serialize(bridgeToAst(tree, src), { evaluator: buildEvaluator() });
  gc?.();
  const m0 = process.memoryUsage().heapUsed;
  const t2times: number[] = [];
  for (let i = 0; i < N; i++) {
    const a = performance.now();
    await serialize(bridgeToAst(tree, src), { evaluator: buildEvaluator() });
