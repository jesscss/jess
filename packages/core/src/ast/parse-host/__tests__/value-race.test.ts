 ...
 *
 * HONEST FRAMING (post-R2): value math is now done in-engine — operators delegate
...
async function race(name: string, src: string): Promise<void> {
  ...
  const t2 = (await serialize(bridgeToAst(tree, src), { evaluator: buildAdapterEvaluator() })).css;
  ...
  // tree2 lane: full synchronous serialize INCLUDING value eval.
  for (let i = 0; i < WARM; i++) await serialize(bridgeToAst(tree, src), { evaluator: buildAdapterEvaluator() });
  gc?.();
  const m0 = process.memoryUsage().heapUsed;
  const t2times: number[] = [];
  for (let i = 0; i < N; i++) {
    const a = performance.now();
    await serialize(bridgeToAst(tree, src), { evaluator: buildAdapterEvaluator() });
...
describe('tree2 bridge — value-eval race (R2: operations + functions)', () => {
  it('race', async () => {
    console.log(`\n===== TREE2 vs TREE RACE — R2 value-eval (gc=${gc ? 'on' : 'off'}) =====`);
    console.log('t2 = full sync serialize INCLUDING value eval ; tree = full REAL oracle render (math inline)');
