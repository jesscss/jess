  ...
  it('scan', async () => {
    ...
    for (const file of files) {
      ...
      try {
        ...
        astRoot = bridgeToAst(parsed.tree, src, file);
      } catch (e) {
      ...
      let flatPass = false;
      try {
        const t2 = (await serialize(astRoot, { evaluator })).css;
      ...
      try {
        const t2 = (await serialize(astRoot, { evaluator, collapseNesting: false })).css;
