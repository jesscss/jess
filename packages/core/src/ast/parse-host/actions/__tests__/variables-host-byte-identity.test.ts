...
/**
 * F1 (variables) byte-identity: the registry-driven tree2 host
...
function direct(src: string): string {
  const { root, errors } = parseToAst(src, g['Stylesheet'], undefined, { trivia: g['rw'] });
...
function bridged(src: string): string {
  return serialize(bridgeToAst(parseLessFn(src).tree, src)).css;
}
...
describe('variables host byte-identity vs bridge', () => {
