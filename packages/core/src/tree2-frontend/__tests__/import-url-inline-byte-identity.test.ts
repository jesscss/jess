...
async function renderTree2(file: string): Promise<string> {
  const src = fs.readFileSync(file, 'utf8');
  const bridged = bridgeToTree2(parseLessFn(src).tree, src, file, createImportState());
...
describe('tree2 @import — url(<x>.less) inlines as Less (byte-identical to Less 4.x)', () => {
  it('import-test-a: url(import-test-f.less) + url("deeper/url-import.less") inline', async () => {
    expect(await renderTree2(`${ROOT}/import/import/import-test-a.less`)).toBe(LESS_4X_REFERENCE);
  });

  it('json/index: @import "../import-test-a" resolves nested url imports from the target dir', async () => {
    expect(await renderTree2(`${ROOT}/import/import/json/index.less`)).toBe(LESS_4X_REFERENCE);
  });
});
