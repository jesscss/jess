/**
 * [tree2-native] Registry-driven parseman build host + a tree2 parse driver.
 *
 * `DispatchTree2Host` is a `FunctionalParseHost` whose `build(type, …)` is ONE
 ...
 *
 * `runFunctionalParseT2` is the tree2 variant of the css-parser
...
export class DispatchTree2Host implements FunctionalParseHost {
...
export interface FunctionalParseT2Result {
 ...
 */
export function runFunctionalParseT2(
  input: string,
  entry: unknown,
  host: DispatchTree2Host = new DispatchTree2Host(),
  options: { trivia?: unknown } = {},
): FunctionalParseT2Result {
