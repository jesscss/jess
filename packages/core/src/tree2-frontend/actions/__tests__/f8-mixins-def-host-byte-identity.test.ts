...
import { serialize, Root, MixinDef, Declaration, root as t2root, type ValueNode, type Param } from '../../../tree2/index.js';
import { bridgeToTree2 } from '../../bridge.js';
import { runFunctionalParseT2 } from '../../dispatch-host.js';

/**
 * [tree2-native] F8 byte-identity: the mixin-definition family produces a
...
function bridgeDef(src: string): MixinDef {
  const root = bridgeToTree2(parseLessFn(src).tree, src) as unknown as Root;
  const def = (root.children ?? []).find((s): s is MixinDef => s instanceof MixinDef);
...
describe('[tree2-native] F8 mixin-def host byte-identity (structural) vs bridge', () => {
...
describe('[tree2-native] F8 mixin-def keyword/multi-token defaults (direct correct; bridge buggy)', () => {
