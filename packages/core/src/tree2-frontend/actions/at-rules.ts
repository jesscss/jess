/**
 * [tree2-native] At-rule BLOCK family (F14): block at-rules and nested at-rules
 ...
 *
 * The construction MIRRORS the bridge oracle (`bridge.ts` `toAtRuleBlock` /
 * `atRuleHeaderPrelude`): the at-keyword is the leading `@name` token; the prelude
...
/**
 * Tokenize a block at-rule's prelude bytes into a tree2 value, turning `@name`
 * references into `VarRef`, `@{name}` into `Interp`, and `@@name` into
 * `VarIndirect`, leaving everything else literal. Byte-for-byte the bridge's
 * `parseValue`, so a static prelude collapses to a single `Word` identical to the
 * legacy→bridge path.
 */
