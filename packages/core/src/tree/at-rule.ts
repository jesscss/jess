...
function registerInnerExtendRootIfHoisted(
  ...
  if (wrapperRules._value.length !== 1) {
    return;
  }
  const first = wrapperRules._value[0];
...
export class AtRule extends Node<AtRuleValue, AtRuleOptions, AtRuleChildData> {
  ...
  /** @internal */ _name!: Any<'atkeyword'> | Interpolated<'atkeyword'>;
  /** @internal */ _prelude: Node | undefined;
  /** @internal */ _rules: Rules | undefined;
  ...
  constructor(value: AtRuleValue, options?: AtRuleOptions, location?: any, treeContext?: any) {
    ...
    this._prelude = value.prelude;
    this._rules = value.rules;
    if (this._name instanceof Node) {
      this.adopt(this._name);
    }
    if (this._prelude instanceof Node) {
      this.adopt(this._prelude);
    }
  ...
  override valueOf() {
    return (this._valueOf ??= (this._name.toString() + (this._prelude ? ' ' + this._prelude.valueOf() : '')));
  }
  ...
  private _preEvalPrelude(node: AtRule, context: Context): MaybePromise<AtRule | Nil> {
    const prelude = node.get('prelude', context);
    ...
    if (atRuleName === '@import') {
      ...
      if (!context.inReferenceImportScope) {
        ...
        const alreadyQueued = topImports.some((queuedNode) => {
          ...
          const queuedPrelude = queued.get('prelude', context);
          return (
            queued === node
    ...
    if (atRuleName === '@layer' && prelude) {
      ...
      if (preludeStr) {
        ...
        for (let i = context.frames.length - 2; i >= 0; i--) {
          ...
          const parentRules = parentFrame.get('rules', context);
          if (parentName?.toTrimmedString?.() === '@layer' && parentRules?._value?.includes(node)) {
  ...
  getHeaderString(options: FinalPrintOptions, withoutComments?: boolean): string {
    ...
    const prelude = this.get('prelude', options.context);
    const rules = this.get('rules', options.context);
  ...
  override evalNode(context: Context): MaybePromise<AtRule | Nil> {
    ...
    const atName = String(node.get('name', context)?.valueOf?.() ?? '');
    ...
    const tryMergeNestedMedia = () => {
      ...
      const outerRules = node.get('rules', context);
      if (!outerRules) {
        return;
      }
      const visible = outerRules._value.filter(n => n.visible);
      if (visible.length !== 1) {
        return;
      }
      const only = visible[0]!;
      if (!isNode(only, N.AtRule) || (only as AtRule).get('name', context)?.valueOf?.() !== '@media') {
        return;
      }
      const inner = only as AtRule;
      const innerRules = inner.get('rules', context);
      if (!innerRules) {
        return;
      }

      // Combine media queries using "and" like Less does.
      const outerPrelude = node.get('prelude', context);
      const innerPrelude = inner.get('prelude', context);
    ...
    return pipe(
      ...
      () => {
        ...
        if (rules) {
          ...
          if (node.isNestable(context)) {
            ...
            if (isThenable(preEvalResult)) {
              return (preEvalResult as Promise<Rules>).then((resolved) => {
                ...
                const onlyRuleSetChild = isNode(bodyToEval._value[0], N.Ruleset);
                ...
                const doRegister = (r: Rules) => {
                  ...
                  const finalRules =
                    onlyRuleSetChild && isNode(r._value[0], N.Rules) ? r._value[0] : r;
          ...
          let onlyRuleSetChild = isNode(bodyToEval._value[0], N.Ruleset);
          ...
          if (isThenable(out)) {
            return (out as Promise<Rules>).then((r) => {
              ...
              const finalRules = onlyRuleSetChild && isNode(r._value[0], N.Rules) ? r._value[0] : r;
              setField(node, 'rules', finalRules, context);
              tryMergeNestedMedia();
          ...
          const finalRules =
            onlyRuleSetChild && isNode(out._value[0], N.Rules) ? out._value[0] : out;
          setField(node, 'rules', finalRules, context);
          tryMergeNestedMedia();
