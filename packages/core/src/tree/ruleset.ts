...
export class Ruleset extends Rules<RulesetValue, RulesetOptions> {
  ...
  private static filterExtendedItems(sel: Selector): Selector | Nil {
    ...
    for (const item of sel.value) {
      ...
      if (item.hasFlag(F_EXTENDED) && !item.hasFlag(F_EXTEND_TARGET)) {
        ...
        seen.add(key);
        kept.push(copySelectorForRulesetMetadata(Ruleset.simplifyGeneratedIsSelector(item) ?? Ruleset.unwrapGeneratedIs(item)));
      }
    }
    if (!sawAddedSelector) {
      for (const item of sel.value) {
  ...
  static filterExtendedForReferenceCompose(parent: Selector, includeUntouchedSiblings: boolean = false): Selector | undefined {
    ...
    for (const item of parent.value) {
      ...
      seen.add(key);
      kept.push(Ruleset.simplifyGeneratedIsSelector(item)
  ...
  composeHeaderSelector(
    ...
    if (!cached) {
      const hasExtendedComposeContext = Boolean(Ruleset.hasExtendedTopLevelSelector(renderSelector)
