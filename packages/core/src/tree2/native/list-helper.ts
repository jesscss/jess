/**
 * Shared kernel for the LIST / VARIADIC native fns (`length`/`extract`/`min`/`max`).
 *
...
export function asList(v: ValueObj): List {
  if (v.kind !== 'list') throw new TypeError('variadic native fn expected a List argument');
  return v;
}
 ...
 * leniency: the `@jesscss/fns` port added one, so native correctly DIVERGES from
