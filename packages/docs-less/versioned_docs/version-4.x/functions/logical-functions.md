...
slug: "/functions/logical-functions"
audiences:
  - less
origin: less
---
...
Notes: A boolean expression supported as the `conditional` parameter are the same as [Guard Statements](../features/mixins#mixin-guards-feature).
```less
if(not (true), foo, bar);
