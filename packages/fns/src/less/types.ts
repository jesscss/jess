...
const isunit = defineFunction(
  ...
  function(value: Node, unit: Node) {
    ...
    const current = (value.data.unit ?? '').toLowerCase();
...
const ispixel = defineFunction(
  'ispixel',
  function(value: Node) {
    return new Bool(value instanceof Dimension && (value.data.unit ?? '').toLowerCase() === 'px');
  },
  {
    params: [{ name: 'value', type: Node }]
  }
...
const ispercentage = defineFunction(
  'ispercentage',
  function(value: Node) {
    return new Bool(value instanceof Dimension && (value.data.unit ?? '').toLowerCase() === '%');
  },
  {
    params: [{ name: 'value', type: Node }]
  }
...
const isem = defineFunction(
  'isem',
  function(value: Node) {
    return new Bool(value instanceof Dimension && (value.data.unit ?? '').toLowerCase() === 'em');
  },
  {
    params: [{ name: 'value', type: Node }]
  }
