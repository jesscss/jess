...
describe('hsl() relative color error paths', () => {
  it('throws when relative hsl has fewer than 3 channels', async () => {
    ...
    await expect(hslImplementation.call(makeThis(argsList))).rejects.toThrow(
  ...
  it('throws when slash alpha has invalid unit', async () => {
    ...
    await expect(hslImplementation.call(makeThis(argsList))).rejects.toThrow('Invalid alpha value unit: px');
  });
  ...
  it('accepts explicit 4th-channel alpha dimension', async () => {
    ...
    const result = await hslImplementation.call(makeThis(argsList));
