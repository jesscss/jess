...
describe('canonical Less trigonometric callables', () => {
  it('returns exact typed dimension nodes', () => {
    ...
    expect(atan(makeDimension(0.5))).toEqual({ type: 'Dimension', number: Math.atan(0.5), unit: 'rad', bytes: '0.46364761rad' });
    expect(sqrt(makeDimension(2.4, 'px'))).toEqual({ type: 'Dimension', number: Math.sqrt(2.4), unit: 'px', bytes: '1.54919334px' });
    expect(sin(makeDimension(2.4, 'px'))).toEqual({ type: 'Dimension', number: Math.sin(2.4), unit: '', bytes: '0.67546318' });
    expect(cos(makeDimension(2.4, 'px'))).toEqual({ type: 'Dimension', number: Math.cos(2.4), unit: '', bytes: '-0.73739372' });
    expect(tan(makeDimension(2.4, 'px'))).toEqual({ type: 'Dimension', number: Math.tan(2.4), unit: '', bytes: '-0.91601429' });
