...
describe('rgb() relative color error paths', () => {
  it('throws when relative rgb has fewer than 3 channels', async () => {
    ...
    await expect(rgbImplementation.call(makeThis(argsList))).rejects.toThrow(
  ...
  it('throws when slash alpha has invalid unit', async () => {
    ...
    await expect(rgbImplementation.call(makeThis(argsList))).rejects.toThrow('Invalid alpha value unit: px');
  });
  ...
  it('accepts explicit 4th-channel alpha dimension', async () => {
    ...
    const result = await rgbImplementation.call(makeThis(argsList));
