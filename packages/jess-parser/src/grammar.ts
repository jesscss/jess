...
export const jessGrammar = compose([cssGrammar, rules(
  { trivia: rw },
  (g: any) => {
   ...
   */
    ...
    const refDot = sequence(
      literal('.'),
      ident
    );
    const refIndex = sequence(
      literal('['),
      refIndexKey,
      literal(']')
    );
    const Reference = node(noTrivia(sequence(
      dollarVar,
      many(choice(
        refDot,
        refIndex
   ...
   */
    ...
    const DollarInterp = node(noTrivia(sequence(
      literal('$'),
      literal('['),
      interpKey,
      expect(
   ...
   */
    ...
    const InterpolatedSelector = node(noTrivia(sequence(
      optional(regex(/[.#]/)),
      many(selTextRun),
      g.DollarBrace,
      many(choice(
   ...
   */
    ...
    const simpleSelector = choice(
      g.AttributeSelector,
      g.PseudoSelector,
      { gate: (s: any) => !!(s?.inner), combinator: parentSelector },
   ...
   */
    ...
    const Expression = node(sequence(
      literal('$('),
      g.exprCompare,
      expect(
        literal(')'),
   ...
   */
    const unwrapSumTail = many(sequence(
      unwrapSumOp,
      g.unwrapProductRest
    ));
    const UnwrapArith = node(
      'Operation',
      noTrivia(choice(
   ...
   */
    ...
    const CollectionEntry = node(sequence(
      collKey,
      literal(':'),
      choice(
        g.JessCollection,
   ...
   */
    ...
    const condKeyword = node(
      'JessKeyword',
      regex(/-?[_a-zA-Z\u0080-\uffff][-_a-zA-Z0-9\u0080-\uffff]*/)
    );
    const condAtom = choice(
      g.Reference,
      g.Dimension,
      g.Num,
      g.Color,
   ...
   */
    const condOr = node(
      ...
      choice(
        ...
        sequence(
          g.condCompare,
          not(regex(/\s*(?:and|or)(?![-\w])/))
        )
      ),
      undefined,
      { collapse: true }
    );
    const controlBody = sequence(
      literal('{'),
      g.declarationList,
      expect(literal('}'))
    );
   ...
   */
    ...
    const MixinParam = node(sequence(
      mixinParamName,
      optional(sequence(
        literal(':'),
        g.valueSequence
    ...
    const mixinGuard = sequence(
      regex(/when(?![-\w])/),
      g.condOr
    );
    const Mixin = node(sequence(
      mixinName,
      literal('('),
      mixinParams,
      literal(')'),
   ...
   */
    const VariableMixinCall = node(sequence(
      dollarVar,
      literal('('),
      callArgs,
      expect(literal(')')),
   ...
   */
    ...
    const extendTarget = choice(
      ...
      sequence(
        optional(extendNs),
        oneOrMore(extendTargetPart)
      )
    );
    const Extend = node(sequence(
      regex(/\$extend(?![-\w])/),
      extendTarget,
      many(sequence(
        literal(','),
   ...
   */
    const SelectorCapture = node(sequence(
      literal('*['),
      g.SelectorList,
      expect(
        literal(']'),
   ...
   */
    ...
    const importSpec = sequence(
      importName,
      optional(asClause)
    );

    const ComposeAtRule = node(sequence(
      regex(/@-compose(?![-\w])/),
      importPath,
      optional(asClause),
      optional(literal(';'))
    ));
    const ExportAtRule = node(sequence(
      regex(/@-export(?![-\w])/),
      importPath,
      optional(literal(';'))
    ));
    const ImportAtRule = node(sequence(
      regex(/@-import(?![-\w])/),
      importPath,
      optional(literal(';'))
    ));
    const UseAtRule = node(sequence(
      regex(/@-use(?![-\w])/),
      importPath,
      optional(asClause),
      optional(literal(';'))
    ));
    const FromAtRule = node(sequence(
      regex(/@-from(?![-\w])/),
      importPath,
      regex(/import(?![-\w])/),
      choice(
   ...
   */
    ...
    const QueryInterpAtRuleBlock = node(
      'QueryAtRuleBlock',
      sequence(
        mediaAtKeyword,
        g.DollarBrace,
