...
export const cssAstGrammar = composeLeaf([cssAstSyntax, opaqueAtRuleRecognition, cssAstPseudoSyntax, rules(
  ...
  (g) => {
    ...
    const CssAstComment = node(
      'CssAstComment',
      blockComment,
      children => comment(tokenText(children[0]))
    );
    const CssAstSimple = node(
      'CssAstSimple',
      simpleSelectorToken,
      ...
      sequence(
        ...
        choice(
          sequence(
            ...
            literal('('),
            g.CssAstPseudoArgument,
            literal(')')
          ),
          sequence(
            g.CssAstSyntaxNthTypeName,
            literal('('),
            g.CssAstOfTypePseudoArgument,
            literal(')')
          ),
          sequence(
   ...
   */
    const CssAstNestingSelector = node(
      'CssAstNestingSelector',
      literal('&'),
      () => simpleSelector('&')
    );
    const CssAstCompound = node(
      'CssAstCompound',
      noTrivia(parser(
    ...
    const CssAstProperty = node(
      'CssAstProperty',
      g.CssAstSyntaxProperty,
      children => tokenText(children[0])
    );
    const CssAstCustomProperty = node(
   ...
   */
    ...
    const CssAstCalcVarFallbackParen = node(
      ...
      sequence(
        not(choice(
          calcVarFallbackParenCrossBracket,
          calcVarFallbackParenCrossBrace
        )),
        literal('('),
   ...
   */
    const CssAstCalcVarFallbackBracket = node(
      ...
      sequence(
        not(choice(
          calcVarFallbackBracketCrossParen,
          calcVarFallbackBracketCrossBrace
        )),
        literal('['),
        scanTo(
          literal(']'),
    ...
    const CssAstCalcVarFallbackBrace = node(
      ...
      sequence(
        not(choice(
          calcVarFallbackBraceCrossParen,
          calcVarFallbackBraceCrossBracket
        )),
        literal('{'),
        scanTo(
          literal('}'),
    ...
    const CssAstCalcVarFallbackTerm = node(
       ...
       */
        choice(
          g.CssAstCalcVarCall,
          g.CssAstCalcVarFallbackCall,
          g.CssAstValueAtom,
          g.CssAstCalcVarFallbackParen,
          g.CssAstCalcVarFallbackBracket,
          g.CssAstCalcVarFallbackBrace,
          g.CssAstCalcVarFallbackPunctuation
        ),
        many(sequence(
          many(calcWhitespace),
          choice(
    ...
    const CssAstCalcVarFallbackItem = node(
      ...
      choice(
        g.CssAstCalcVarFallbackTerm,
        g.CssAstCalcVarFallbackEmpty
      ),
   ...
   */
    const CssAstDeclarationParen = node(
      ...
      sequence(
        literal('('),
        optional(g.CssAstDeclarationValue),
        literal(')')
      ),
      children => block(valueSlotChildren(children)[0] ?? any(''))
    );
     ...
     */
      choice(
        ...
        sequence(
          nonSlashDeclarationAnyCharacter,
          many(declarationAnyCharacter)
        )
      ),
      children => any(children.map(tokenText).join(''))
   ...
   */
     ...
     */
      choice(
        ...
        g.CssAstUnicodeRange,
        g.CssAstDeclarationIdent,
        g.CssAstDeclarationParen,
        g.CssAstQuoted,
        CssAstCustomPropertyValue,
        g.CssAstDeclarationAny
      ),
      children => firstValue(children)
    );
   ...
   */
    ...
    const CssAstAtRuleStatement = node(
      ...
      sequence(
        g.CssAstSyntaxStatementAtRuleName,
        g.CssAstStatementPrelude,
        literal(';')
      ),
      (children) => {
        const name = tokenText(children[0]);
   ...
   */
    ...
    const CssAstStatementPrelude = node(
    ...
    const CssAstOpaqueAtPrelude = node(
      'CssAstOpaqueAtPrelude',
      g.CssAstOpaqueCapturePrelude,
      (children) => {
        const text = children.length === 0 ? '' : tokenText(children[0]).trim();
   ...
   */
    ...
    const CssAstGeneralEnclosedGroup = node(
      ...
      choice(
        noTrivia(sequence(
          literal('('),
          g.CssAstGeneralEnclosedContent,
          literal(')')
        )),
        noTrivia(sequence(
          literal('['),
    ...
    const CssAstQueryFunction = node(
      ...
      children => funcCall(
        tokenText(children[0]!),
        [any(children.length > 2 ? tokenText(children[2]!) : '')]
      )
    );
   ...
   */
    ...
    const cssNestedBody = choice(
      g.CssAstComment,
      g.CssAstAtRuleStatement,
      g.CssAstDeclaration,
      g.CssAstNestedConditionalBlock,
      g.CssAstDescriptorBlock,
      g.CssAstFontFeatureValuesBlock,
      g.CssAstScopeBlock,
      g.CssAstNestedStartingStyleBlock,
      g.CssAstNestedLayerBlock,
      g.CssAstPageBlock,
      g.CssAstKeyframes,
      g.CssAstDocumentBlock,
      g.CssAstOpaqueAtRuleBlock,
      g.CssAstRuleset,
      literal(';')
    );
    const cssDeclarationBody = choice(
      g.CssAstComment,
      g.CssAstDeclaration,
      literal(';')
    );
    const cssConditionalBody = choice(
      g.CssAstComment,
      g.CssAstConditionalBlock,
      g.CssAstDescriptorBlock,
      g.CssAstFontFeatureValuesBlock,
      g.CssAstScopeBlock,
      g.CssAstLayerBlock,
      g.CssAstStartingStyleBlock,
      g.CssAstPageBlock,
      g.CssAstDocumentBlock,
      g.CssAstOpaqueAtRuleBlock,
      g.CssAstRuleset
    );
    const cssBlockBody = choice(
      g.CssAstComment,
      g.CssAstAtRuleStatement,
      g.CssAstConditionalBlock,
      g.CssAstDescriptorBlock,
