...
import {
  ...
  /*
   * CSS grammar reducer helpers (hoisted to core/ast so the composable base
   * carries import provenance for every reducer binding).
   */
  authoredText,
  blockStatements,
  branchSegments,
...
const cssFactory = (g: GrammarSelf) => {
  ...
  /*
   * The simple-selector atom shared by the compound towers. `&` (NestingSelector)
   * is NOT an arm here: a nested `&` is valid inside `CompoundSelector` but a
   * top-level compound never begins with one, so `TopLevelCompoundSelector`
   * inherits this atom as-is while `CompoundSelector` adds the `&` arm on top.
   * A superset overrides this one leaf to widen every compound tower
   * (interpolation, `%placeholder`, …) via open-recursive compose.
   */
