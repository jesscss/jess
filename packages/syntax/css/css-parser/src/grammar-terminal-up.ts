/**
 * Terminal-up CSS grammar — tournament Candidate A, Shape 1.
 *
...
const terminalUpFactory = (g: Record<string, Combinator>) => {
  ...
  const UrlQuoted = node(
    'Url',
    sequence(routed(), Quoted, literal(')')),
  ...
  const Component = choice(IdentOrFunction, Quoted, Color, UnicodeRange, Numeric, Group, Punctuation);
  ...
  const AttributeSelector = node(
    ...
    sequence(
      literal('['),
      g.Identifier,
      optional(sequence(g.AttributeOperator, choice(Quoted, g.Identifier))),
  ...
  const ComplexSelector = node(
    'ComplexSelector',
    sequence(CompoundSelector, many(sequence(optional(combinator), CompoundSelector))),
    children => simpleSelector(String(children[0]))
  );
  ...
  const Declaration = node(
    'Declaration',
    sequence(g.Identifier, literal(':'), ValueList, optional(Important)),
  ...
  const AtRuleBlock = node(
    'AtRuleBlock',
    sequence(routed(), optional(ValueList), Block),
  ...
  const AtRuleStatement = node(
    'AtRuleStatement',
    sequence(routed(), optional(ValueList), literal(';')),
  ...
  /** The document. */
  const Stylesheet = node('Stylesheet', many(Item), children => stylesheet(children as Statement[]));
...
/** The AST artifact. */
export const cssTerminalUpGrammar = composeLeaf([cssSyntax, cssPseudoSyntax, rules(
