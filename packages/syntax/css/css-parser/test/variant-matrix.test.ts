...
/**
 * The CSS quarter of the 16-cell variant matrix: {AST, CST} x {trackLines off, on}.
 *
...
describe('CSS variant matrix', () => {
  ...
  it('cell CST/trackLines=off — yields a CST whose spans carry offsets and NO line facts', () => {
    assertCstCell(parseCssCst(MATRIX_SOURCE).tree, { tracked: false, rootEnd: ROOT_SPAN_END });
  });

  it('cell CST/trackLines=on — yields a CST whose spans carry real line AND column', () => {
    assertCstCell(parseCssCstWithLines(MATRIX_SOURCE).tree, { tracked: true, rootEnd: ROOT_SPAN_END });
  });
  ...
  it('the two CST cells agree on offsets, so only the line facts differ', () => {
    const off = parseCssCst(MATRIX_SOURCE).tree;
    const on = parseCssCstWithLines(MATRIX_SOURCE).tree;
