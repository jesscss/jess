...
import { parse as parseWithPositions } from '@jesscss/scss-parser/positions';
import type { ScssParseError } from '@jesscss/scss-parser';
 ...
 */
function failureOf(source: string, entry: typeof parse = parse): ScssParseError {
  try {
    entry(source);
  } catch (error) {
    expect((error as Error).name).toBe('ScssParseError');
    return error as ScssParseError;
  }
...
describe('SCSS leftover-input errors', () => {
  it.each([
    ...
    expect(failure.reason).toContain('complete SCSS stylesheet');
  ...
  it.each([
    ['offsets-only entry', parse],
    ['line-aware entry', parseWithPositions]
  ])('localises a first token that is not SCSS at all (%s)', (_label, entry) => {
    const failure = failureOf(MALFORMED, entry);

    expect(failure.offset).toBe(0);
    expect(failure.line).toBe(1);
    expect(failure.column).toBe(1);
    expect(failure.message).toBe('Unexpected SCSS syntax.');
    expect(failure.reason).toContain('start of an SCSS rule');
