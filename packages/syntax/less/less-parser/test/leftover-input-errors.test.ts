...
import { parse as parseWithPositions } from '@jesscss/less-parser/positions';
import type { LessParseError } from '@jesscss/less-parser';
 ...
 */
function failureOf(source: string, entry: typeof parse = parse): LessParseError {
  try {
    entry(source);
  } catch (error) {
    expect((error as Error).name).toBe('LessParseError');
    return error as LessParseError;
  }
  throw new Error(`Expected ${JSON.stringify(source)} to fail to parse.`);
}

describe('Less leftover-input errors', () => {
  it.each([
    ...
    expect(failure.message).toBe('Unexpected Less input after a complete stylesheet.');
    expect(failure.reason).toContain('complete Less stylesheet');
  ...
  it.each([
    ['offsets-only entry', parse],
    ['line-aware entry', parseWithPositions]
  ])('localises a first token that is not Less at all (%s)', (_label, entry) => {
    ...
    expect(failure.reason).toContain('start of a Less rule');
    expect(failure.fix).toBeTruthy();
  });
