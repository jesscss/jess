...
import { parse as parseWithPositions } from '@jesscss/jess-parser/positions';
import type { JessParseError } from '@jesscss/jess-parser';
 ...
 */
function failureOf(source: string, entry: typeof parse = parse): JessParseError {
  try {
    entry(source);
  } catch (error) {
    expect((error as Error).name).toBe('JessParseError');
    return error as JessParseError;
  }
  throw new Error(`Expected ${JSON.stringify(source)} to fail to parse.`);
}

describe('Jess leftover-input errors', () => {
  it.each([
    ...
    expect(failure.message).toBe('Unexpected Jess input after a complete stylesheet.');
    expect(failure.reason).toContain('complete Jess stylesheet');
  ...
  it.each([
    ['offsets-only entry', parse],
    ['line-aware entry', parseWithPositions]
  ])('localises a first token that is not Jess at all (%s)', (_label, entry) => {
    ...
    expect(failure.reason).toContain('start of a Jess rule');
  ...
  it('reaches the leftover branch through an over-closed nested block', () => {
    const source = '.a { .b { color: red; } } }\n.c { color: blue; }';
    const failure = failureOf(source);

    expect(source[failure.offset]).toBe('}');
    expect(failure.line).toBe(1);
    expect(failure.column).toBe(27);
    expect(failure.message).toBe('Unexpected Jess input after a complete stylesheet.');
  });
  ...
  it('counts trailing trivia as consumed rather than left over', () => {
    /*
     * The root span stops at the last non-trivia token here, so span end is not
     * the coverage fact for these sources. Appending garbage is: it proves the
     * trivia was consumed and the parser resumed *after* it.
     */
