...
Jess currently supports both `set` and `with` when composing stylesheets:

- `set` mutates the imported module in compilation scope (shared downstream for that resolved module path).
- `with` applies overrides only for the current compose operation.
