...
### Namespaced `$extend` targets

An `$extend` namespace filters the **immediate module boundary** it names; it is
not a path that must repeat every namespace used by nested modules. When a
query crosses that boundary, the namespace segment is consumed and the
remaining selector is matched against the module's accessible mutable child
stylesheets.
...
Omitting the namespace is allowed: `$extend .box` searches all mutable
stylesheets accessible from the current extend. Use a namespace when you want
to filter that search to one immediate composed module.
