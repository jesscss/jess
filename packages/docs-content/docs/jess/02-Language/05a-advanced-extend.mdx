...
<Tabs>
  <TabItem value="jess" label="Jess">
    ...
    .bear {
      $extend .animal !exact;   // style .bear like .animal
      background: brown;
    }
    ```
  </TabItem>
...
- **Exact** — matches only where the target is the *whole* selector.
- **`all` (partial)** — matches the target *wherever it appears as part of* a
  larger selector, and rewrites that spot in place. Think non-destructive
  search-and-replace.

:::note Jess flips the default

In **Less**, `:extend(.b)` is exact and `:extend(.b all)` is partial. In
**Jess**, the `$extend .b;` statement defaults to the partial (`all`) match, and
you write `$extend .b !exact;` for an exact match.
...
`docs/future/core-architecture/EXTEND-SEMANTICS.md`.
