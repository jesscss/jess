...
| `$foo+: value` | merge-assign | merge into `foo` if it already exists (lists / sequences / collections) |
| `$foo := value` | global (non-shadowing) assign | reassign the existing outer binding rather than creating a new shadowing local |

```scss
$foo?: 1;        // sets foo to 1 only if it wasn't already defined
$list+: 1, 2;    // merges 1, 2 into the existing list
$foo := bar;     // reassign the outer `foo`, do not shadow
```

:::info

`$foo := bar` and the linear set `$^foo: bar` (see [Set linearly](#set-linearly))
both write the existing outer binding instead of shadowing. `:=` is the direct
operator spelling of that intent.
