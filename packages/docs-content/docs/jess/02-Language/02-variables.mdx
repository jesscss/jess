...
| `$foo+: value` | compound-add | sugar for `$foo: $foo + value`. What `+` does is delegated to `$foo`'s type — number → add, list → concat, string → join, etc. |
...
$n+: 1;          // $n: $n + 1  (compound-add; `+` follows $n's type)
