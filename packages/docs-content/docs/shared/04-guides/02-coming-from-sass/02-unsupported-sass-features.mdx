...
:::note
This page covers features that **parse** but do not **evaluate**. For input that Sass+ rejects outright as a **parse error** (invalid CSS that Sass tolerates — bogus combinators, escaped directive keywords, etc.), see [Stricter than Sass](./stricter-than-sass). For the full per-feature status of the SCSS dialect — supported, excluded by design, and not yet implemented — see the [Sass+ support matrix](./support-matrix).
:::
