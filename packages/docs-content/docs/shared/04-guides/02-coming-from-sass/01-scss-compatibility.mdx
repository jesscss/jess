...
Jess’s goal is to make **Sass input parseable**, while preferring **Jess-native output semantics** where they’re strictly better (for example, maps). In native Jess, `$name` is a live/current reference and `$$name` is the scoped/final lookup; `$!` is retired.
