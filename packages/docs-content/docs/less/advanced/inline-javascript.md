...
> now goes through `@use` / `@-use` script modules.
...
syntax no longer evaluates as JavaScript.
...
**Use a `@use` / `@-use` script module** when you genuinely need JavaScript. Script
modules replace the old `@plugin` file-execution path with an explicit, sandboxed
import:
