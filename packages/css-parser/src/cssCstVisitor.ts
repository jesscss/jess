...
/**
 * Only evaluate / create the visitor
 * when it's time to visit the CST.
 */
export const cstVisitor = {
  visit(parser: CssCstParser, cst: CstNode) {
    const CstVisitor = parser.parser.getBaseCstVisitorConstructor<CstNode, Node>()
    return (
      new (class CssCstVisitor extends CstVisitor {
        constructor() {
          super()
...
export class CstVisitor {
  ...
  visit(ctx: CstNode | BaseTokenType, Clazz?: any): any {
    ...
    if (!visit) {
      throw new Error(`CST '${(ctx as CstNode).name}' is not valid.`)
    }
 ...
 */
export const parse = (input: string, fileInfo: FileInfo = {}) => {
