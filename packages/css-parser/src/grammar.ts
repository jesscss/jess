...
const ws = regex(/[ \t\n\r\f]+/);
const comment = regex(/\/\*(?:[^*]|\*(?!\/))*\*\//);
const rw = trivia(oneOrMore(choice(label('whitespace', ws), label('blockComment', comment))));
...
export const cssGrammar = rules({ trivia: rw }, (g: any) => {
  ...
  const Declaration = node(
    ...
    sequence(propName, literal(':'), g.valueList, not(literal('{')), optional(important), optional(literal(';'))));
  /**
