...
export function qualifiedRule(this: C, T: TokenMap, selectorAlt?: AltContext) {
  ...
  return (ctx: RuleContext = {}) => {
    ...
    $.CONSUME(T.LCurly);
    let rules = $.SUBRULE($.declarationList, { ARGS: [ctx] }) as unknown as Rules;
    $.CONSUME(T.RCurly);
  ...
  return (ctx: RuleContext = {}) => {
    ...
    if (!$.RECORDING_PHASE) {
      ...
      if (!args) {
        ...
        args = new List(newData, undefined, loc ?? $.getLocationInfo(prop), this.context);
      }
      return new Call({
        name: 'var',
        args
...
export function mediaAtRule(this: C, T: TokenMap, preludeRule?: PreludeRule) {
  ...
  return (ctx: RuleContext = {}) => {
    ...
    $.CONSUME(T.LCurly);
    rules = $.SUBRULE($.atRuleBody, { ARGS: [ctx] }) as unknown as Rules;
    $.CONSUME(T.RCurly);
            ...
            },
            {
              ALT: () => {
                ...
                if (!RECORDING_PHASE) {
                  ...
                  let newData = [$.wrap(new Any(ident.image, { role: 'ident' }, $.getLocationInfo(ident), this.context)), ...seq.data];
                  return new QueryCondition(newData, undefined, [startOffset, startLine!, startColumn!, seq.location[3]!, seq.location[4]!, seq.location[5]!], this.context);
                }
                return seq;
              }
            },
...
export function keyframesAtRule(this: C, T: TokenMap) {
  ...
  return (ctx: RuleContext = {}) => {
    ...
    $.CONSUME(T.LCurly);
    const rules = $.SUBRULE($.declarationList, { ARGS: [ctx] }) as unknown as Rules;
    $.CONSUME(T.RCurly);
...
export function containerAtRule(this: C, T: TokenMap, preludeRule?: PreludeRule) {
  ...
  return (ctx: RuleContext = {}) => {
    ...
    $.CONSUME(T.LCurly);
    const rules = $.SUBRULE($.atRuleBody, { ARGS: [ctx] }) as unknown as Rules;
    $.CONSUME(T.RCurly);
...
export function supportsAtRule(this: C, T: TokenMap, preludeRule?: any) {
  ...
  return (ctx: RuleContext = {}) => {
    ...
    $.CONSUME(T.LCurly);
    let rules = $.SUBRULE($.atRuleBody, { ARGS: [ctx] }) as unknown as Rules;
    $.CONSUME(T.RCurly);
...
export function supportsInParens(this: C, T: TokenMap) {
  ...
  return (ctx: RuleContext = {}) => $.OR<any>(conditionAlt(ctx));
}
...
export function nestedAtRule(this: C, T: TokenMap) {
  ...
  return (ctx: RuleContext = {}) => {
    ...
    rules = $.SUBRULE($.declarationList, { ARGS: [ctx] }) as unknown as Rules;
    $.CONSUME(T.RCurly);
          ...
          $.OR9([
            {
              GATE: () => assumeDeclList,
              ALT: () => {
                declRules = $.SUBRULE($.atRuleBody, { ARGS: [{ ...ctx, inner: true }] }) as unknown as Rules;
              }
            },
            {
              GATE: () => !assumeDeclList,
