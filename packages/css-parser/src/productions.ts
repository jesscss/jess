  ...
  let ruleAlt = alt ?? [
    {
      ALT: () => {
        ...
        if (!$.RECORDING_PHASE) {
          let nameNode = $.wrap(new Flag(name!.image, undefined, $.getLocationInfo(name!), this.context), true);
...
export function customBlock(this: C, T: TokenMap, alt?: Alt) {
  ...
  return () => {
    ...
    if (!RECORDING_PHASE) {
      ...
      if (type) {
        let seq = new Sequence(nodes!, undefined, undefined, this.context);
        if (type === 'paren') {
          ...
          rule = $.OR2([
            {
              ALT: () => {
                ...
                if (!RECORDING_PHASE) {
                  ...
                  return $.wrap(
                    new Declaration({
                      name: $.wrap(new Name(ident.image), true),
                      value: $.wrap(value)
                    }, undefined, location, this.context),
                    'both');
                }
