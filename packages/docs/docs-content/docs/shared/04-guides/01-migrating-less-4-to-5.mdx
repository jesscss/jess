...
- Prefer a plain Less expression or function when one exists
  (`@columns: max(12, 8);`).
- For genuine scripting, import the module with `@use`; local or package
  JavaScript and TypeScript require `@jesscss/plugin-js`. Use the deprecated
  `@plugin` bridge only for legacy Less plugins.
