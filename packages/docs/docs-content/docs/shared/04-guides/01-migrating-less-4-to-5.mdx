...
| `when (~"true")` | truthy | **falsy** | Less 4 re-parsed the escaped string's bytes back through evaluation, so `~"true"` arrived as the keyword `true`. **A quote is a quote** — its contents must not decide truthiness. |
