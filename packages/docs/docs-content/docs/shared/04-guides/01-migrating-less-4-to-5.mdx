...
Where you relied on backtick JS, migrate to a plain expression or the current
plugin boundary:

- Prefer a plain Less expression or function when one exists
  (`@columns: max(12, 8);`).
- For genuine scripting, use a preloaded plugin or the deprecated `@plugin`
  bridge until script modules ship.
...
built-in functions where possible (`@columns: max(12, 8);`). Script and data
imports with `@-use` / `@-from` are still a syntax preview; see
...
module system, **`@compose`** — a namespaced import with isolated scope. Your
existing `@import` keeps working (with a warning), so migrate at your own pace.
