...
That is one library, not the ecosystem. If you maintain a Sass library that relies on `""` or `()` being truthy in a bare `@if`, this is the change that affects you, and the fix is to write the comparison explicitly — which is portable to both.
