...
`@-use` imports a JS/TS/JSON namespace, while `@-from` imports named,
default, or namespace exports. When parsing SCSS, Jess treats `@use` as a
stylesheet import by default; a path that clearly targets `.js`, `.ts`, or
`.json` is a script or data module. Built-in `sass:*` modules are available
without a script runtime. See [Modules & imports](/docs/language/modules-and-imports)
for the canonical syntax and runtime requirements.
