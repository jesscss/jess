...
Jess stylesheet imports support namespaces, isolated scope, configuration, and
forwarding through `@-export`.

### `@use` (scripts) → `@-use` / `@-from`

This is a syntax preview in the current alpha. `@-use` is planned as the
namespace form for JS/TS/JSON modules, while `@-from` is planned for named,
default, or namespace imports. The evaluator does not bind those imports yet.

When parsing SCSS, Jess treats `@use` as a stylesheet import by default. A path
that clearly targets `.js`, `.ts`, or `.json` parses to a script-module
node, which currently round-trips without binding exports. See
...
- A quoted key such as `"regular"` remains a `Quoted` value in the current AST,
  while bare `regular` is a `Keyword`. Sass equality treats those keys as equal;
  quote-normalized storage and output are recorded follow-up work.
