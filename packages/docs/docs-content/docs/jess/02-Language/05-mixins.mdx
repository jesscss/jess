...
A parametric mixin may call itself with **different** arguments to build a loop. A guard is the termination condition — the recursion stops when the guard no longer matches:

```less
.loop($n) when ($n > 0) {
  .item-${n} { width: $n; }
  .loop($n - 1); // recurse with a smaller value
}
.loop(3);
```
→
...
- **A plain (non-parametric) ruleset that references its own name is _not_ recursive.** Because it takes no arguments, re-entering it could not make any progress, so Jess skips the self-reference and the ruleset renders exactly once — it never loops and never errors. (The enclosing definition is simply excluded from the set of candidates for that call. This is the same "a self-reference that can't progress is skipped" principle that lets `$a: $a + 1` read the previous `$a` instead of looping.)
- **A recursive mixin with a broken guard is caught.** If a parametric mixin recurses without ever terminating (for example `.loop($n) when ($n > 0) { .loop($n + 1); }`), Jess stops it at a high recursion-depth limit and raises a clean error, `maximum mixin recursion depth exceeded`, rather than crashing with a stack overflow. Legitimate guarded recursion runs far below this limit and is unaffected.
...
## Passing a block of rules to a mixin

Like Sass, a mixin **call** can carry a block of rules. Write a `:` after the call's `)`, then the block. Inside the mixin, `$content()` renders that block wherever it sits.
...
:::info[No `@` in the content slot]
A standalone [anonymous mixin](#anonymous-mixins) is written `@{ … }` or `@() { … }`. In the content slot the `@` is dropped: after a complete call, a leading `:` can only introduce a content block, so the `:` alone is the signifier.
:::

### `content` is an ordinary variable

`content` is not a special name. Passing a block **binds an ordinary variable named `content` in the call frame**, the way `arguments` is bound in a JavaScript function, and `$content()` is an ordinary call on it. Two things follow from that.

**Call-site variables are visible inside the block.** The block is evaluated where it was written, so it closes over the caller's scope:
...
**Calling `$content()` when no block was passed is a plain unbound-name error**, not a silent no-op. See [Sass+ semantic differences](/docs/guides/coming-from-sass/semantic-differences#content-blocks-content-is-an-ordinary-variable) for how that differs from Sass.

### Blocks with parameters

Give the block a parameter list in `(…)` after the `:`, and call it in **value position** as `$content($value)`. The block then behaves like an anonymous function: the value you pass fills its parameter, and its `result` is what `$content($value)` returns.
...
In Sass this parameter list is spelled `@include m using ($base) { … }`. Note the position: Jess's `$content($value)` is a **value-position** call, while the plain `$content()` that renders a block in statement position takes no arguments.
