...
Jess compiler at-rules use a dash prefix: `@-compose`, `@-use`, `@-from`,
`@-export`, and `@-import`. Bare CSS at-rules stay bare, so Jess can add
compile-time behavior without taking names away from CSS.

:::caution[Module loading is not wired yet]

These module at-rules parse and round-trip in the current alpha, but no resolver
loads or evaluates their targets yet. The examples below document the settled
syntax and intended behavior.
...
- `set` establishes a one-time, compilation-wide configuration for the
  resolved module. Later imports of that module reuse the same values.
- `with` provides overrides for the current compose only. A later compose
  starts from the module's defaults unless it supplies its own configuration.
...
Mutability belongs to the composed module, not to the selector name: another
stylesheet can define its own `.box` and remain protected when composed without
`mutable`.

:::note Planned: explicit protected boundaries

`@-compose (protected)` is the planned explicit opt-out from inherited
mutability. A mutable library can use it when one of its child modules must
remain closed to downstream extends:

```less
// library.jess
@-compose (protected) './base.jess';
```

This is a follow-up to the default-protected and recursive-mutable compose
...
An `$extend` target does not need a namespace. A bare target such as
`$extend .box` searches all mutable stylesheets accessible from the current
extend.

Use a namespace only when you want to filter that search to one immediate
composed module. The namespace is not a path that must repeat every namespace
used by nested modules. When a query crosses the named boundary, that namespace
segment is consumed and the remaining selector is matched against the module's
accessible mutable child stylesheets.

```less
// library.jess
@-compose './base.jess' as foundation;
```
...
Here, `library|.box` enters `library.jess`, then applies `.box` to its
accessible mutable child stylesheets. The `foundation` namespace still exists
inside `library.jess`; an extend written there can use
`$extend foundation|.box`. The outer query does not need to spell
`foundation`.
