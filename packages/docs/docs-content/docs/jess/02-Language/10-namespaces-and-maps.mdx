...
Jess collection literals currently author the common CSS-object shape directly:
...
Bracketed value-key entries are the intended spelling for non-identifier keys,
but they are not public Jess syntax until the parser work lands. When that work
is ready, non-identifier keys should use `[value]:` entries such as `[1px]: a`,
`[red]: b`, and `[$key]: c`.
...
The trade-off is deliberate: if you use numbers as collection keys, numeric
`[…]` will not reach them, because a numeric subscript is always a position. Use
map functions such as `map.get($m, 1px)` for those until bracketed value-key
lookup syntax is completed. Positional indexing is the common case and numeric
keys are rare, so the rare case is the one that takes an explicit value-key
operation.
...
Similarly, even though Sass added the concept of namespaces, it doesn't use it for maps, instead using the `map-get` (or `map.get`) function.
...
The practical DX difference is that Jess gives the common case the simple syntax:
CSS-shaped data, JavaScript-shaped property access, and bracket lookup when the
key is a value. You do not have to pretend a ruleset is data, and you do not have
to wrap every ordinary read in `map.get(...)`.
