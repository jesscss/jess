...
| **quoted strings**, `~"…"`, and quoted `url("…")` bodies | `${…}`, `$(…)` | `$[…]` |
| **unquoted declaration-value `url(...)` bodies** | `${…}`; every other `$` is literal URL text | `$(…)` as an expression; `$[…]` never a lookup |
...
$family: "modern";
.fonts {
  src: url("fonts/${family}.css");
}
```
→
```css
.fonts {
  src: url("fonts/modern.css");
}
```
