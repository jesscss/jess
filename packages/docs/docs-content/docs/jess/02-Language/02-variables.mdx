...
[Conditionals and iteration](./conditionals-iteration).

:::caution[Not enforced yet for `$if` and `$while`]

Today only `$for` enforces this. A `:` declaration inside `$if` or `$while`
still leaks into the surrounding block, and `:=` inside `$for` fails to find an
outer binding — [jess#232](https://github.com/jesscss/jess/issues/232). Write
