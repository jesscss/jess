...
Jess lets you reuse any selector as a mixin—just include it with `$ > .some-class;` (without parentheses).
...
Jess supports passing sets of rules to mixins by "assigning" an anonymous mixin callback to the mixin call. The mixin can then render rules using the built-in `content` mixin.
