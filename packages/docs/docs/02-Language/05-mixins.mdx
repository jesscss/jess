...
In Jess, like Less, mixins can look like a simple selector (`tag`, `.class`, or `#id`), followed by parentheses
...
Mixins are included ("mixed in") to other rules using the [CSS namespace separator](https://developer.mozilla.org/en-US/docs/Web/CSS/Namespace_separator), `|`.
...
:::info[Compound mixin names]

Like Less, mixin names can actually be compound selectors - sequences of (`.class`) or id (`#id`) selectors. In addition, Jess mixin names can be plain identifiers. This is done to support seamless migration from Less & Sass to Jess. All mixin name types are supported.
...
One of the reasons Jess uses the namespace separator is because we are including all matching mixins in the output within that namespace.
...
Jess lets you reuse any selector as a mixin. In Jess, we can save a selector as a value with `*(.selector)`. To use this value as a mixin, we can include it as part of an expression e.g. `$*(.selector)();`.
...
To do so, you drop the parentheses after the selector from the above example, and use `*name` to mix in matching selectors and mixins.
