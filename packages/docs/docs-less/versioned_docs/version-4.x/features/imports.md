...
When importing a file relative to the current file, prefer `./` for clarity. Less also supports bare specifiers (for example `@import "my-module";`) which often resolve package/module imports, so explicit `./` helps signal local-file intent.
...
Imagine that `reference` marks every at-rule and selector with a _reference flag_ in the imported file, imports as normal, but when the CSS is generated, "reference" selectors (as well as any media queries containing only reference selectors) are not output. `reference` styles will not show up in your generated CSS unless the reference styles are used as [mixins](./mixins) or [extended](./extend).
...
* **[mixins](./mixins)**: When a `reference` style is used as an [implicit mixin](./mixins), its rules are mixed-in, marked "not reference", and appear in the referenced place as normal.
