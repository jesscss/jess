...
The older bare form (`@media @breakpoint`) still compiles but emits a deprecation
warning, and is not accepted by Less 5.x. Use `@{breakpoint}` when a variable is
part of at-rule syntax; ordinary declaration values continue to use `@breakpoint`.
...
This works fine because of [Lazy Evaluation](#lazy-evaluation) - `@base-color` is overridden and `@dark-color` is a dark red.
